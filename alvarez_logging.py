"""Centralized logging setup for Alvarez Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.alvarez/logs/`` (profile-aware via ``get_alvarez_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``alvarez_cli.web_server``,
    ``alvarez_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Alvarez (TUI, gateway, ``hy_memory`` server, MCP
...
from alvarez_constants import get_config_path, get_alvarez_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_alvarez_session_injector", False):
    ...
    _session_record_factory._alvarez_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``alvarez logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``alvarez logs --component gateway``.
    "gateway": ("gateway", "alvarez_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("alvarez_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "alvarez_cli.web_server",
        "alvarez_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    alvarez_home: Optional[Path] = None,
    ...
    """Configure the Alvarez logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    alvarez_home
        Override for the Alvarez home directory.  Falls back to
        ``get_alvarez_home()`` (profile-aware).
    log_level
    ...
    home = alvarez_home or get_alvarez_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_alvarez_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._alvarez_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the alvarez group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from alvarez_cli.config import is_managed
