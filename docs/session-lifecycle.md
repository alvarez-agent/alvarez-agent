...
The canonical transcript store is SQLite via `SessionDB` (from `alvarez_state`). The
...
Counts consecutive restarts via a JSON file (`{ALVAREZ_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `alvarez update`, `alvarez gateway restart`,
or `/restart`.
