...
When running Alvarez inside Docker, the default `network_mode: host` gives the
...
The Alvarez [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ alvarez-agent │   │ alvarez-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   ALVAREZ_UID=$(id -u) ALVAREZ_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=alvarez,alvarez-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://alvarez-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Alvarez trust model and vulnerability reporting
