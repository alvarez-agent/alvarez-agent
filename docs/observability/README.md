# Alvarez Observer Hooks

Alvarez observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Alvarez' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "alvarez.observer.v1"
```

Hook callbacks are fail-open. Alvarez catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Alvarez only builds
