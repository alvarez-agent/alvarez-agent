...
Alvarez middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Alvarez'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Alvarez calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Alvarez retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Alvarez guardrails,
...
- `telemetry_schema_version`: currently `alvarez.observer.v1`
- `middleware_schema_version`: currently `alvarez.middleware.v1`
...
Alvarez stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Alvarez runs
them as a nested chain in registration order. Middleware failures are fail-open:
Alvarez logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Alvarez applies middleware in this order:
...
For each tool call, Alvarez applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Alvarez pre-execution path against the effective arguments:
...
```bash
alvarez plugins enable <plugin-name>
```

For isolated local testing, use one `ALVAREZ_HOME` for plugin enablement and the
agent run:

```bash
export ALVAREZ_HOME=/tmp/alvarez-middleware-test
mkdir -p "$ALVAREZ_HOME"
alvarez plugins enable <plugin-name>
alvarez chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run alvarez plugins enable <plugin-name>
uv run alvarez chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["alvarez_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/alvarez-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Alvarez expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Alvarez treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Alvarez preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Alvarez does not convert downstream
