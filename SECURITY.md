# Alvarez Agent Security Policy

This document describes Alvarez Agent's trust model, names the one
...
- Environment details (`alvarez version`, commit SHA, OS, Python
  version).
...
Alvarez Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Alvarez Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Alvarez Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Alvarez Agent's documentation
...
Alvarez Agent supports two OS-level isolation postures. They address
...
Alvarez Agent supports this in two ways:

- **Alvarez Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Alvarez Agent's in-process heuristics
...
Alvarez Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Alvarez Agent
itself. Bugs in Alvarez Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Alvarez Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Alvarez Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Alvarez Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  alvarez-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Alvarez Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Alvarez Agent. Bugs in Alvarez
...
- Alvarez Agent includes supply-chain guards for MCP server
