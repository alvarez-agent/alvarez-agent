#!/usr/bin/env python3
"""
Alvarez Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Alvarez Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills alvarez-agent-dev,github-auth
...
os.environ["ALVAREZ_QUIET"] = "1"  # Our own modules

import yaml

from alvarez_cli.fallback_config import get_fallback_chain
from alvarez_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from alvarez_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from alvarez_cli.pt_input_extras import (
...
from alvarez_cli.banner import _format_context_length, format_banner_version_label
...
from alvarez_cli.browser_connect import (
...
load_alvarez_dotenv(alvarez_home=_alvarez_home, project_env=_project_env)
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("ALVAREZ_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.alvarez/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ALVAREZ_IGNORE_USER_CONFIG=1 is set (via ``alvarez chat --ignore-user-config``),
    the user config at ``~/.alvarez/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ALVAREZ_HOME}/config.yaml)
    user_config_path = _alvarez_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ALVAREZ_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "reasoning_effort": "",
            "service_tier": "",
            # Built-in moods live in alvarez_cli/config.py DEFAULT_CONFIG
            # (single source of truth); filled in below after the literal.
            "moods": {},
        },
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see alvarez_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    # Built-in moods: single source of truth is the full config schema.
    try:
        from alvarez_cli.config import DEFAULT_CONFIG as _full_defaults
        defaults["agent"]["moods"] = dict(_full_defaults["agent"]["moods"])
    except Exception:
        pass
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and AlvarezCLI.__init__ checks "default" first.
    ...
    # alvarez_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `alvarez config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (alvarez_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && alvarez` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _ALVAREZ_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_ALVAREZ_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ALVAREZ_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.alvarez/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from alvarez_logging import setup_logging
...
try:
    from alvarez_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``alvarez --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from alvarez_cli.mcp_startup import wait_for_mcp_discovery
...
from alvarez_cli.banner import build_welcome_banner
from alvarez_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from alvarez_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("ALVAREZ_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("ALVAREZ_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.alvarez/alvarez-agent`` clone is updated only by ``alvarez update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run alvarez -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"alvarez-{short_id}"
    branch_name = f"alvarez/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"alvarez pid={os.getpid()}", str(wt_path)],
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`alvarez_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from alvarez_constants import get_alvarez_home as _get_alvarez_home
        _alvarez_home_maint = _get_alvarez_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_alvarez_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`alvarez_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``alvarez/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("alvarez-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``alvarez/alvarez-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``alvarez -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("alvarez/alvarez-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. ALVAREZ_LIGHT / ALVAREZ_TUI_LIGHT env (true/false) — explicit override
#   2. ALVAREZ_TUI_THEME=light|dark — explicit theme
#   3. ALVAREZ_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Alvarez assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("ALVAREZ_LIGHT", "ALVAREZ_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("ALVAREZ_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("ALVAREZ_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from alvarez_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_alvarez_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._alvarez_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from alvarez_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Alvarez output it's common to display cron schedules verbatim.
    #
...
from alvarez_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_alvarez_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_alvarez_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._alvarez_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_alvarez_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._alvarez_bp_start = now
        ...
        _vt100_mod._alvarez_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``AlvarezCLI._busy_command()``.
        """
        yield self

# ASCII Art - ALVAREZ-AGENT logo (full width, single line - requires ~95 char terminal)
ALVAREZ_AGENT_LOGO = """[bold #FFD700] █████╗ ██╗     ██╗   ██╗ █████╗ ██████╗ ███████╗███████╗[/]
[bold #FFD700]██╔══██╗██║     ██║   ██║██╔══██╗██╔══██╗██╔════╝╚══███╔╝[/]
[#FFBF00]███████║██║     ██║   ██║███████║██████╔╝█████╗    ███╔╝ [/]
[#FFBF00]██╔══██║██║     ╚██╗ ██╔╝██╔══██║██╔══██╗██╔══╝   ███╔╝  [/]
[#CD7F32]██║  ██║███████╗ ╚████╔╝ ██║  ██║██║  ██║███████╗███████╗[/]
...
# ASCII Art - Alvarez Caduceus (compact, fits in left panel)
ALVAREZ_CADUCEUS = """[#B8860B]⠀⠀⠀⠀⢀⣀⣤⣤⣤⣤⣤⣤⣤⣀⡀⠀⠀⠀⠀[/]
[#B8860B]⠀⠀⣠⣶⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣄⠀⠀[/]
[#FFBF00]⠀⣼⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣧⠀[/]
[#FFBF00]⣰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣆[/]
[#FFD700]⣿⣿⡿⠿⠿⠿⠿⠿⠿⠿⠿⠿⠿⠿⢿⣿⣿⣿[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "ALVAREZ - AI Agent Framework"
        tiny_line = "ALVAREZ"
    else:
        agent_name = _skin.get_branding("agent_name", "Alvarez Agent") if _skin else "Alvarez Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("ALVAREZ_FAST_STARTUP_BANNER") == "1":
        from alvarez_cli import __release_date__ as _release_date
        from alvarez_cli import __version__ as _version

        version_line = f"Alvarez Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from alvarez_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.alvarez/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.alvarez/config.yaml on first use)
...
class AlvarezCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Alvarez Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: ALVAREZ_MAX_TOKENS)
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from alvarez_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("ALVAREZ_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ALVAREZ_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ALVAREZ_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `alvarez chat --ignore-rules` in alvarez_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("ALVAREZ_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ALVAREZ_EPHEMERAL_SYSTEM_PROMPT", "")
            or CLI_CONFIG["agent"].get("system_prompt", "")
        )
        self.moods = CLI_CONFIG["agent"].get("moods", {})
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `alvarez update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Alvarez processes for this ALVAREZ_HOME.
        ...
        self._history_file = _alvarez_home / ".alvarez_history"
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        """
        try:
            from alvarez_cli.curses_ui import flush_stdin
            flush_stdin()
        except Exception:
            pass
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``alvarez pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from alvarez_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Alvarez'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from alvarez_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from alvarez_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from alvarez_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from alvarez_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Alvarez")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - AlvarezCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``alvarez doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``alvarez doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Alvarez needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from alvarez_cli.model_switch import is_nous_alvarez_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_alvarez_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Hermes 3 & 4 models are NOT agentic and are not "
                "designed for use with Alvarez Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.alvarez/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from alvarez_cli.clipboard import save_clipboard_image

        img_dir = get_alvarez_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'alvarez setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from alvarez_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from alvarez_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from alvarez_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Alvarez!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from alvarez_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from alvarez_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [Alvarez #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``alvarez sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `alvarez sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ALVAREZ_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.alvarez/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``alvarez --resume <id>``
        ...
        saved_dir = get_alvarez_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"alvarez_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: alvarez --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from alvarez_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if not getattr(result, "success", False):
            return True
        try:
            from alvarez_cli.model_cost_guard import expensive_model_warning
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Alvarez session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from alvarez_cli.context_switch_guard import merge_preflight_compression_warning
        ...
        try:
            from alvarez_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `alvarez model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from alvarez_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from alvarez_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from alvarez_cli.model_switch import (
            switch_model,
            parse_model_flags,
            resolve_persist_behavior,
        )
        from alvarez_cli.providers import get_label
        ...
        # clobber disk config.
        from alvarez_cli.inventory import build_models_payload, load_picker_context
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from alvarez_cli.commands import resolve_command
    ...
    @staticmethod
    def _resolve_mood_prompt(value) -> str:
        """Accept string or dict mood value; return system prompt string."""
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_alvarez_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_alvarez_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in alvarez_cli/commands.py instead of touching every dispatch site.
        from alvarez_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from alvarez_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from alvarez_cli.skin_engine import get_active_skin
            ...
            # Use original case (handler lowercases the mood name itself)
            self._handle_mood_command(cmd_original)
        elif canonical == "pet":
            ...
            from alvarez_cli.main import _print_version_info
            ...
            from alvarez_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `alvarez plugins
                ...
                from alvarez_cli.plugins_cmd import (
                ...
                # catalog lives behind `alvarez plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: alvarez plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_alvarez_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: alvarez plugins list)")
                else:
                    # Loaded-plugin details (tools/hooks/commands counts, errors)
                    # keyed by name, when available.
                    loaded: dict = {}
                    try:
                        from alvarez_cli.plugins import get_plugin_manager
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: alvarez plugins list)")
                    print("  Enable/disable: alvarez plugins enable/disable <name>")
            except Exception as e:
            ...
            # Check for plugin-registered slash commands
                ...
                from alvarez_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from alvarez_cli.commands import COMMANDS
    ...
    def _toggle_verbose(self):
        ...
        from alvarez_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ALVAREZ_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ALVAREZ_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from alvarez_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by alvarez_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from alvarez_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.alvarez/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "alvarez_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _alvarez_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard ALVAREZ_HOME, so no -p needed.
            try:
            ...
            print(f"  alvarez --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  alvarez -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from alvarez_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from alvarez_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from alvarez_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Alvarez Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Alvarez Agent! Type your message or /help for commands."
        ...
        try:
            from alvarez_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(ALVAREZ_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Alvarez migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from alvarez_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in alvarez_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from alvarez_cli.model_switch import resolve_persist_behavior
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Alvarez binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from alvarez_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Alvarez Agent")
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if (lines_hit or chars_hit) and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _alvarez_home / "pastes"
        ...
        # Dynamic prompt: shows Alvarez symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Alvarez needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Alvarez needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Alvarez needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = AlvarezCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_alvarez_osd_patched", False):
                ...
                _pt_renderer._alvarez_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ALVAREZ_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ALVAREZ_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the alvarez process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: alvarez setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `alvarez sessions list`
...
def _run_kanban_goal_loop_q(cli: "AlvarezCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``ALVAREZ_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("ALVAREZ_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from alvarez_cli import kanban_db as _kb
    from alvarez_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Alvarez Agent CLI - Interactive AI Assistant
    ...
    os.environ["ALVAREZ_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Alvarez Gateway (messaging platforms)...")
    ...
    # Default to alvarez-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Alvarez): with no explicit --toolsets, collapse
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `alvarez skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    # mode registers its own inside AlvarezCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  ALVAREZ_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("ALVAREZ_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``alvarez chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("ALVAREZ_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from alvarez_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Alvarez" box, no tool-gen
                        ...
                        if os.environ.get("ALVAREZ_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("ALVAREZ_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from alvarez_cli.kanban_db import (
                ...
                # Single-query mode (`alvarez chat -q "…"`): skip the welcome
