 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.alvarez/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.alvarez', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (ALVAREZ_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.ALVAREZ_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.alvarez', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.ALVAREZ_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.alvarez', 'document_cache');
const AUDIO_CACHE_DIR = process.env.ALVAREZ_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.alvarez', 'audio_cache');
...
// `alvarez update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Alvarez Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Alvarez Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Alvarez and trigger
      ...
      // Ignore Alvarez' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `alvarez_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
