# ============================================================================
# Alvarez Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$AlvarezHome = $(if ($env:ALVAREZ_HOME) { $env:ALVAREZ_HOME } else { "$env:LOCALAPPDATA\alvarez" }),
    [string]$InstallDir = $(if ($env:ALVAREZ_HOME) { "$env:ALVAREZ_HOME\alvarez-agent" } else { "$env:LOCALAPPDATA\alvarez\alvarez-agent" }),
    ...
    # builds apps/desktop into a launchable Alvarez.exe.
    #
    # Why opt-in:
    #   * Alvarez-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Alvarez.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Alvarez.exe
    ...
    #     `alvarez desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "|             * Alvarez Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AlvarezHome)) {
        New-Item -ItemType Directory -Force -Path $AlvarezHome | Out-Null
    }
    $envFile = Join-Path $AlvarezHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AlvarezHome "node"
...
function Install-Uv {
    # Alvarez owns its own uv at $AlvarezHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (alvarez_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `alvarez update` stay in sync.
    $managedUv = Join-Path $AlvarezHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AlvarezHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AlvarezHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AlvarezHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AlvarezHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ALVAREZ_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Alvarez-Setup.exe
...
function Install-Git {
    ...
    which Alvarez uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\alvarez\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Alvarez needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Alvarez copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\alvarez\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ALVAREZ_GIT_BASH_PATH`` (User scope) so Alvarez can find it in a fresh
    ...
    # Download PortableGit into $AlvarezHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AlvarezHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Alvarez features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Alvarez needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ALVAREZ_GIT_BASH_PATH`` (User env scope) so Alvarez can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AlvarezHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AlvarezHome\git\usr\bin\bash.exe
    $candidates += "$AlvarezHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AlvarezHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ALVAREZ_GIT_BASH_PATH", $candidate, "User")
            $env:ALVAREZ_GIT_BASH_PATH = $candidate
            Write-Info "Set ALVAREZ_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Alvarez may not find Git Bash."
    Write-Info "If needed, set ALVAREZ_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$AlvarezHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AlvarezHome\node;$env:Path"
        Write-Success "Node.js $version found (Alvarez-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Alvarez-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AlvarezHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AlvarezHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\alvarez-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AlvarezHome\node") { Remove-Item -Recurse -Force "$AlvarezHome\node" }
                Move-Item $extractedDir.FullName "$AlvarezHome\node"
                ...
                $version = & "$AlvarezHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AlvarezHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\alvarez-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `alvarez
                    # update` path (#4735).
                    ...
                    $stashName = "alvarez-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``alvarez update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Alvarez behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running alvarez processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\alvarez-agent-extract"
    ...
    # next `alvarez update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Alvarez-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running alvarez process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running alvarez processes before recreating venv..."
            # The launcher CLI (alvarez.exe) plus its child tree.
            & taskkill /F /T /IM alvarez.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM alvarez.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m alvarez_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not alvarez.exe.
            ...
            # user (see alvarez_cli/gateway_windows.py), so the installer always
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `alvarez.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install alvarez-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and alvarez.exe broken with
    ...
    # here, not 30 seconds later when the user runs `alvarez`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register alvarez.exe in dist-info/RECORD but fail to
        ...
        # `alvarez` command while alvarez-agent.exe / alvarez-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m alvarez_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `alvarez dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `alvarez dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `alvarez dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up alvarez command..."
    
    if ($NoVenv) {
        $alvarezBin = "$InstallDir"
    } else {
        $alvarezBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so alvarez is globally available
    # On Windows, the alvarez.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$alvarezBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$alvarezBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $alvarezBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ALVAREZ_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\alvarez instead
    # of the Unix default ~/.alvarez
    $currentAlvarezHome = [Environment]::GetEnvironmentVariable("ALVAREZ_HOME", "User")
    if (-not $currentAlvarezHome -or $currentAlvarezHome -ne $AlvarezHome) {
        [Environment]::SetEnvironmentVariable("ALVAREZ_HOME", $AlvarezHome, "User")
        Write-Success "Set ALVAREZ_HOME=$AlvarezHome"
    }
    $env:ALVAREZ_HOME = $AlvarezHome
    
    # Update current session
    $env:Path = "$alvarezBin;$env:Path"
    
    Write-Success "alvarez command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.alvarez-bootstrap-complete which tells the Alvarez
    ...
    # Alvarez-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `alvarez update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".alvarez-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ALVAREZ_HOME directory structure ($AlvarezHome, default %LOCALAPPDATA%\alvarez)
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlvarezHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AlvarezHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AlvarezHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Alvarez's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$AlvarezHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in alvarez_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $AlvarezHome"
    
    # Seed bundled skills into $AlvarezHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AlvarezHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $AlvarezHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AlvarezHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $AlvarezHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Alvarez-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'alvarez setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\alvarez-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\alvarez-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\alvarez-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Alvarez` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Alvarez.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_alvarez_desktop command
    # resolves apps/desktop/release/win-unpacked/Alvarez.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Alvarez-managed
    ...
    # The --dir mode produces an unpacked Alvarez.exe in
    ...
    # are NOT trying to work around). The Alvarez icon + product name are
    # stamped onto Alvarez.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\alvarez-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Alvarez.exe",
        "$desktopDir\release\win-arm64-unpacked\Alvarez.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Alvarez.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Alvarez icon + identity are stamped onto Alvarez.exe by the
    ...
    #    Alvarez.exe. We deliberately do NOT point them at `alvarez desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Alvarez.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Alvarez.lnk')
        )
        ...
        # with the Alvarez icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.alvarez/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'alvarez setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m alvarez_cli.main setup
    } else {
        python -m alvarez_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $alvarezCmd = "$InstallDir\venv\Scripts\alvarez.exe"
    if (-not (Test-Path $alvarezCmd)) {
        $alvarezCmd = "alvarez"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AlvarezHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'alvarez whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: alvarez gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AlvarezHome\logs\gateway.log"
            Start-Process -FilePath $alvarezCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AlvarezHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: alvarez gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: alvarez gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AlvarezHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlvarezHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlvarezHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlvarezHome\alvarez-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   alvarez              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   alvarez setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   alvarez config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   alvarez config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   alvarez gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   alvarez update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Alvarez repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Alvarez-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Alvarez to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
