...
if [ -n "${ALVAREZ_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ALVAREZ_INSTALL_DIR"
...
#   code at /usr/local/lib/alvarez-agent, command at /usr/local/bin/alvarez,
#   data still at /root/.alvarez (ALVAREZ_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ALVAREZ_HOME/.no-bundled-skills so future"
            echo "                   'alvarez update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Alvarez.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.alvarez/alvarez-agent"
            echo "                   default (root, Linux): /usr/local/lib/alvarez-agent"
            echo "  --alvarez-home PATH  Data directory (default: ~/.alvarez, or \$ALVAREZ_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Alvarez installs the code under"
            echo "  /usr/local/lib/alvarez-agent and links the command into"
            echo "  /usr/local/bin/alvarez (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ALVAREZ_HOME"
            echo "  (default /root/.alvarez).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ALVAREZ_HOME/alvarez-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs alvarez setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `alvarez update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Alvarez-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Alvarez Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install alvarez command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `alvarez` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ALVAREZ_HOME/alvarez-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ALVAREZ_HOME/alvarez-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/alvarez-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ALVAREZ_HOME/alvarez-agent — then preserve it)
#
# Always no-op when the user set --dir or $ALVAREZ_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ALVAREZ_HOME.
    if is_termux; then
        INSTALL_DIR="$ALVAREZ_HOME/alvarez-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ALVAREZ_HOME/alvarez-agent/.git" ]; then
            INSTALL_DIR="$ALVAREZ_HOME/alvarez-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/alvarez-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/alvarez-agent"
        ...
        # /usr/local/bin/alvarez wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/alvarez"
        log_info "  Data:    $ALVAREZ_HOME (unchanged)"
...
# Point a Alvarez-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ALVAREZ_HOME/node/bin — which is NOT on
...
# Alvarez's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Alvarez-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ALVAREZ_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ALVAREZ_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ALVAREZ_HOME/node/etc/npmrc"
}

get_alvarez_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/alvarez" ]; then
        echo "$link_dir/alvarez"
    else
        echo "alvarez"
    fi
}
...
install_uv() {
    ...
    # Alvarez owns its own uv at $ALVAREZ_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (alvarez_cli/managed_uv.py) looks in the same
    # place, so install.sh and `alvarez update` stay in sync.
    local _managed_uv="$ALVAREZ_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ALVAREZ_HOME/bin ..."
    mkdir -p "$ALVAREZ_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/alvarez-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/alvarez-uv-installer.$$.sh")"
    ...
    # directly into $ALVAREZ_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ALVAREZ_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Alvarez-
...
check_node() {
    ...
    # Repair pre-existing Alvarez-managed installs where `npm install -g` lands
    ...
    # Prefer a Alvarez-managed Node from a previous run over a too-old system one.
    if [ -x "$ALVAREZ_HOME/node/bin/node" ] && node_satisfies_build "$("$ALVAREZ_HOME/node/bin/node" --version)"; then
        export PATH="$ALVAREZ_HOME/node/bin:$PATH"
        log_success "Node.js $("$ALVAREZ_HOME/node/bin/node" --version) found (Alvarez-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Alvarez-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.alvarez/node/..."
    ...
    # Place into ~/.alvarez/node/ and symlink binaries into the same bin dir
    # the alvarez command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ALVAREZ_HOME/node"
    mkdir -p "$ALVAREZ_HOME"
    mv "$extracted_dir" "$ALVAREZ_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ALVAREZ_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ALVAREZ_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ALVAREZ_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ALVAREZ_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.alvarez/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Alvarez install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Alvarez Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `alvarez update` path
                # (#4735).
                ...
                local stash_name
                stash_name="alvarez-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``alvarez update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Alvarez behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"alvarez-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up alvarez command..."

    if [ "$USE_VENV" = true ]; then
        ALVAREZ_BIN="$INSTALL_DIR/venv/bin/alvarez"
    else
        ALVAREZ_BIN="$(which alvarez 2>/dev/null || echo "")"
        if [ -z "$ALVAREZ_BIN" ]; then
            log_warn "alvarez not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$ALVAREZ_BIN" ]; then
        log_warn "alvarez entry point not found at $ALVAREZ_BIN"
    ...
    # Create a user-facing shim for the alvarez command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ALVAREZ_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ALVAREZ_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/alvarez"
    cat > "$command_link_dir/alvarez" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ALVAREZ_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/alvarez"
    log_success "Installed alvarez launcher → $command_link_display_dir/alvarez"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "alvarez command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v alvarez' \
                >/dev/null 2>&1; then
        ...
        log_info "alvarez not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Alvarez Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Alvarez Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Alvarez Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so alvarez works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.alvarez directory structure (config at top level, code in subdir)
    mkdir -p "$ALVAREZ_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.alvarez/.env (top level, easy to find)
    if [ ! -f "$ALVAREZ_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ALVAREZ_HOME/.env"
            log_success "Created ~/.alvarez/.env from template"
        else
            touch "$ALVAREZ_HOME/.env"
            log_success "Created ~/.alvarez/.env"
        fi
    else
        log_info "~/.alvarez/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ALVAREZ_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.alvarez/config.yaml (top level, easy to find)
    if [ ! -f "$ALVAREZ_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ALVAREZ_HOME/config.yaml"
            log_success "Created ~/.alvarez/config.yaml from template"
        fi
    else
        log_info "~/.alvarez/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in alvarez_cli/default_soul.py — the
    ...
    if [ ! -f "$ALVAREZ_HOME/SOUL.md" ]; then
        cat > "$ALVAREZ_HOME/SOUL.md" << 'SOUL_EOF'
You are Alvarez Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.alvarez/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.alvarez/"

    # Seed bundled skills into ~/.alvarez/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `alvarez update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'alvarez update'." \
            > "$ALVAREZ_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ALVAREZ_HOME/.no-bundled-skills"
        log_info "  Future 'alvarez update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.alvarez/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.alvarez/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ALVAREZ_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ALVAREZ_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.alvarez/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$ALVAREZ_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Alvarez Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Alvarez will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Alvarez Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (alvarez --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `alvarez update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'alvarez setup' after install."
        return 0
    fi
    ...
    # Run alvarez setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m alvarez_cli.main setup < /dev/tty
    else
        python -m alvarez_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ALVAREZ_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Alvarez to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ALVAREZ_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'alvarez whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ALVAREZ_CMD="$(get_alvarez_command_path)"
                $ALVAREZ_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'alvarez whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'alvarez gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ALVAREZ_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ALVAREZ_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: alvarez gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: alvarez gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ALVAREZ_CMD gateway > "$ALVAREZ_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.alvarez/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: alvarez gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: alvarez gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ALVAREZ_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ALVAREZ_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ALVAREZ_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}alvarez${NC}              Start chatting"
    echo -e "   ${GREEN}alvarez setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}alvarez config${NC}       View/edit configuration"
    echo -e "   ${GREEN}alvarez config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}alvarez gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}alvarez update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'alvarez' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'alvarez' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'alvarez' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ALVAREZ_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ALVAREZ_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ALVAREZ_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ALVAREZ_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ALVAREZ_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Alvarez for pip install"
    ...
    ALVAREZ_CMD="$(command -v alvarez 2>/dev/null || echo "")"
    if [ -n "$ALVAREZ_CMD" ]; then
        log_info "Running alvarez setup..."
        "$ALVAREZ_CMD" setup
    else
        log_warn "alvarez command not found on PATH"
        log_info "Try: python -m alvarez_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Alvarez` rename
...
# _purge_electron_build_cache() used by `alvarez desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Alvarez-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `alvarez desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Alvarez" ]; then
            app="$desktop_dir/release/linux-unpacked/Alvarez"
        elif [ -x "$desktop_dir/release/linux-unpacked/alvarez" ]; then
            app="$desktop_dir/release/linux-unpacked/alvarez"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Alvarez.app" \
            "$desktop_dir/release/mac/Alvarez.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Alvarez is damaged and can't be opened").
    ...
    # checkout stays clean for the next `alvarez update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Alvarez-managed Node
            # provisioned during prerequisites/node-deps (at $ALVAREZ_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ALVAREZ_HOME.
    # $ALVAREZ_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'alvarez update' on this host install.
