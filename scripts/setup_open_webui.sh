...
# Bootstrap Open WebUI against Alvarez Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.alvarez/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-alvarez.sh
...
#   OPEN_WEBUI_NAME='Johnny Alvarez'
...
#   ALVAREZ_API_PORT=8642
#   ALVAREZ_API_HOST=127.0.0.1
#   ALVAREZ_API_MODEL_NAME='Alvarez Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Alvarez Agent WebUI}"
...
ALVAREZ_ENV_FILE="${ALVAREZ_ENV_FILE:-$HOME/.alvarez/.env}"
ALVAREZ_API_PORT="${ALVAREZ_API_PORT:-8642}"
ALVAREZ_API_HOST="${ALVAREZ_API_HOST:-127.0.0.1}"
ALVAREZ_API_CONNECT_HOST="${ALVAREZ_API_CONNECT_HOST:-127.0.0.1}"
ALVAREZ_API_MODEL_NAME="${ALVAREZ_API_MODEL_NAME:-Alvarez Agent}"
ALVAREZ_API_BASE_URL="http://${ALVAREZ_API_CONNECT_HOST}:${ALVAREZ_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-alvarez.sh"
LOG_DIR="$HOME/.alvarez/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ALVAREZ_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ALVAREZ_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.alvarez.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.alvarez</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.alvarez"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.alvarez"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-alvarez.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Alvarez Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-alvarez.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.alvarez/logs/openwebui.log
StandardError=append:%h/.alvarez/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-alvarez.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ALVAREZ_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Alvarez API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ALVAREZ_ENV_FILE"
  upsert_env API_SERVER_HOST "$ALVAREZ_API_HOST" "$ALVAREZ_ENV_FILE"
  upsert_env API_SERVER_PORT "$ALVAREZ_API_PORT" "$ALVAREZ_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ALVAREZ_API_MODEL_NAME" "$ALVAREZ_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ALVAREZ_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Alvarez gateway so API server settings take effect...'
  alvarez gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ALVAREZ_API_CONNECT_HOST}:${ALVAREZ_API_PORT}/health" >/dev/null; then
    log 'Alvarez API server did not answer on the first check. Trying to start gateway in the background...'
    nohup alvarez gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ALVAREZ_API_CONNECT_HOST}:${ALVAREZ_API_PORT}/health" >/dev/null
  ...
  log "Alvarez API endpoint: ${ALVAREZ_API_BASE_URL}"
