#!/usr/bin/env python3
"""Live test harness for Alvarez Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any alvarez imports.
ORIGINAL_HOME = os.environ.get("ALVAREZ_HOME")
ORIGINAL_AUTH = Path.home() / ".alvarez" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.alvarez/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.alvarez/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="alvarez_ts_live_"))
    alvarez_home = home_dir / ".alvarez"
    alvarez_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, alvarez_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".alvarez" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, alvarez_home / ".env")
        ...
        load_alvarez_dotenv(alvarez_home=str(Path.home() / ".alvarez"))
    ...
    (alvarez_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return alvarez_home
...
def reset_module_state():
    """Drop cached modules so the new ALVAREZ_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "alvarez_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ALVAREZ_HOME"] = str(home)
...
def main():
    ...
    # Restore original ALVAREZ_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ALVAREZ_HOME"] = ORIGINAL_HOME
    else:
