#!/usr/bin/env python3
"""Build the Alvarez Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated lists (``OPENROUTER_MODELS``,
``OPENROUTER_MODELS``) and writes them to a JSON manifest that the
Alvarez CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Alvarez release.
...
a self-hosted ``model-catalog.json`` (set ``model_catalog.url``)
"""
...
# Ensure ALVAREZ_HOME is set for imports that touch it at module level.
os.environ.setdefault("ALVAREZ_HOME", os.path.join(os.path.expanduser("~"), ".alvarez"))

from alvarez_cli.models import OPENROUTER_MODELS  # noqa: E402
