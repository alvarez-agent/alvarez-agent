#!/usr/bin/env bash
# Canonical test runner for alvarez-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.alvarez/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.alvarez/alvarez-agent/venv"; do
...
if [ -f "$HOME/.alvarez/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.alvarez"
...
exec env -i \
  ...
  ${ALVAREZ_RUN_SLOW_PET_TESTS:+ALVAREZ_RUN_SLOW_PET_TESTS="$ALVAREZ_RUN_SLOW_PET_TESTS"} \
