...
version: 1.0.0
author: arceus77-7, enhanced by Alvarez Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Alvarez terminal calls (desktop app flow only)
...
### Service Account (recommended for Alvarez)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ALVAREZ_HOME:-~/.alvarez}/.env` (the skill will prompt for this on first load).
...
## Alvarez Execution Pattern (desktop app flow)

Alvarez terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/alvarez-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/alvarez-op.sock"
