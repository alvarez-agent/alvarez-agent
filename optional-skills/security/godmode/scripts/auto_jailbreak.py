...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ALVAREZ_HOME", os.path.expanduser("~/.alvarez")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ALVAREZ_HOME", Path.home() / ".alvarez")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Alvarez config paths
# ═══════════════════════════════════════════════════════════════════

ALVAREZ_HOME = Path(os.getenv("ALVAREZ_HOME", Path.home() / ".alvarez"))
CONFIG_PATH = ALVAREZ_HOME / "config.yaml"
PREFILL_PATH = ALVAREZ_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Alvarez models — already uncensored, just needs clean prompt
    "alvarez": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "alvarez" in model_lower or "nous" in model_lower:
        return "alvarez"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Alvarez config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.alvarez/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Alvarez for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Alvarez for changes to take effect.")
