...
- Skill helper paths:
  - `${ALVAREZ_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ALVAREZ_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ALVAREZ_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client alvarez
```
...
```bash
PUBLISH="${ALVAREZ_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client alvarez
```
...
```bash
DRIVE="${ALVAREZ_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `alvarez`)    |
