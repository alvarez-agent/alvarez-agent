#!/usr/bin/env python3
"""Canvas LMS API CLI for Alvarez Agent.
...
def _check_config():
    ...
    if missing:
        alvarez_env = os.path.join(
            os.environ.get("ALVAREZ_HOME", os.path.expanduser("~/.alvarez")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {alvarez_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Alvarez Agent"
    )
