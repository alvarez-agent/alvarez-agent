...
description: Migrate a user's OpenClaw customization footprint into Alvarez Agent. Imports Alvarez-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Alvarez Agent (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  alvarez:
    tags: [Migration, OpenClaw, Alvarez, Memory, Persona, Import]
    related_skills: [alvarez-agent]
---

# OpenClaw -> Alvarez Migration

Use this skill when a user wants to move their OpenClaw setup into Alvarez Agent with minimal manual cleanup.
...
```bash
alvarez claw migrate              # Full interactive migration
...
alvarez claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `alvarez setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_alvarez.py` to:

- import `SOUL.md` into the Alvarez home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Alvarez memory entries
- merge OpenClaw command approval patterns into Alvarez `command_allowlist`
- migrate Alvarez-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Alvarez working-directory configuration
- copy OpenClaw skills into `~/.alvarez/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Alvarez workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.alvarez/tts/`
- archive non-secret docs that do not have a direct Alvarez destination
...
- `scripts/openclaw_to_alvarez.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.alvarez/skills/migration/openclaw-migration/scripts/openclaw_to_alvarez.py`

Do not guess a shorter path like `~/.alvarez/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.alvarez/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Alvarez-compatible secrets, currently:
...
Alvarez CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Alvarez. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Alvarez workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Alvarez skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.alvarez/skills/migration/openclaw-migration/scripts/openclaw_to_alvarez.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.alvarez/skills/migration/openclaw-migration/scripts/openclaw_to_alvarez.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.alvarez/skills/migration/openclaw-migration/scripts/openclaw_to_alvarez.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.alvarez/skills/migration/openclaw-migration/scripts/openclaw_to_alvarez.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.alvarez/skills/migration/openclaw-migration/scripts/openclaw_to_alvarez.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.alvarez/skills/migration/openclaw-migration/scripts/openclaw_to_alvarez.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Alvarez unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Alvarez targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Alvarez destination. Unsupported auth blobs must still be reported as skipped.
...
- Alvarez persona state imported
- Alvarez memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.alvarez/skills/openclaw-imports/`
