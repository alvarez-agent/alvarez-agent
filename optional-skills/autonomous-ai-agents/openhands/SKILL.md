...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Alvarez Agent
license: MIT
platforms: [linux, macos]
metadata:
  alvarez:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, alvarez-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Alvarez.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Alvarez-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `alvarez-agent` (Alvarez subagents via `delegate_task`).
