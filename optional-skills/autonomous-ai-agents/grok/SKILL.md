...
version: 0.1.0
author: Matt Maximo (MattMaximo), Alvarez Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  alvarez:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, alvarez-agent]
---

# Grok Build CLI — Alvarez Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Alvarez terminal. Grok
...
1. Prepare stable input files first with Alvarez tools (`read_file`,
...
2. **Don't conflate Alvarez' xAI auth with the `grok` CLI's auth.** Alvarez'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Alvarez' xAI auth carries
   over.
