---
name: honcho
description: Configure and use Honcho memory with Alvarez -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
...
# Honcho Memory for Alvarez

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Alvarez profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/alvarez#running-honcho-locally-with-alvarez

### Verify

```bash
alvarez honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Alvarez profile's AI peer
...
Honcho models conversations as interactions between **peers**. Alvarez creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Alvarez instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Alvarez run |
| `global` | Single session across all directories |

Manual override: `alvarez honcho map my-project-name`
...
Each Alvarez profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
alvarez profile create coder --clone
# creates host block alvarez.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `alvarez.coder` host block in `honcho.json`
...
```bash
alvarez honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Alvarez when Honcho memory is active.
...
Config file: `$ALVAREZ_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `alvarez honcho setup`. Ensure `memory.provider: honcho` is in `~/.alvarez/config.yaml`.

### Memory not persisting across sessions
Check `alvarez honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `alvarez profile create <name> --clone`. For existing profiles: `alvarez honcho sync`.
...
| Command | Description |
|---------|-------------|
| `alvarez honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `alvarez honcho status` | Show resolved config, connection test, peer info for active profile |
| `alvarez honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `alvarez honcho disable` | Disable Honcho for the active profile |
| `alvarez honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `alvarez honcho peers` | Show peer identities across all profiles |
| `alvarez honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `alvarez honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `alvarez honcho sessions` | List known directory-to-session-name mappings |
| `alvarez honcho map <name>` | Map current working directory to a Honcho session name |
| `alvarez honcho identity` | Seed AI peer identity or show both peer representations |
| `alvarez honcho sync` | Create host blocks for all Alvarez profiles that don't have one yet |
| `alvarez honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Alvarez + Honcho |
| `alvarez memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `alvarez memory status` | Show active memory provider and config |
| `alvarez memory off` | Disable external memory provider |
