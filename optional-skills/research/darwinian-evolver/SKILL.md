...
version: 0.1.0
author: Bihruze (Asahi0x), Alvarez Agent
license: MIT
...
import upstream classes into Alvarez itself.
...
```bash
mkdir -p ~/.alvarez/cache/darwinian-evolver && cd ~/.alvarez/cache/darwinian-evolver
...
```bash
cd ~/.alvarez/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.alvarez/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.alvarez/skills/research/darwinian-evolver
DE_DIR=~/.alvarez/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Alvarez core.
   Custom driver scripts under `~/.alvarez/skills/...` are user-side and fine.
