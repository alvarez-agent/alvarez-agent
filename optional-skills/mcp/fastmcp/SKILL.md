...
- smoke-test a server with the FastMCP CLI before wiring it into Alvarez or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Alvarez. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.alvarez/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.alvarez/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Alvarez integration, either:

- configure the server in `~/.alvarez/config.yaml` using the `native-mcp` skill, or
...
### Alvarez cannot see the deployed server

The server-building part may be correct while the Alvarez config is not. Load the `native-mcp` skill and configure the server in `~/.alvarez/config.yaml`, then restart Alvarez.
