...
Maps each role archetype to the Alvarez skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public alvarez-agent
...
## Alvarez skills relevant to video production

### Visual / rendering skills (`alvarez-agent/skills/creative/`)
...
### Design / pre-production skills (`alvarez-agent/skills/creative/`)
...
### Audio / media skills (`alvarez-agent/skills/creative/` + `skills/media/`)
...
These are **not** Alvarez skills but external CLIs / APIs that profiles invoke.
...
## Built-in Alvarez tools for media review

These are native Alvarez tools — not invoked via terminal but through their own
...
key is present in `${ALVAREZ_HOME:-~/.alvarez}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Alvarez profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ALVAREZ_HOME:-~/.alvarez}/.env` → environment variable.
...
When a new Alvarez-public video skill ships:
