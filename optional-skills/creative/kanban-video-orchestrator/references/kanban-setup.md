...
the actual `setup.sh` that creates the project workspace, configures Alvarez
...
2. **Create profiles** — `alvarez profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.alvarez/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `alvarez kanban create` assigned to the director
...
```bash
alvarez profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.alvarez/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.alvarez/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Alvarez
...
Each profile gets a `SOUL.md` at `~/.alvarez/profiles/<name>/SOUL.md` that
...
```bash
alvarez kanban create "Direct production of <video title>" \
    --assignee director \
...
the Alvarez `.env` (`${ALVAREZ_HOME:-$HOME/.alvarez}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _alvarez_env="${ALVAREZ_HOME:-$HOME/.alvarez}/.env"
    if grep -q "^${var}=" "$_alvarez_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_alvarez_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_alvarez_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY alvarez ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY alvarez OPENROUTER_API_KEY || exit 1
# ...
```
