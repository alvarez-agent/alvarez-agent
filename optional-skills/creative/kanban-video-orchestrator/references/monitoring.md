...
alvarez kanban watch --tenant <project-slug>

# Snapshot of the board
alvarez kanban list --tenant <project-slug>
alvarez kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
alvarez kanban stats --tenant <project-slug>

# Visual dashboard (browser)
alvarez dashboard

# Inspect a specific task (includes comments + events)
alvarez kanban show <task-id>

# Follow a single task's event stream
alvarez kanban tail <task-id>
```

Verify available subcommands with `alvarez kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `alvarez kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `alvarez kanban block <id>` then `alvarez kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `alvarez kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `alvarez kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `alvarez profile list` (profile exists?), `alvarez status` (gateway/dispatcher up?), and verify tenant. |
...
alvarez kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
alvarez kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(alvarez kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
alvarez kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
alvarez kanban block <task-id>
alvarez kanban archive <task-id>

# Diagnose what happened
alvarez kanban show <task-id>      # task body, comments, recent events
alvarez kanban tail <task-id>      # follow the live event stream
alvarez kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `alvarez kanban create` for the director
...
while true; do
    clear
    alvarez kanban list --tenant <slug>
    echo "---"
    alvarez kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `alvarez kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `alvarez profile list` and start
