...
Usage (via cron with --no-agent):
    ...
    alvarez cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ALVAREZ_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Alvarez-Watcher/1.0"})
