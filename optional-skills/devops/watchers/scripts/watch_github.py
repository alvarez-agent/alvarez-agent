...
Usage (via cron with --no-agent):
    ...
    alvarez cron create alvarez-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ALVAREZ_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name my-issues --repo owner/repo --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Alvarez .env file
(``${ALVAREZ_HOME:-~/.alvarez}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Alvarez-Watcher/1.0",
    }
