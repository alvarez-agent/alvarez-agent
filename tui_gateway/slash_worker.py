"""Persistent slash-command worker — one AlvarezCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``ALVAREZ_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("ALVAREZ_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("ALVAREZ_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: AlvarezCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ALVAREZ_SESSION_KEY"] = args.session_key
    os.environ["ALVAREZ_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) AlvarezCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AlvarezCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
