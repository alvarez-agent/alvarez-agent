...
# from shadowing Alvarez's own top-level modules.  ``alvarez_bootstrap`` lives at
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ALVAREZ_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ALVAREZ_TUI_SIDECAR_URL")
...
# tested; override via ``ALVAREZ_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ALVAREZ_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ALVAREZ_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``alvarez --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``alvarez_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is None or not thread.is_alive():
        return
    try:
        from alvarez_cli.mcp_startup import _resolve_discovery_timeout
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``alvarez --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``alvarez dashboard`` spawn theirs via
    ``alvarez_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
    ...
    if thread is not None and thread.is_alive():
        return True
    try:
        from alvarez_cli.mcp_startup import (
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``alvarez_cli.mcp_startup`` thread used by the
    ...
    try:
        from alvarez_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def main():
    ...
    # server stalled the whole shell on "summoning alvarez…" for the full
