...
def _is_alvarez_provider_credential(name: str) -> bool:
    """True if ``name`` is a Alvarez-managed provider credential (API key,
    token, or similar) per ``_ALVAREZ_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Alvarez API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Alvarez credential into the execute_code child.
    """
    try:
        ...
        return True
    return name in _ALVAREZ_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Alvarez-managed provider credentials (from
    ``_ALVAREZ_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Alvarez-managed
    ...
    Non-Alvarez third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_alvarez_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Alvarez provider "
                "credential %r (blocked by _ALVAREZ_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Alvarez-managed provider credentials must not be passed
                ...
                if _is_alvarez_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Alvarez "
                        "provider credential %r from config.yaml (blocked "
                        "by _ALVAREZ_PROVIDER_ENV_BLOCKLIST). Operator "
