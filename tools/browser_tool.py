...
from alvarez_constants import agent_browser_runnable, get_alvarez_home
from utils import env_int, is_truthy_value
from alvarez_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Alvarez secret
...
def _build_browser_env() -> dict:
    ...
    Strips Alvarez-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``alvarez_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = alvarez_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    alvarez_node_bin = str(alvarez_home / "node" / "bin")
    alvarez_node_root = str(alvarez_home / "node")
    alvarez_nm_bin = str(alvarez_home / "node_modules" / ".bin")
    return [alvarez_node_bin, alvarez_node_root, alvarez_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
    ...
    if "sandbox" in combined:
        ...
        if _running_in_docker():
            hints.append(
                ...
                "missing. A local Chromium install is required — inside the "
                "container run: npx agent-browser install --with-deps "
...
# plugins drop in under ``~/.alvarez/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.alvarez/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Alvarez silently
...
def _run_chrome_fallback_command(
    ...
    if not _chromium_installed():
        if _running_in_docker():
            hint = (
                ...
                "A local Chromium install is required — inside the container "
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Alvarez CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-alvarez_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed alvarez processes — this way every clean alvarez exit sweeps
    ...
    # Sweep orphans from other crashed alvarez processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live alvarez processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current alvarez PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live alvarez process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning alvarez process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced alvarez PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent alvarez instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-alvarez_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live alvarez process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Alvarez-managed
    ...
    # node_modules binary, which disappears on the next ``alvarez update`` and
    ...
    # Build an extended search PATH including Alvarez-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_alvarez_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_alvarez_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_alvarez_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    if (
        ...
        if _running_in_docker():
            hint = (
                "Chromium browser is missing. A local Chromium install is "
                "required — inside the container run: "
                "npx agent-browser install --with-deps "
                "(or: npx playwright install --with-deps chromium)"
            )
        else:
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this alvarez PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = alvarez_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Alvarez
    ...
    from alvarez_constants import get_alvarez_dir
    screenshots_dir = get_alvarez_dir("cache/screenshots", "browser_screenshots")
    ...
    if check_browser_requirements():
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    print("     Docker: a local Chromium install is required —")
