...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ALVAREZ_YOLO_MODE", ""))
...
# os.environ["ALVAREZ_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export ALVAREZ_INTERACTIVE.
_alvarez_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "alvarez_interactive",
    default=None,
)


def set_alvarez_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["ALVAREZ_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``ALVAREZ_INTERACTIVE`` env var for legacy callers.
    """
    return _alvarez_interactive_ctx.set("1" if interactive else "")


def reset_alvarez_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_alvarez_interactive_context`."""
    _alvarez_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``ALVAREZ_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _alvarez_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("ALVAREZ_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ALVAREZ_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ALVAREZ_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ALVAREZ_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ALVAREZ_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ALVAREZ_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ALVAREZ_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ALVAREZ_CRON_SESSION"):
        return False
    if env_var_enabled("ALVAREZ_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ALVAREZ_HOME, or by the resolved absolute
# active profile home path such as /home/alvarez/.alvarez/config.yaml. The
# resolved-absolute form is folded into the ~/.alvarez/ patterns at detection
...
# go stale when ALVAREZ_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ALVAREZ_ENV_PATH = (
    r'(?:~\/\.alvarez/|'
    r'(?:\$home|\$\{home\})/\.alvarez/|'
    r'(?:\$alvarez_home|\$\{alvarez_home\})/)'
    r'\.env\b'
)
# ~/.alvarez/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ALVAREZ_ENV_PATH; matches the ALVAREZ_HOME override form as
# well as ~/.alvarez/.
_ALVAREZ_CONFIG_PATH = (
    r'(?:~\/\.alvarez/|'
    r'(?:\$home|\$\{home\})/\.alvarez/|'
    r'(?:\$alvarez_home|\$\{alvarez_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_ALVAREZ_ENV_PATH}|'
    rf'{_ALVAREZ_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    # `alvarez` and `gateway` (e.g. `alvarez -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\balvarez\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart alvarez gateway (kills running agents)"),
    (r'\balvarez\s+update\b', "alvarez update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `alvarez gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart alvarez-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart alvarez-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(alvarez|gateway|cli\.py)\b', "kill alvarez/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill alvarez` but not
    # `kill -9 $(pgrep -f alvarez)` because the substitution is opaque
    ...
    # the `alvarez gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.alvarez.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(alvarez|ai\.alvarez)\b', "stop/restart alvarez launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Alvarez file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.alvarez/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Alvarez-managed security file (~/.alvarez/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ALVAREZ_CONFIG_PATH}|{_ALVAREZ_ENV_PATH})', "in-place edit of Alvarez config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ALVAREZ_CONFIG_PATH}|{_ALVAREZ_ENV_PATH})', "in-place edit of Alvarez config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ALVAREZ_CONFIG_PATH}|{_ALVAREZ_ENV_PATH})', "in-place edit of Alvarez config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.alvarez/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ALVAREZ_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Alvarez home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\alvarez), so folding the user home
    # first would eat the prefix the Alvarez-home fold needs.
    command = _rewrite_resolved_alvarez_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ALVAREZ_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.alvarez``); the matched
...
def _rewrite_resolved_alvarez_home(command: str) -> str:
    """Rewrite the resolved absolute Alvarez home prefix to ``~/.alvarez/``.

    Resolves the active ``ALVAREZ_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.alvarez/`` so the static ``_ALVAREZ_CONFIG_PATH`` / ``_ALVAREZ_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/alvarez/.alvarez/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ALVAREZ_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from alvarez_constants import get_alvarez_home
        home = get_alvarez_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.alvarez")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["ALVAREZ_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ALVAREZ_SPINNER_PAUSE" in os.environ:
            del os.environ["ALVAREZ_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ALVAREZ_INTERACTIVE or ALVAREZ_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ALVAREZ_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ALVAREZ_EXEC_ASK")
    ...
    try:
        ...
        try:
            from alvarez_cli.config import load_config as _load_cfg
