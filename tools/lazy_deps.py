"""
Lazy dependency installer for opt-in Alvarez Agent backends.

Many Alvarez features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``alvarez-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``alvarez tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``ALVAREZ_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/alvarez`` read-only), setting
  ``ALVAREZ_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Alvarez, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (ALVAREZ_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
...
_LAZY_TARGET_ENV = "ALVAREZ_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ALVAREZ_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("ALVAREZ_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``alvarez update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="alvarez-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``alvarez_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = alvarez_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``alvarez update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``alvarez update``. Never raises; lazy-install failures
