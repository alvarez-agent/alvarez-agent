#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Alvarez Skills Hub.
...
Used by alvarez_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _alvarez_home() -> Path:
    return get_alvarez_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _alvarez_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            alvarez_meta = metadata.get("alvarez", {})
            if isinstance(alvarez_meta, dict):
                tags = alvarez_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            alvarez_meta = metadata.get("alvarez", {})
            if isinstance(alvarez_meta, dict):
                raw_tags = alvarez_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (shipped with the repo) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.alvarez/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                alvarez_meta = meta_block.get("alvarez", {})
                if isinstance(alvarez_meta, dict):
                    tags = alvarez_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Alvarez centralized index source
# ---------------------------------------------------------------------------

# No hosted skills index is configured by default — an operator can point
# this at a self-hosted index built with the same schema.
ALVAREZ_INDEX_URL = os.environ.get("ALVAREZ_SKILLS_INDEX_URL", "")
ALVAREZ_INDEX_TTL = 6 * 3600  # 6 hours


def _alvarez_index_cache_file() -> Path:
    return _index_cache_dir() / "alvarez-index.json"


def _load_alvarez_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ALVAREZ_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    if not ALVAREZ_INDEX_URL:
        logger.debug("no skills hub index configured")
        return None

    # Check local cache
    alvarez_index_cache_file = _alvarez_index_cache_file()
    if alvarez_index_cache_file.exists():
        try:
            age = time.time() - alvarez_index_cache_file.stat().st_mtime
            if age < ALVAREZ_INDEX_TTL:
                return json.loads(alvarez_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ALVAREZ_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Alvarez index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Alvarez index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        alvarez_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        alvarez_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class AlvarezIndexSource(SkillSource):
    """Skill source backed by the centralized Alvarez Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_alvarez_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "alvarez-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "alvarez-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        AlvarezIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "alvarez-index"
