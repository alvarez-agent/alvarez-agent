...
etc.) surface as specific blocked checks via `alvarez computer-use doctor`
...
# fetches the latest release, so a `ALVAREZ_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `ALVAREZ_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("ALVAREZ_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when Alvarez should disable cua-driver telemetry for this user.
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or ALVAREZ_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `alvarez computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  alvarez computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `alvarez tools` and enable the Computer Use toolset to install it automatically."
    )
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Alvarez-managed secrets out of the child env.
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        #   - Gets a distinct agent-cursor color per Alvarez run, with
        ...
        #     concurrent Alvarez runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Alvarez run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"alvarez-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Alvarez' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps alvarez `capture(mode, app)` → cua-driver `list_windows` +
