...
This module provides the glue:
    - ``AlvarezTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Alvarez Agent"
"""
...
from alvarez_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__alvarez_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ALVAREZ_HOME so each profile gets its own OAuth tokens.
    Layout: ``ALVAREZ_HOME/mcp-tokens/``
    """
    try:
        from alvarez_constants import get_alvarez_home
        base = Path(get_alvarez_home())
    except ImportError:
        base = Path(os.environ.get("ALVAREZ_HOME", str(Path.home() / ".alvarez")))
...
# ---------------------------------------------------------------------------
# AlvarezTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AlvarezTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ALVAREZ_HOME/mcp-tokens/<server_name>.json         -- tokens
        ALVAREZ_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ALVAREZ_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Alvarez records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Alvarez.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `alvarez mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Alvarez Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`alvarez mcp login {server_name}` interactively first to complete "
