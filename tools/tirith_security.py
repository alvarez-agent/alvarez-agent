...
it is automatically downloaded from GitHub releases to $ALVAREZ_HOME/bin/tirith.
...
def _get_alvarez_home() -> str:
    """Return the Alvarez home directory, respecting ALVAREZ_HOME env var."""
    return str(get_alvarez_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_alvarez_home(), ".tirith-install-failed")
...
def _alvarez_bin_dir() -> str:
    """Return $ALVAREZ_HOME/bin, creating it if needed."""
    d = os.path.join(_get_alvarez_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ALVAREZ_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_alvarez_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ALVAREZ_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ALVAREZ_HOME/bin/tirith
    ...
    alvarez_bin = os.path.join(_alvarez_bin_dir(), "tirith")
    if os.path.isfile(alvarez_bin) and os.access(alvarez_bin, os.X_OK):
        _resolved_path = alvarez_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return alvarez_bin
