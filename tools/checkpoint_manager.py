...
-----------------------------------------------------------------------------
    ...
    ~/.alvarez/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/alvarez/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_alvarez_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/alvarez"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Alvarez convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("ALVAREZ_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Alvarez infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``alvarez checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `alvarez checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "alvarez@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Alvarez Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``ALVAREZ_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ALVAREZ_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ALVAREZ_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def ensure_checkpoint(self, working_dir: str, reason: str = "auto") -> bool:
        ...
        # Skip root, home, and other overly broad directories. abs_dir is
        # symlink-resolved, so home must be compared resolved too (macOS
        # HOME under /var/folders resolves to /private/var/...).
        if abs_dir in {"/", str(Path.home()), str(Path.home().resolve())}:
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "ALVAREZ_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `alvarez checkpoints` CLI
# ---------------------------------------------------------------------------
