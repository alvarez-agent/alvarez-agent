...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``alvarez_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from alvarez_constants import get_subprocess_home
...
_alvarez_config_resolved_loaded = False


def _get_alvarez_config_resolved() -> str | None:
    """Return the resolved absolute path of the Alvarez config file (cached)."""
    global _alvarez_config_resolved, _alvarez_config_resolved_loaded
    if _alvarez_config_resolved_loaded:
        return _alvarez_config_resolved
    _alvarez_config_resolved_loaded = True
    try:
        from alvarez_cli.config import get_config_path
        _alvarez_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _alvarez_config_resolved = str(Path(_expand_tilde("~/.alvarez/config.yaml")).resolve())
        except Exception:
            _alvarez_config_resolved = None
    return _alvarez_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file.
    alvarez_config = _get_alvarez_config_resolved()
    if alvarez_config and (resolved == alvarez_config or normalized == alvarez_config):
        return (
            f"Refusing to write to Alvarez config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.alvarez/config.yaml directly or use 'alvarez config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Alvarez mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Alvarez
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Alvarez state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.alvarez/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Alvarez process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.alvarez/…`` path.

    Returns ``None`` when the write is in-scope or outside Alvarez scope.
    ...
    # in a session that cd'd into ``~/.alvarez/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Alvarez internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under ALVAREZ_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ALVAREZ_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Alvarez-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Alvarez-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Alvarez profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Alvarez profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with alvarez_tools.write_file() for very "
            "large files."
        )
