...
def _handle_send(args):
    ...
    if not pconfig or not pconfig.enabled:
        return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.alvarez/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: alvarez config set {home_env} <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("ALVAREZ_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("ALVAREZ_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("ALVAREZ_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("ALVAREZ_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    # built-in platforms; from PlatformEntry.max_message_length for plugin
    # platforms, resolved via the registry fallback below).
    _MAX_LENGTHS = {
    ...
    # --- Non-telegram platforms (plugin platforms via registry/adapter) ---
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``ALVAREZ_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``ALVAREZ_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
#   - the ``alvarez send`` CLI command (alvarez_cli/send_cmd.py)
