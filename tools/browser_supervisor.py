...
One ``CDPSupervisor`` runs per Alvarez ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "alvarez-dialog-bridge.invalid"
...
(() => {
  if (window.__alvarezDialogBridgeInstalled) return;
  window.__alvarezDialogBridgeInstalled = true;
  const ENDPOINT = "http://alvarez-dialog-bridge.invalid/";
