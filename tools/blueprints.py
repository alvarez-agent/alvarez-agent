...
`alvarez skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.alvarez.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.alvarez.blueprint`` is a mapping containing
    ...
    alvarez = meta.get("alvarez") if isinstance(meta, dict) else None
    blueprint = alvarez.get("blueprint") if isinstance(alvarez, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.alvarez.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.alvarez.blueprint`` block) they can hand
    to ``alvarez skills publish`` to share. ``body`` is the plain-language
