...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``alvarez tools`` repaint, tool-registration scans,
    ...
    2. ``~/.alvarez/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_alvarez_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.alvarez/.env`` first, then ``os.environ``.

    Wraps :func:`alvarez_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from alvarez_cli.config import get_env_value as _alvarez_get_env_value

        value = _alvarez_get_env_value(name)
...
def alvarez_xai_user_agent() -> str:
    """Return a stable Alvarez-specific User-Agent for xAI HTTP calls."""
    try:
        from alvarez_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Alvarez-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Alvarez config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Alvarez home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_alvarez_home() / "state"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Alvarez-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``alvarez_cli.config.get_env_value`` so keys
    stored in ``~/.alvarez/.env`` (the standard Alvarez location) are honored —
