...
MoA-enabled; the normal Alvarez agent loop still owns tool calling and turn
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Alvarez boilerplate, not advisory signal).
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Alvarez agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Alvarez agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
