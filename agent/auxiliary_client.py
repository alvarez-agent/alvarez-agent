...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Alvarez owns auxiliary retry + provider/model fallback policy (the
    ...
    # Alvarez sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Alvarez control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Alvarez triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ALVAREZ_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ALVAREZ_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ALVAREZ_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ALVAREZ_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "AlvarezAgent",
}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Alvarez Agent)",
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``alvarez_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``alvarez auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``alvarez_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from alvarez_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Alvarez auth store.
    ...
    try:
        from alvarez_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from alvarez_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from alvarez_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from alvarez_cli.models import copilot_default_headers
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `alvarez setup` or `alvarez model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`alvarez_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from alvarez_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second alvarez
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``alvarez config aux reset``)."""
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    — e.g. a long-lived process pinned a model that has since been dropped
    from the provider's catalog. Distinct from :func:`_is_payment_error` (which also matches some 404s for
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Alvarez' built-in discovery
...
def _resolve_auto(
    ...
    Priority:
      ...
      2. OpenRouter → custom → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `alvarez model` but the
    #    old OPENAI_BASE_URL lingers in ~/.alvarez/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: alvarez model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.alvarez/.env",
    ...
    # on aggregators (OpenRouter) who previously got routed to a
    ...
    if main_provider == "moa":
        try:
            from alvarez_cli.config import load_config
            from alvarez_cli.moa_config import resolve_moa_preset
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from alvarez_cli.copilot_auth import copilot_request_headers
    ...
    # See _create_openai_client: disable SDK-internal retries so Alvarez owns
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from alvarez_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: alvarez model)")
                return None, None
    ...
    # OpenRouter bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: alvarez model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from alvarez_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider name
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: alvarez doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("alvarez_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from alvarez_cli.models import _should_use_copilot_responses_api
...
def get_auxiliary_extra_body() -> dict:
    ...
    Kept for callers (goals, kanban, profile describer) that merge it into
    their requests; no provider currently needs extra attribution.
    """
    return {}
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from alvarez_cli.providers import get_provider
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`alvarez_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def call_llm(
    ...
    *,
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `alvarez model`."
                    )
    ...
    try:
        ...
        # When the resolved provider returns 401 and explicit provider
        # credential refresh does not apply,
