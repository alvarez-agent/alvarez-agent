...
as ``kind: backend``) or ``~/.alvarez/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``alvarez tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ALVAREZ_HOME/cache/videos/``, creating parents as needed."""
    from alvarez_constants import get_alvarez_home

    path = get_alvarez_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ALVAREZ_HOME/cache/videos/``.
