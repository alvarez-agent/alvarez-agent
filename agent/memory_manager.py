...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ALVAREZ_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Alvarez expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``alvarez_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_alvarez_home()`` themselves.
        """
        if "alvarez_home" not in kwargs:
            from alvarez_constants import get_alvarez_home
            kwargs["alvarez_home"] = str(get_alvarez_home())
        for provider in self._providers:
            try:
