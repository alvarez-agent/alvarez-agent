...
upstream prefix cache warm.  See ``alvarez-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Alvarez never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ALVAREZ_HOME persona while keeping
    ...
    # Pointer to the alvarez-agent skill + docs for user questions about Alvarez itself.
    stable_parts.append(ALVAREZ_AGENT_HELP_GUIDANCE)
    ...
    # ALVAREZ_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Alvarez, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Alvarez profile the agent is running
    # under so it doesn't conflate ~/.alvarez/skills/ (default profile) with
    # ~/.alvarez/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Alvarez profile: default. Other profiles (if any) live "
            "under ~/.alvarez/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Alvarez profile: {active_profile}. This session reads "
            f"and writes ~/.alvarez/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.alvarez/skills/, ~/.alvarez/plugins/, "
            f"~/.alvarez/cron/, ~/.alvarez/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Alvarez never rebuilds or reinjects parts of it
