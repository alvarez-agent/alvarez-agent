...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Alvarez with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.alvarez/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Alvarez
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``alvarez tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``alvarez setup`` for
