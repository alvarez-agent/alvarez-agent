...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Alvarez-Session-Id header), and an
        ...
        if env_var_enabled("ALVAREZ_DUMP_REQUEST_STDOUT"):
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to alvarez's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from alvarez_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: alvarez' own dispatch loop intentionally keeps an
