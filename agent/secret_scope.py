...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ALVAREZ_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Alvarez runtime / deployment
    "ALVAREZ_HOME", "ALVAREZ_PROFILE", "ALVAREZ_GATEWAY_LOCK_DIR",
    "ALVAREZ_MAX_ITERATIONS", "ALVAREZ_MAX_TOKENS", "ALVAREZ_API_TIMEOUT",
    "ALVAREZ_REDACT_SECRETS", "ALVAREZ_NOUS_TIMEOUT_SECONDS",
    "_ALVAREZ_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ALVAREZ_KANBAN_DB", "ALVAREZ_KANBAN_WORKSPACES_ROOT", "ALVAREZ_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "ALVAREZ_KANBAN_",
    "ALVAREZ_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(alvarez_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(alvarez_home) / ".env")
