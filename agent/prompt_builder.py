...
from alvarez_constants import get_alvarez_home, get_skills_dir, is_wsl
...
_ALVAREZ_MD_NAMES = (".alvarez.md", "ALVAREZ.md")


def _find_alvarez_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.alvarez.md`` or ``ALVAREZ.md``.
    ...
    # could pick up a .alvarez.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ALVAREZ_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Alvarez Agent, an intelligent AI assistant. "
...
ALVAREZ_AGENT_HELP_GUIDANCE = (
    "You run on Alvarez Agent. When the user needs help with Alvarez itself — "
    ...
    "`alvarez-agent` skill with skill_view(name='alvarez-agent') for guidance "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.alvarez/kanban.db`. Your task id is in "
    "`$ALVAREZ_KANBAN_TASK`; your workspace is `$ALVAREZ_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ALVAREZ_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ALVAREZ_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ALVAREZ_KANBAN_BRANCH:-wt/$ALVAREZ_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ALVAREZ_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`alvarez profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `alvarez kanban <verb>` for board operations. Use "
...
#
# The alvarez-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to alvarez-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `alvarez computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Alvarez "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Alvarez terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Alvarez WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Alvarez itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Alvarez runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Alvarez itself is running. The host OS, home, and cwd "
                f"of the Alvarez process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Alvarez "
    ...
    # Alvarez desktop GUI — any agent running under the desktop app should know
    # it. ALVAREZ_DESKTOP marks the backend powering the chat; ALVAREZ_DESKTOP_TERMINAL
    # marks a alvarez launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("ALVAREZ_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("ALVAREZ_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Alvarez desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Alvarez
    ...
    extra = (os.getenv("ALVAREZ_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_alvarez_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.alvarez/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ALVAREZ_PLATFORM")
        or get_session_env("ALVAREZ_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Alvarez Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `alvarez-agent` skill "
            "first. It has the actual commands (e.g. `alvarez config set …`, `alvarez tools`, "
            "`alvarez setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ALVAREZ_HOME and return its content, or None.
    ...
    """
    try:
        from alvarez_cli.config import ensure_alvarez_home
        ensure_alvarez_home()
    except Exception as e:
        logger.debug("Could not ensure ALVAREZ_HOME before loading SOUL.md: %s", e)

    soul_path = get_alvarez_home() / "SOUL.md"
...
def _load_alvarez_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.alvarez.md / ALVAREZ.md — walk to git root."""
    alvarez_md_path = _find_alvarez_md(cwd_path)
    if not alvarez_md_path:
        return ""
    try:
        content = alvarez_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = alvarez_md_path.name
        try:
            rel = str(alvarez_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".alvarez.md", context_length=context_length,
            read_path=str(alvarez_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", alvarez_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .alvarez.md / ALVAREZ.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ALVAREZ_HOME is independent and always included when present.
    ...
    project_context = (
        _load_alvarez_md(cwd_path, context_length)
    ...
    # SOUL.md from ALVAREZ_HOME only — skip when already loaded as identity
    if not skip_soul:
