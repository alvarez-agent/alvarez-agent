...
Storage
-------
Bundles live in ``~/.alvarez/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ALVAREZ_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``alvarez bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ALVAREZ_HOME.

    Honors ``ALVAREZ_BUNDLES_DIR`` for tests; falls back to
    ``<ALVAREZ_HOME>/skill-bundles``.
    """
    override = os.environ.get("ALVAREZ_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_alvarez_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `alvarez bundles` CLI subcommand.
# ---------------------------------------------------------------------------
