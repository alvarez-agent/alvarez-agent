...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `alvarez backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - alvarez_home (str): The active ALVAREZ_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.alvarez``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "alvarez").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'alvarez memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], alvarez_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'alvarez memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``alvarez_home`` is the active ALVAREZ_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ALVAREZ_HOME.

        ``alvarez backup`` only walks ALVAREZ_HOME, so any provider state kept
        ...
        of the archive, and ``alvarez import`` restores them to their original
