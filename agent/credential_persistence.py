...
``auth.json``.  They intentionally have no dependency on ``alvarez_cli.auth`` so
...
# Sources Alvarez owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Alvarez-owned OAuth/device-code state)
