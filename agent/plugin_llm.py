...
Plugins built on Alvarez Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Alvarez
...
:class:`~alvarez_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Alvarez' main config
...
handles every provider, fallback chain, and per-task override Alvarez
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`alvarez_cli.plugins.PluginContext`
