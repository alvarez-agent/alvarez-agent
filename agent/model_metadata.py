...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Alvarez also honours ALVAREZ_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ALVAREZ_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_alvarez_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Alvarez Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # (native MiniMax-M3, OpenRouter minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Alvarez from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Alvarez' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from alvarez_constants import get_alvarez_home
    return get_alvarez_home() / "context_length_cache.yaml"
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter
            and "maximum context length" in error_lower)
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter minimax/minimax-m3).
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Alvarez grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from alvarez_cli.models import get_copilot_model_context
    ...
    # the dedicated Copilot/GMI branches above.
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Alvarez sends to providers:
