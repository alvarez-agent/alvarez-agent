...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Alvarez starts and finds ``~/.openclaw/``.

    Points users at ``alvarez claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``alvarez claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Alvarez, run "
        "`alvarez claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `alvarez claw cleanup` (renames it to ~/.openclaw.pre-migration — "
