...
Owns one Codex thread per Alvarez session. Drives `turn/start`, consumes
...
# Alvarez' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ALVAREZ_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Alvarez/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Alvarez session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ALVAREZ_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ALVAREZ_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="alvarez",
            client_title="Alvarez Agent",
            client_version=_get_alvarez_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Alvarez' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Alvarez' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own alvarez-tools callback we
            # auto-accept — the user already approved Alvarez' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Alvarez approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Alvarez returns 'once', 'session', 'always', or 'deny'.
...
def _get_alvarez_version() -> str:
    """Best-effort Alvarez version string for codex's userAgent line."""
    try:
