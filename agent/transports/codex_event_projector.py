"""Projects codex app-server events into Alvarez' messages list.

The translator that lets Alvarez' memory/skill review keep working under the
...
Alvarez' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Alvarez
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Alvarez' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Alvarez stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Alvarez already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Alvarez' messages store text only.
        text_parts: list[str] = []
