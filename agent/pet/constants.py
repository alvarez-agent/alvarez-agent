...
row taxonomy is inferred from the atlas shape so Alvarez can render both legacy
...
# rows (1536x1872), while older Alvarez/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Alvarez' activity state names. They are not always identical to the
...
# Legacy Alvarez/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Alvarez activity names -> accepted row-name aliases in descending
