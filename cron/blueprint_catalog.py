...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``alvarez://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``alvarez://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"alvarez://blueprint/{quote(blueprint.key)}{qs}"
