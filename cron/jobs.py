...
Jobs are stored in ~/.alvarez/cron/jobs.json
Output is saved to ~/.alvarez/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own ALVAREZ_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same ALVAREZ_HOME — so a job authored in profile
# `coder` lives in `~/.alvarez/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_alvarez_home()` (the active profile home), NOT `get_default_alvarez_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever ALVAREZ_HOME the
...
# `_get_alvarez_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
ALVAREZ_DIR = get_alvarez_home().resolve()
CRON_DIR = ALVAREZ_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``alvarez cron status`` process share it
...
# threshold in `alvarez cron status` (alvarez_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``alvarez`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Alvarez timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `alvarez_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                alvarez_tz = _alvarez_now().tzinfo
                dt = dt.replace(tzinfo=alvarez_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _alvarez_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Alvarez configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Alvarez timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _alvarez_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Alvarez/system local time
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall alvarez-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `alvarez cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``alvarez cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `alvarez cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _alvarez_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_alvarez_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.alvarez/scripts/; ``.sh`` / ``.bash`` files run via bash,
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _alvarez_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _alvarez_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``ALVAREZ_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("ALVAREZ_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
# execution. Unlike the quick-snapshot store (`alvarez_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``alvarez_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _alvarez_now().strftime("%Y-%m-%d_%H-%M-%S")
