...
#
# Wired into the image as /etc/cont-init.d/01-alvarez-setup by the
...
ALVAREZ_HOME="${ALVAREZ_HOME:-/opt/data}"
INSTALL_DIR="/opt/alvarez"

# Drop to alvarez via s6-setuidgid, but skip it when already non-root.
as_alvarez() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid alvarez "$@"; }
...
# starts non-root. The baked install tree under /opt/alvarez is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ALVAREZ_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ALVAREZ_UID/ALVAREZ_GID — or the PUID/PGID aliases — which the
...
# `id -u alvarez` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# alvarez UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u alvarez)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-alvarez UID).
...
and the baked /opt/alvarez install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ALVAREZ_UID=\$(id -u) -e ALVAREZ_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the alvarez user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ALVAREZ_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid alvarez mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ALVAREZ_HOME paths whose parents only
# root can create (e.g. `ALVAREZ_HOME=/home/alvarez/.alvarez` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ALVAREZ_UID/ALVAREZ_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ALVAREZ_UID/
# ALVAREZ_GID still win when both are set.  See #15290, salvages #25872.
ALVAREZ_UID="${ALVAREZ_UID:-${PUID:-}}"
ALVAREZ_GID="${ALVAREZ_GID:-${PGID:-}}"

if [ -n "${ALVAREZ_UID:-}" ] && validate_uid_gid "$ALVAREZ_UID" && [ "$ALVAREZ_UID" != "$(id -u alvarez)" ]; then
    echo "[stage2] Changing alvarez UID to $ALVAREZ_UID"
    usermod -u "$ALVAREZ_UID" alvarez
fi
if [ -n "${ALVAREZ_GID:-}" ] && validate_uid_gid "$ALVAREZ_GID" && [ "$ALVAREZ_GID" != "$(id -g alvarez)" ]; then
    echo "[stage2] Changing alvarez GID to $ALVAREZ_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ALVAREZ_GID" alvarez 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised alvarez user (UID 10000)
...
# alvarez process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes alvarez. Idempotent across container
...
#   - alvarez is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G alvarez 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] alvarez already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" alvarez 2>/dev/null; then
        echo "[stage2] Added alvarez to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group alvarez failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ALVAREZ_UID is remapped or the top-level $ALVAREZ_HOME isn't owned by
# the runtime alvarez UID, restore ownership to alvarez — but ONLY for the
# directories alvarez actually writes to. The full $ALVAREZ_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of alvarez-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_alvarez_uid=$(id -u alvarez)
...
chown_alvarez_tree() {
    ...
    chown -R alvarez:alvarez "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$ALVAREZ_HOME" 2>/dev/null)" != "$actual_alvarez_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ALVAREZ_HOME (targeted) to alvarez ($actual_alvarez_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ALVAREZ_HOME: chown the directory itself (not its contents)
    # so alvarez can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ALVAREZ_HOME"; then
        :
    else
        chown alvarez:alvarez "$ALVAREZ_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ALVAREZ_HOME failed (rootless container?) — continuing"
    fi
    # Alvarez-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by alvarez (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ALVAREZ_HOME/$sub" ]; then
            chown_alvarez_tree "$ALVAREZ_HOME/$sub"
        fi
    done
fi
...
# alvarez user. Hosted/container instances keep mutable state under
# $ALVAREZ_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ALVAREZ_DISABLE_LAZY_INSTALLS=1. Keeping /opt/alvarez root-owned and
...
# $ALVAREZ_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ALVAREZ_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to alvarez
...
# Always reset ownership of $ALVAREZ_HOME/profiles to alvarez on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> alvarez …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as alvarez and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ALVAREZ_HOME/profiles" ]; then
    chown_alvarez_tree "$ALVAREZ_HOME/profiles"
fi

# Always reset ownership of $ALVAREZ_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged alvarez runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$ALVAREZ_HOME/cron" ]; then
    chown_alvarez_tree "$ALVAREZ_HOME/cron"
fi

# Reset ownership of alvarez-owned top-level state files on every boot.
# The targeted data-volume chown above only covers alvarez-owned
# *subdirectories*; loose state files living directly under $ALVAREZ_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> alvarez …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged alvarez runtime then hits
...
# sweep so host-owned files in a bind-mounted $ALVAREZ_HOME are never
...
# entries of alvarez_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ALVAREZ_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ALVAREZ_HOME/$f"; then
            :
        else
            chown alvarez:alvarez "$ALVAREZ_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the alvarez runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ALVAREZ_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ALVAREZ_HOME/config.yaml"; then
        :
    else
        chown alvarez:alvarez "$ALVAREZ_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ALVAREZ_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as alvarez user ---
# Run as alvarez via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ALVAREZ_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_alvarez mkdir -p \
    "$ALVAREZ_HOME/backups" \
    "$ALVAREZ_HOME/cron" \
    "$ALVAREZ_HOME/sessions" \
    "$ALVAREZ_HOME/logs" \
    "$ALVAREZ_HOME/logs/gateways" \
    "$ALVAREZ_HOME/hooks" \
    "$ALVAREZ_HOME/memories" \
    "$ALVAREZ_HOME/skills" \
    "$ALVAREZ_HOME/skins" \
    "$ALVAREZ_HOME/plans" \
    "$ALVAREZ_HOME/workspace" \
    "$ALVAREZ_HOME/home" \
    "$ALVAREZ_HOME/pairing" \
    "$ALVAREZ_HOME/platforms/pairing" \
    "$ALVAREZ_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/alvarez/.install_method (see Dockerfile), NOT written here into
# $ALVAREZ_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ALVAREZ_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.alvarez:/opt/data) and sometimes shared with a
...
# 'alvarez update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ALVAREZ_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/alvarez stamp, so deleting the data-dir copy is safe).
if [ -f "$ALVAREZ_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ALVAREZ_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ALVAREZ_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ALVAREZ_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ALVAREZ_HOME/$dest"; then
            :
        else
            as_alvarez cp "$INSTALL_DIR/$src" "$ALVAREZ_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ALVAREZ_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ALVAREZ_HOME/.env"; then
        :
    else
        chown alvarez:alvarez "$ALVAREZ_HOME/.env" 2>/dev/null || true
        chmod 600 "$ALVAREZ_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ALVAREZ_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `alvarez update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ALVAREZ_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ALVAREZ_HOME/config.yaml" ]; then
    s6-setuidgid alvarez "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ALVAREZ_HOME/auth.json" ] && [ -n "${ALVAREZ_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ALVAREZ_HOME/auth.json"; then
        :
    else
        printf '%s' "$ALVAREZ_AUTH_JSON_BOOTSTRAP" > "$ALVAREZ_HOME/auth.json"
        chown alvarez:alvarez "$ALVAREZ_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ALVAREZ_HOME/auth.json"
    fi
fi
...
# can set ALVAREZ_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ALVAREZ_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ALVAREZ_HOME/gateway_state.json" ] && \
        [ "${ALVAREZ_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ALVAREZ_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ALVAREZ_HOME/gateway_state.json"
        chown alvarez:alvarez "$ALVAREZ_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ALVAREZ_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_alvarez "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/alvarez/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Alvarez spawns for the
...
# main-wrapper.sh propagates it into the supervised ``alvarez`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-alvarez, dashboard, etc.).
