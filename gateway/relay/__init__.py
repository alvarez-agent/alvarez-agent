"""Relay/connector support package for the Alvarez gateway.
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.alvarez/.env``.
    ...
    sets neither ``ALVAREZ_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``alvarez gateway enroll``: has a PINNED
    ...
    try:
        from alvarez_cli.auth import resolve_nous_access_token
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'alvarez'}"
