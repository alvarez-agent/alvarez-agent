...
There are two ways to add a platform to the Alvarez gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.alvarez/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Alvarez code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `alvarez gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `alvarez_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Alvarez core.
...
And add it to the `alvarez-gateway` composite:

```python
"alvarez-gateway": {
    "includes": [..., "alvarez-your-platform"]
}
```
...
## 12. Status Display (`alvarez_cli/status.py`)
...
## 13. Gateway Setup Wizard (`alvarez_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ alvarez_cli/ toolsets.py \
  --include="*.py" -l | sort -u
