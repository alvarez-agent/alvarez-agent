...
# Audio file extensions Alvarez recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Alvarez' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Alvarez-created Telegram private-chat
...
from alvarez_constants import get_default_alvarez_root, get_alvarez_dir, get_alvarez_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.alvarez/.env manually."
)
...
IMAGE_CACHE_DIR = get_alvarez_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_alvarez_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_alvarez_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; AlvarezAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_alvarez_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_alvarez_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_alvarez_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_alvarez_dir("cache/screenshots", "browser_screenshots")
...
_ALVAREZ_ROOT = get_default_alvarez_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ALVAREZ_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ALVAREZ_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ALVAREZ_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ALVAREZ_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ALVAREZ_HOME / "image_cache",
    _ALVAREZ_HOME / "audio_cache",
    _ALVAREZ_HOME / "video_cache",
    _ALVAREZ_HOME / "document_cache",
    _ALVAREZ_HOME / "browser_screenshots",
    ...
    _ALVAREZ_HOME / "cache" / "images",
    _ALVAREZ_HOME / "cache" / "audio",
    _ALVAREZ_HOME / "cache" / "videos",
    _ALVAREZ_HOME / "cache" / "documents",
    _ALVAREZ_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Alvarez root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* ALVAREZ_HOME's cache, so a gateway running at
    the root (e.g. ``ALVAREZ_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when ALVAREZ_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _ALVAREZ_ROOT / "profiles"
    try:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Alvarez profile and shared Alvarez root both contain control
    ...
    # ALVAREZ_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.alvarez
    ...
    for alvarez_root in (_ALVAREZ_HOME, _ALVAREZ_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(alvarez_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Alvarez
    secrets (``~/.alvarez/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ALVAREZ_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Alvarez-managed cache, under an operator-allowlisted root
    (``ALVAREZ_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Alvarez root (~/.alvarez/.env,
    ...
    # ``MEDIA:~/.alvarez/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?alvarez\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+alvarez[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``ALVAREZ_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Alvarez commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("ALVAREZ_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ALVAREZ_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ALVAREZ_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Alvarez sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Alvarez home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Alvarez home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.alvarez/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from alvarez_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ALVAREZ_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ALVAREZ_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ALVAREZ_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ALVAREZ_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ALVAREZ_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ALVAREZ_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _alvarez_run_generation is set on
