...
The PID file lives at ``{ALVAREZ_HOME}/gateway.pid``.  ALVAREZ_HOME defaults to
``~/.alvarez`` but can be overridden via the environment variable.  This means
separate ALVAREZ_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ALVAREZ_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ALVAREZ_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "alvarez" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Alvarez gateway lifecycle subcommand from a command line.
    ...
    test also matched ``alvarez_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\alvarez-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Alvarez's
    ...
    for token in tokens:
        ...
        if basename in ("alvarez-gateway", "alvarez-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "alvarez_cli.main" in joined
        or "alvarez_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("alvarez", "alvarez.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `alvarez gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Alvarez-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Alvarez gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a ALVAREZ_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.alvarez`` or ``$ALVAREZ_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``alvarez_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``ALVAREZ_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"alvarez_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit ALVAREZ_HOME= on the argv. ALVAREZ_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "alvarez_home=" in command_lc and f"alvarez_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `alvarez status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``ALVAREZ_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    ALVAREZ_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ALVAREZ_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``alvarez.service`` + ``alvarez-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different ALVAREZ_HOME. When two profile gateway
    # services share the same default ~/.alvarez (ALVAREZ_HOME not set
    ...
    # field is absent in markers written by older Alvarez versions; treat
    ...
    replacer_home = record.get("replacer_alvarez_home")
    if replacer_home is not None and replacer_home != str(get_alvarez_home()):
        return False
    ...
    # ``alvarez gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_alvarez_home": str(get_alvarez_home()),
