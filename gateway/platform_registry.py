...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``alvarez setup`` display).
    ...
    # built-ins).  Used by ``alvarez gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `alvarez`
        # invocation -- including plain `alvarez chat`, which never touches any
        ...
        # `alvarez setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``alvarez setup``/``gateway status``, channel
