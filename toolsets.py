...
_ALVAREZ_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on ALVAREZ_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (ALVAREZ_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `alvarez tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``alvarez tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ALVAREZ_KANBAN_TASK env "
    ...
    # Coding posture (base Alvarez — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in alvarez_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Alvarez toolsets (CLI + messaging platforms)
    #
    ...
    # the `alvarez send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "alvarez-cli": {
        ...
        "tools": _ALVAREZ_CORE_TOOLS,
        "includes": []
    },

    "alvarez-cron": {
        # Mirrors alvarez-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `alvarez tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as alvarez-cli; gated by `alvarez tools`",
        "tools": _ALVAREZ_CORE_TOOLS,
        "includes": []
    },
    ...
    "alvarez-webhook": {
        ...
        "tools": _ALVAREZ_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "alvarez-gateway": {
        ...
        "tools": [],
        "includes": ["alvarez-telegram", "alvarez-webhook"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``alvarez-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ALVAREZ_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``alvarez-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (alvarez-<name>).
        # Gives them _ALVAREZ_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("alvarez-"):
            platform_name = name[len("alvarez-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ALVAREZ_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
