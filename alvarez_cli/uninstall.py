"""
Alvarez Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.alvarez/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Alvarez PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing alvarez-agent or alvarez PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Alvarez Agent" comment and following line
                if '# Alvarez Agent' in line or '# alvarez-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('alvarez' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing alvarez
                if 'alvarez' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the alvarez wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "alvarez",
        Path("/usr/local/bin/alvarez"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains alvarez_cli reference)
                content = wrapper.read_text()
                if 'alvarez_cli' in content or 'alvarez-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(alvarez_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``alvarez`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Alvarez home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (alvarez_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``alvarez gateway run`` processes
    ...
    try:
        from alvarez_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from alvarez_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ALVAREZ_HOME`` and ``ALVAREZ_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\alvarez\git\cmd``, ``%LOCALAPPDATA%\alvarez\git\bin``,
#      ``%LOCALAPPDATA%\alvarez\git\usr\bin``, ``%LOCALAPPDATA%\alvarez\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\alvarez\git\`` and Node to
#      ``%LOCALAPPDATA%\alvarez\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``alvarez dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\alvarez\gateway-service\`` and sometimes
...
def _alvarez_path_markers(alvarez_home: Path) -> list[str]:
    """Path-entry substrings that identify Alvarez-owned User-PATH entries."""
    root = str(alvarez_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare alvarez-agent install dir.
    markers = [root + "\\alvarez-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ALVAREZ_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "alvarez".  We don't want to catch
    # unrelated entries like "calvarez-foo" or "ephermeral", so we look for
    # backslash-alvarez as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(alvarez_home: Path) -> list[str]:
    """Strip Alvarez-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _alvarez_path_markers(alvarez_home)
            kept: list[str] = []
...
def remove_alvarez_env_vars_windows() -> list[str]:
    """Delete ALVAREZ_HOME and ALVAREZ_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ALVAREZ_HOME", "ALVAREZ_GIT_BASH_PATH"):
                try:
...
def _is_default_alvarez_home(alvarez_home: Path) -> bool:
    """Return True when ``alvarez_home`` points at the default (non-profile) root."""
    try:
        from alvarez_constants import get_default_alvarez_root
        return alvarez_home.resolve() == get_default_alvarez_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ALVAREZ_HOME directory.

    We shell out to ``alvarez -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ALVAREZ_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m alvarez_cli.main` so we don't depend on a `alvarez`
    #    wrapper that may be half-removed mid-uninstall.
    alvarez_invocation = [_sys.executable, "-m", "alvarez_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                alvarez_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ALVAREZ_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``alvarez uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ALVAREZ_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from alvarez_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(alvarez_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Alvarez Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Alvarez Chat GUI installation was found.")
        print(f"  Checked: {alvarez_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Alvarez agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(alvarez_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Alvarez agent at {alvarez_home / 'alvarez-agent'}")
        print(f"  • Your config, sessions, and secrets under {alvarez_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Alvarez agent is still installed. Run 'alvarez' to use the CLI,")
    print("or 'alvarez uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.alvarez/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.alvarez/ for future reinstall
    """
    project_root = get_project_root()
    alvarez_home = get_alvarez_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ALVAREZ_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_alvarez_home(alvarez_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.alvarez data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Alvarez Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {alvarez_home / 'config.yaml'}")
    print(f"  Secrets: {alvarez_home / '.env'}")
    print(f"  Data:    {alvarez_home / 'cron/'}, {alvarez_home / 'sessions/'}, {alvarez_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ALVAREZ_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Alvarez data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Alvarez code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``alvarez`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ALVAREZ_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in alvarez_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\alvarez\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(alvarez_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Alvarez-owned PATH entries in User environment")

        log_info("Removing ALVAREZ_HOME / ALVAREZ_GIT_BASH_PATH User env vars...")
        removed_env = remove_alvarez_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Alvarez-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing alvarez command...")
    ...
    #     (only when they still point into this Alvarez home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Alvarez-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(alvarez_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Alvarez-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(alvarez_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ALVAREZ_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from alvarez_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(alvarez_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.alvarez/, just remove the alvarez-agent subdir
            if alvarez_home in project_root.parents or project_root.parent == alvarez_home:
    ...
    #     under ALVAREZ_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(alvarez_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(alvarez_home)
    ...
    # 5. Optionally remove ~/.alvarez/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ALVAREZ_HOME dirs live under
        ...
        try:
            if alvarez_home.exists():
                shutil.rmtree(alvarez_home)
                log_success(f"Removed {alvarez_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {alvarez_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {alvarez_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {alvarez_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  (reinstall from a git checkout of the Alvarez repo)", Colors.DIM))
        else:
            print(color("  (reinstall from a git checkout of the Alvarez repo)", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Alvarez Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m alvarez_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import alvarez_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``alvarez_constants`` + ``alvarez_cli.colors``
    (and lazily ``alvarez_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m alvarez_cli.uninstall")
    parser.add_argument(
        "--mode",
