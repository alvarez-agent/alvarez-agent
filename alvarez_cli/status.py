"""
Status command for alvarez CLI.

Shows the status of all Alvarez Agent components.
"""
...
from alvarez_cli.auth import AuthError, resolve_provider
from alvarez_cli.colors import Colors, color
from alvarez_cli.config import get_env_path, get_env_value, get_alvarez_home, load_config
from alvarez_cli.models import provider_label
from alvarez_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``alvarez config``'s
...
def show_status(args):
    """Show status of all Alvarez Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Alvarez Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: alvarez model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: alvarez auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: alvarez auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: alvarez model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from alvarez_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from alvarez_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   alvarez gateway")
    ...
    jobs_file = get_alvarez_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_alvarez_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'alvarez doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'alvarez setup' to configure", Colors.DIM))
    print()
