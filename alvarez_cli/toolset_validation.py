...
the tool registry or launching Alvarez (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``alvarez-cli`` to the non-existent ``alvarez``. ``resolve_toolset('alvarez')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``alvarez-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``alvarez`` instead of ``alvarez-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `alvarez tools` to reconfigure."
        )
    return warnings
