...
# xAI's public API is OpenAI-compatible for the endpoints Alvarez commonly
# uses. The Responses endpoint is included because Alvarez' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Alvarez-managed OAuth credentials."""

    auth_hint = "alvarez auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`alvarez auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`alvarez auth reset xai-oauth` or re-authenticate with "
                    "`alvarez auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `alvarez auth add xai-oauth --type oauth`."
            )
