"""CLI handlers for the ``alvarez proxy`` subcommand."""
...
from alvarez_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "alvarez proxy requires aiohttp. Install one of:\n"
        "  pip install 'alvarez-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"alvarez auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Alvarez proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Alvarez proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: alvarez proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``alvarez proxy <subcommand>``."""
    ...
    print(
        "alvarez proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  alvarez proxy start [--provider xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  alvarez proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  alvarez proxy providers\n"
