"""
Alvarez Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``alvarez desktop`` / ``alvarez gui``)
     Built inside the agent checkout under ``$ALVAREZ_HOME/alvarez-agent/``:
       ...
       - ``$ALVAREZ_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Alvarez.app`` or ``~/Applications/Alvarez.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Alvarez`` (NSIS per-user)
...
the app name ("Alvarez"), separate from ``$ALVAREZ_HOME``:
  - macOS:   ``~/Library/Application Support/Alvarez``
  - Windows: ``%APPDATA%\\Alvarez``
  - Linux:   ``$XDG_CONFIG_HOME/Alvarez`` (default ``~/.config/Alvarez``)
...
``alvarez uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(alvarez_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return alvarez_home / "alvarez-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Alvarez"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Alvarez"
...
def source_built_gui_artifacts(alvarez_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``alvarez desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``alvarez-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(alvarez_home)
    ...
    return [
        ...
        alvarez_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Alvarez" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Alvarez.app"),
            home / "Applications" / "Alvarez.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Alvarez).
            local_base / "Programs" / "Alvarez",
            # Older / alternate layout some builds used.
            local_base / "alvarez-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Alvarez")
    else:
        ...
        paths += [
            data_base / "applications" / "alvarez.desktop",
            data_base / "applications" / "Alvarez.desktop",
        ]
    return paths


def agent_is_installed(alvarez_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ALVAREZ_HOME.
    ...
    if (agent_root / "alvarez_cli").is_dir():
        return True
...
def gui_is_installed(alvarez_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(alvarez_home):
        if p.exists():
            return True
...
def gui_install_summary(alvarez_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = alvarez_home if alvarez_home is not None else get_alvarez_home()
...
def uninstall_gui(alvarez_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``alvarez-agent/alvarez_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ALVAREZ_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove alvarez' or "
            "'sudo dnf remove alvarez'). AppImage builds are a single file you "
