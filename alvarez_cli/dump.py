"""
Dump command for alvarez CLI.

Outputs a compact, plain-text summary of the user's Alvarez setup
...
from alvarez_cli.config import get_alvarez_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.alvarez_build_sha`` by the Dockerfile's
    ``ALVAREZ_GIT_SHA`` build-arg (see ``alvarez_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``alvarez dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from alvarez_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(alvarez_home: Path) -> int:
...
def _cron_summary(alvarez_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = alvarez_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"alvarez_home:      {display_alvarez_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `alvarez auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `alvarez auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["alvarez-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(alvarez_home)}")
    lines.append(f"  skills:             {_count_skills(alvarez_home)}")
