"""
Interactive setup wizard for Alvarez Agent.
...
from alvarez_cli.nous_subscription import get_nous_subscription_features
...
# display_alvarez_home imported lazily at call sites (stale-module safety during alvarez update)
...
from alvarez_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from alvarez_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Alvarez Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Alvarez using environment variables or config commands:")
    print_info("  alvarez config set model.provider custom")
    print_info("  alvarez config set model.base_url http://localhost:8080/v1")
    print_info("  alvarez config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'alvarez setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``ALVAREZ_NONINTERACTIVE=1`` (see ``alvarez_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ALVAREZ_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ALVAREZ_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from alvarez_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'alvarez setup')")


def _print_setup_summary(config: dict, alvarez_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'alvarez setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from alvarez_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `alvarez tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from alvarez_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'alvarez setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'alvarez setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'alvarez setup terminal'"))
    ...
    # Spotify (OAuth via alvarez auth spotify — check auth.json, not env vars)
    try:
        from alvarez_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'alvarez setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {alvarez_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('alvarez setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('alvarez setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('alvarez setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('alvarez setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('alvarez setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('alvarez config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('alvarez config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('alvarez config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('alvarez', Colors.GREEN)}              Start chatting")
    print(f"   {color('alvarez gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('alvarez doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `alvarez tools` and `alvarez setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``alvarez model``)
    ...
    provider added to ``alvarez model`` is automatically available here.
    ...
    # Delegate to the shared alvarez model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from alvarez_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: alvarez model")
    ...
    # on demand via `alvarez auth add`, `alvarez setup` vision, and
    # `alvarez setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``alvarez model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from alvarez_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Alvarez auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via alvarez setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'alvarez setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Alvarez runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `alvarez setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `alvarez setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ALVAREZ_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ALVAREZ_MAX_ITERATIONS")
    ...
    print_info("  Run `alvarez setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ALVAREZ_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from alvarez_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_alvarez_home(Path(get_alvarez_home()))
    except Exception:
        pass
...
def _profile_name_from_alvarez_home(alvarez_home) -> str | None:
    """Return the active profile name when ALVAREZ_HOME is a profile dir."""
    if alvarez_home.parent.name == "profiles":
        return alvarez_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Alvarez delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Alvarez to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from alvarez_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  alvarez config edit")
    print_info("   Open config in your editor:  alvarez config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from alvarez_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Alvarez from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'alvarez setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     alvarez config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_alvarez_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: alvarez gateway install --system")
                print_info("  Or run in foreground:  alvarez gateway")
        else:
            from alvarez_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   alvarez gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   alvarez gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `alvarez setup tools` and `alvarez tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``alvarez_cli.auth`` — lists every supported
    ...
    """
    try:
        from alvarez_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from alvarez_cli.config
    ...
    if section_key == "model":
        ...
        from alvarez_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_alvarez.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_alvarez migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_alvarez", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Alvarez.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Alvarez to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Alvarez at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Alvarez at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Alvarez at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Alvarez at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Alvarez equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Alvarez config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Alvarez.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Alvarez's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(alvarez_home: Path) -> bool:
    ...
    print_info("Alvarez can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: alvarez claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=alvarez_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: alvarez claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Alvarez configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Alvarez config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Alvarez (use alvarez claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      alvarez setup           — full or quick (auto-detected)
      alvarez setup model     — just model/provider
      alvarez setup tts       — just text-to-speech
      alvarez setup terminal  — just terminal backend
      alvarez setup gateway   — just messaging platforms
      alvarez setup tools     — just tool configuration
      alvarez setup agent     — just agent settings
    """
    from alvarez_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Alvarez Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Alvarez Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Alvarez Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, alvarez_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Alvarez configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'alvarez setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(alvarez_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Alvarez?",
            [
                "Quick Setup — pick a provider and model, sensible defaults for the rest (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, alvarez_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, alvarez_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {alvarez_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'alvarez config edit'")
    ...
    # Tune later with `alvarez setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, alvarez_home)


def _run_first_time_quick_setup(config: dict, alvarez_home, is_existing: bool):
    """Streamlined first-time setup: provider + model, terminal & messaging.

    Runs the provider/model picker, then configures the terminal backend and
    (optionally) a messaging platform. Applies sensible defaults for
    everything else (agent settings, tools); the user can customize later via
    ``alvarez setup <section>`` or switch providers with ``alvarez model``.
    """
    from alvarez_cli.config import load_config

    # Step 1: Provider + model selection (same flow as `alvarez model`).
    try:
    ...
    # Re-sync the wizard's config dict from disk — the provider flow (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'alvarez setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    alvarez setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  alvarez setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``alvarez tools`` (which
    ...
    try:
        ...
        from alvarez_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``alvarez setup agent`` /
    ``alvarez config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, alvarez_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `alvarez update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        alvarez tools")
        print_info("  Seed skills:         alvarez skills opt-in --sync")
        print_info("  Add MCP servers:     alvarez mcp add")
        print_info("  Enable plugins:      alvarez plugins")
        print_info("  Tune agent settings: alvarez setup agent")
        print()
        _print_setup_summary(config, alvarez_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, alvarez_home)


def _blank_slate_walkthrough(config: dict, alvarez_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`alvarez update` runs from re-injecting them. Opt back in any")
            print_info("time with `alvarez skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `alvarez tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `alvarez plugins list` / `alvarez plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `alvarez plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `alvarez mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `alvarez mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   alvarez tools")
    print_info("  Seed skills:         alvarez skills opt-in --sync")
    print_info("  Add MCP servers:     alvarez mcp add")
    print_info("  Tune agent settings: alvarez setup agent")
    print()
...
def _run_quick_setup(config: dict, alvarez_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'alvarez setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Alvarez to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'alvarez setup gateway'.")
