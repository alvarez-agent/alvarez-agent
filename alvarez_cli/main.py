#!/usr/bin/env python3
"""
Alvarez CLI - Main entry point.

Usage:
    alvarez                     # Interactive chat (default)
    alvarez chat                # Interactive chat
    alvarez gateway             # Run gateway in foreground
    alvarez gateway start       # Start gateway as service
    alvarez gateway stop        # Stop gateway service
    alvarez gateway status      # Show gateway status
    alvarez gateway install     # Install gateway service
    alvarez gateway uninstall   # Uninstall gateway service
    alvarez setup               # Interactive setup wizard
    alvarez logout              # Clear stored authentication
    alvarez status              # Show status of all components
    alvarez cron                # Manage cron jobs
    alvarez cron list           # List cron jobs
    alvarez cron status         # Check if cron scheduler is running
    alvarez doctor              # Check configuration and dependencies
    alvarez honcho setup                    # Configure Honcho AI memory integration
    alvarez honcho status                   # Show Honcho config and connection status
    alvarez honcho sessions                 # List directory → session name mappings
    alvarez honcho map <name>               # Map current directory to a session name
    alvarez honcho peer                     # Show peer names and dialectic settings
    alvarez honcho peer --user NAME         # Set user peer name
    alvarez honcho peer --ai NAME           # Set AI peer name
    alvarez honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    alvarez honcho mode                     # Show current memory mode
    alvarez honcho mode [hybrid|honcho|local]  # Set memory mode
    alvarez honcho tokens                   # Show token budget settings
    alvarez honcho tokens --context N       # Set session.context() token cap
    alvarez honcho tokens --dialectic N     # Set dialectic result char cap
    alvarez honcho identity                 # Show AI peer identity representation
    alvarez honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    alvarez honcho migrate                  # Step-by-step migration guide: OpenClaw native → Alvarez + Honcho
    alvarez version             Show version
    alvarez update              Update to latest version
    alvarez uninstall           Uninstall Alvarez Agent
    alvarez acp                 Run as an ACP server for editor integration
    alvarez sessions browse     Interactive session picker with search

    alvarez claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: alvarez_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``alvarez_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``alvarez update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``alvarez_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, alvarez crashes on import and the user can't run
# ``alvarez update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import alvarez_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'alvarez' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``alvarez tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``alvarez.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("alvarez")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"alvarez", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"alvarez")
        # Windows: the .exe name is already ``alvarez.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* alvarez_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ALVAREZ_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".alvarez", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ALVAREZ_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ALVAREZ_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ALVAREZ_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ALVAREZ_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`alvarez --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from alvarez_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Alvarez Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``alvarez --version`` before config/logging imports on Termux."""
    if os.environ.get("ALVAREZ_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from alvarez_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from alvarez_cli.subcommands.cron import build_cron_parser
from alvarez_cli.subcommands.gateway import build_gateway_parser
from alvarez_cli.subcommands.profile import build_profile_parser
from alvarez_cli.subcommands.model import build_model_parser
from alvarez_cli.subcommands.setup import build_setup_parser
from alvarez_cli.subcommands.postinstall import build_postinstall_parser
from alvarez_cli.subcommands.slack import build_slack_parser
from alvarez_cli.subcommands.login import build_login_parser
from alvarez_cli.subcommands.logout import build_logout_parser
from alvarez_cli.subcommands.auth import build_auth_parser
from alvarez_cli.subcommands.status import build_status_parser
from alvarez_cli.subcommands.webhook import build_webhook_parser
from alvarez_cli.subcommands.hooks import build_hooks_parser
from alvarez_cli.subcommands.doctor import build_doctor_parser
from alvarez_cli.subcommands.security import build_security_parser
from alvarez_cli.subcommands.dump import build_dump_parser
from alvarez_cli.subcommands.debug import build_debug_parser
from alvarez_cli.subcommands.backup import build_backup_parser
from alvarez_cli.subcommands.import_cmd import build_import_cmd_parser
from alvarez_cli.subcommands.config import build_config_parser
from alvarez_cli.subcommands.version import build_version_parser
from alvarez_cli.subcommands.update import build_update_parser
from alvarez_cli.subcommands.uninstall import build_uninstall_parser
from alvarez_cli.subcommands.gui import build_gui_parser
from alvarez_cli.subcommands.logs import build_logs_parser
from alvarez_cli.subcommands.prompt_size import build_prompt_size_parser
from alvarez_cli.subcommands.memory import build_memory_parser
from alvarez_cli.subcommands.acp import build_acp_parser
from alvarez_cli.subcommands.tools import build_tools_parser
from alvarez_cli.subcommands.insights import build_insights_parser
from alvarez_cli.subcommands.skills import build_skills_parser
from alvarez_cli.subcommands.pairing import build_pairing_parser
from alvarez_cli.subcommands.plugins import build_plugins_parser
from alvarez_cli.subcommands.mcp import build_mcp_parser
from alvarez_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (alvarez tools, alvarez setup, alvarez model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'alvarez {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any alvarez module import.
#
# Many modules cache ALVAREZ_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ALVAREZ_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.alvarez/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ALVAREZ_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `alvarez mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Alvarez' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo alvarez -p <name>` against the invoking user's home.
        ...
        candidate = home / ".alvarez" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`alvarez chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors alvarez_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ALVAREZ_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.alvarez/profiles/coder or
    # /opt/data/profiles/coder).  If ALVAREZ_HOME points to the alvarez root
    # instead (e.g. systemd hardcodes ALVAREZ_HOME=/root/.alvarez), we must
    # still read active_profile — the user may have switched profiles via
    # `alvarez profile use` and the gateway should honour that choice.
    # See issue #22502.
    alvarez_home_env = os.environ.get("ALVAREZ_HOME", "")
    if profile_name is None and alvarez_home_env:
        if Path(alvarez_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the alvarez root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ALVAREZ_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``alvarez gateway run`` to mean
    # "the root ALVAREZ_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ALVAREZ_S6_SUPERVISED_CHILD"):
        try:
            from alvarez_constants import get_default_alvarez_root

            active_path = get_default_alvarez_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ALVAREZ_HOME
    if profile_name is not None:
        try:
            from alvarez_cli.profiles import resolve_profile_env

            alvarez_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            alvarez_home = _resolve_sudo_user_profile_env(profile_name)
            if not alvarez_home:
            ...
            # A bug in profiles.py must NEVER prevent alvarez from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ALVAREZ_HOME"] = alvarez_home
...
# Load .env from ~/.alvarez/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from alvarez_cli.config import get_alvarez_home
from alvarez_cli.env_loader import load_alvarez_dotenv

load_alvarez_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ALVAREZ_REDACT_SECRETS env
# var BEFORE alvarez_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_alvarez_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from alvarez_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "ALVAREZ_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ALVAREZ_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `alvarez` subcommands
...
try:
    from alvarez_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from alvarez_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if alvarez_constants not importable yet
...
from alvarez_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to alvarez_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (alvarez_cli.main._model_flow_*) keep resolving unchanged.
from alvarez_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_alvarez_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ALVAREZ_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ALVAREZ_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from alvarez_cli.config import get_env_path, get_alvarez_home, load_config
    from alvarez_cli.auth import get_auth_status

    # Determine whether Alvarez itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from alvarez_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_alvarez_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from alvarez_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for OAuth credentials in the auth store
    auth_file = get_alvarez_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Alvarez has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Alvarez to use their tokens.
    if _has_alvarez_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, alvarez_bin
        cli_args: the original CLI arguments (everything after 'alvarez')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    alvarez_bin = container_info["alvarez_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo alvarez {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo alvarez {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  alvarez --tui --resume {target}")
    if title:
        print(f'  alvarez --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @alvarez/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@alvarez" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/alvarez-ink/package.json",
    "packages/alvarez-ink/index.js",
    "packages/alvarez-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ALVAREZ_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ALVAREZ_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ALVAREZ_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ALVAREZ_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    alvarez_home = os.environ.get("ALVAREZ_HOME") or str(Path.home() / ".alvarez")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ALVAREZ_HOME": alvarez_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(alvarez_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(alvarez_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if alvarez_cli_dir is None:
        alvarez_cli_dir = Path(__file__).parent
    bundled = alvarez_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``alvarez update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ALVAREZ_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Alvarez checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `alvarez update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Alvarez checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `alvarez --tui`\n"
        "If the checkout is still inconsistent, run `alvarez update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ALVAREZ_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ALVAREZ_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from alvarez_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ALVAREZ_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ALVAREZ_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ALVAREZ_TUI_DIR (e.g. `unset ALVAREZ_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @alvarez/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @alvarez/ink resolves through
        # packages/alvarez-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "alvarez-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from alvarez_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from alvarez_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="alvarez-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ALVAREZ_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ALVAREZ_PYTHON_SRC_ROOT"] = os.environ.get(
        "ALVAREZ_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ALVAREZ_PYTHON", sys.executable)
    env.setdefault("ALVAREZ_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ALVAREZ_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ALVAREZ_MODEL"] = model
        env["ALVAREZ_INFERENCE_MODEL"] = model
    if provider:
        env["ALVAREZ_TUI_PROVIDER"] = provider
        env["ALVAREZ_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ALVAREZ_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ALVAREZ_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ALVAREZ_TUI_SKILLS"] = value
    if query:
        env["ALVAREZ_TUI_QUERY"] = query
    if image:
        env["ALVAREZ_TUI_IMAGE"] = image
    if checkpoints:
        env["ALVAREZ_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ALVAREZ_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ALVAREZ_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ALVAREZ_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ALVAREZ_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ALVAREZ_ACCEPT_HOOKS"] = "1"
    ...
    # ALVAREZ_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `alvarez --tui` try to
    ...
    # still set ALVAREZ_TUI_RESUME themselves.
    env.pop("ALVAREZ_TUI_RESUME", None)
    if resume_session_id:
        env["ALVAREZ_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `alvarez update` so
    ...
    if code == 42:
        from alvarez_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ALVAREZ_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``alvarez kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``alvarez kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ALVAREZ_KANBAN_BOARD"):
        return
    try:
        from alvarez_cli.kanban_db import get_current_board

        os.environ["ALVAREZ_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.alvarez/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Alvarez — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Alvarez still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ALVAREZ_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ALVAREZ_TUI") == "1":
        return True
    try:
        from alvarez_cli.config import load_config
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'alvarez sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from alvarez_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from alvarez_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'alvarez doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Alvarez isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  alvarez setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'alvarez setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from alvarez_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ALVAREZ_YOLO_MODE"] = "1"
    ...
    # setup (config, rules files, plugins, MCP servers) or from Alvarez itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (ALVAREZ_SAFE_MODE is checked by
    # alvarez_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["ALVAREZ_SAFE_MODE"] = "1"
        os.environ["ALVAREZ_IGNORE_USER_CONFIG"] = "1"
        os.environ["ALVAREZ_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.alvarez/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["ALVAREZ_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from alvarez_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from alvarez_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_setup(args):
    ...
    from alvarez_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from alvarez_cli.config import stamp_install_method
    from alvarez_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Alvarez post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from alvarez_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``alvarez model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from alvarez_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from alvarez_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ALVAREZ_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from alvarez_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'alvarez model' for "
                "available providers, or run 'alvarez doctor' to diagnose config "
                "issues."
            )
    ...
    from alvarez_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        group_providers,
    ...
    # PROVIDER_GROUPS in alvarez_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.alvarez/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.alvarez/.env if the active provider is not 'custom'.
    ...
    """
    from alvarez_cli.config import get_env_value, save_env_value, load_config
...
#
# Alvarez uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `alvarez model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `alvarez model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`alvarez_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from alvarez_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from alvarez_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Alvarez only falls back to a lightweight backend (e.g.")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``alvarez model`` flow, then route aux tasks to them here.
    """
    from alvarez_cli.config import load_config
    from alvarez_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from alvarez_cli.auth import _prompt_model_selection
    from alvarez_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from alvarez_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from alvarez_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from alvarez_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Alvarez URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from alvarez_cli.curses_ui import curses_radiolist
...
# downstream call sites read `alvarez_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `alvarez` invocation — including
# fast paths like `alvarez --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``alvarez setup`` / ``alvarez model``.
    ...
    recover from a malformed paste without editing ``~/.alvarez/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from alvarez_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from alvarez_cli.config import save_env_value
    from alvarez_cli.secret_prompt import masked_secret_prompt
    ...
    from alvarez_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `alvarez setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from alvarez_constants import display_alvarez_home as _dhh_fn

            print(
                f"    Alvarez will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Alvarez CLI with a provider."""
    from alvarez_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from alvarez_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from alvarez_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from alvarez_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from alvarez_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from alvarez_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``alvarez slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: alvarez slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `alvarez slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from alvarez_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from alvarez_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from alvarez_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from alvarez_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from alvarez_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `alvarez security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from alvarez_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from alvarez_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from alvarez_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from alvarez_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Alvarez home directory to a zip file."""
    if getattr(args, "quick", False):
        from alvarez_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from alvarez_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Alvarez backup from a zip file."""
    from alvarez_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from alvarez_cli.banner import format_banner_version_label
    ...
    try:
        ...
        from alvarez_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Alvarez Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from alvarez_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from alvarez_cli.uninstall import run_gui_uninstall
    ...
    from alvarez_cli.uninstall import run_uninstall
...
# Critical files that every ``alvarez`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``alvarez update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "alvarez_cli/main.py",
    "alvarez_cli/config.py",
    "alvarez_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "alvarez_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``alvarez`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="alvarez-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``alvarez update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``alvarez_cli/web_dist/`` (per vite.config.ts
    outDir: "../alvarez_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "alvarez_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``alvarez`` launcher present but ``alvarez_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the alvarez venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — alvarez venv python3, no nix-shell overhead
    ...
    # a alvarez venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``alvarez update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``alvarez web``).
    ...
    # build path usable on a stock `py -m alvarez_cli.main web` invocation.
    ...
    from alvarez_constants import find_node_executable, with_alvarez_node_path
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_alvarez_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (alvarez web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``alvarez update`` can unconditionally call ``alvarez desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``alvarez desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ALVAREZ_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ALVAREZ_HOME."""
    from alvarez_constants import get_alvarez_home
    return get_alvarez_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``alvarez update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Alvarez.app/Contents/MacOS/Alvarez"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Alvarez.exe",
            release_dir / "win-ia32-unpacked" / "Alvarez.exe",
            release_dir / "win-arm64-unpacked" / "Alvarez.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "alvarez",
            release_dir / "linux-unpacked" / "Alvarez",
            release_dir / "linux-arm64-unpacked" / "alvarez",
            release_dir / "linux-arm64-unpacked" / "Alvarez",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Alvarez'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_alvarez_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Alvarez.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Alvarez.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Alvarez.exe: Access is
    ...
    an unrelated "Alvarez" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Alvarez is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Alvarez.app/Contents/MacOS/Alvarez  ->  app bundle = .../Alvarez.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``alvarez desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Alvarez Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Alvarez Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the ALVAREZ_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from alvarez_logging import setup_logging as _setup_logging_gui
    ...
    # with_alvarez_node_path() copies os.environ when called with no arg.
    env = with_alvarez_node_path()
    if getattr(args, "fake_boot", False):
        env["ALVAREZ_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ALVAREZ_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "alvarez_root", None):
        env["ALVAREZ_DESKTOP_ALVAREZ_ROOT"] = str(Path(args.alvarez_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ALVAREZ_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    # `ALVAREZ_DESKTOP_DISABLE_GPU=... alvarez desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ALVAREZ_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ALVAREZ_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  alvarez gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Alvarez.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Alvarez.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Alvarez.exe, close any")
                    print("  running Alvarez desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> alvarez desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Alvarez is
    ...
    if source_mode:
        print("→ Launching Alvarez Desktop from source build...")
    ...
    print(f"→ Launching packaged Alvarez Desktop: {' '.join(launch_command)}")
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `alvarez update`.
    ...
    print("  Preview now:  alvarez curator run --dry-run")
    print("  Pause it:     alvarez curator pause")
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``alvarez update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``alvarez update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: alvarez curator status)"
    )
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.alvarez-update-staging"
    backup = f"{dst}.alvarez-update-old"
...
def _update_via_zip(args):
    """Update Alvarez Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `alvarez update "
            f"--branch {branch}`, or update against main with `alvarez update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="alvarez-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"alvarez-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for alvarez-agent itself
        ...
        # GitHub ZIPs extract to alvarez-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"alvarez-agent-{branch}")
    ...
    from alvarez_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: alvarez skills list-modified  "
    ...
    try:
        from alvarez_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "alvarez-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Alvarez behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes alvarez completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Alvarez couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Alvarez couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Alvarez behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Alvarez couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Alvarez couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `alvarez update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_alvarez_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_alvarez_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``alvarez update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from alvarez_constants import get_default_alvarez_root

    default_home = get_default_alvarez_root()
...
# Install-scoped breadcrumb dropped right before ``alvarez update`` mutates the
...
# survives and the next ``alvarez`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ALVAREZ_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``alvarez update`` left half-done.
    ...
    protocol stream (``alvarez acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if alvarez.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _alvarez_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Alvarez is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Alvarez from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `alvarez update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from alvarez_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``alvarez update`` is still progressing even if pip/uv itself is silent.
    """
...
def _alvarez_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_alvarez_instances(
    ...
    The Alvarez Desktop Electron app spawns ``alvarez.EXE`` as a backend child,
    so during ``alvarez update`` the user-invoked process and the desktop's
    ...
    shims (``alvarez.exe`` / ``alvarez-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``alvarez update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``alvarez.exe``) is a separate process from the Python
    ...
    for shim in _alvarez_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated alvarez.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``alvarez update``
    ...
    #      alvarez.exe sitting *under* a non-Alvarez parent (e.g. a Alvarez
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "alvarez.exe"
    lines = ["✗ Another alvarez.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Alvarez Desktop, exit any open `alvarez` REPLs, and")
    lines.append("  stop the gateway (`alvarez gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `alvarez update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_alvarez_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``alvarez.exe``.
    ...
    when ``alvarez update`` runs, ``alvarez.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``alvarez.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next alvarez invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Alvarez Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Alvarez Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _alvarez_exe_shims(scripts_dir):
        ...
        print(
            "    Close Alvarez Desktop, exit other `alvarez` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `alvarez update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_alvarez_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``alvarez.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``alvarez.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``alvarez`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_alvarez_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``alvarez.exe.old.*`` left by prior updates.

    Called early on every alvarez invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``alvarez update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `alvarez update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``alvarez.exe`` / ``alvarez-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_alvarez_exe`` for the rationale.
    """
    ...
    # hours later inside ``alvarez update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``alvarez.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``alvarez.exe`` shim is locked during ``alvarez update``, or when uv/distlib
    skips a launcher write. The symptom is ``alvarez-agent.exe`` and
    ``alvarez-acp.exe`` present but ``alvarez.exe`` missing, so ``alvarez`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `alvarez update --force` after "
            "closing other alvarez processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m alvarez_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``alvarez update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``alvarez.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``alvarez`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `alvarez update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `alvarez update --force` after closing other alvarez processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from alvarez_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ALVAREZ_HOME/bin/uv``, but on Termux the official
    ...
    """
    from alvarez_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `alvarez update`, so we install root-only first
    ...
    nixos_env = with_alvarez_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `alvarez update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``alvarez update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.alvarez/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``alvarez update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``alvarez update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.alvarez/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``alvarez update --gateway``) the update is already
    ...
    try:
        ...
        # alvarez_cli.config.get_alvarez_home to simulate setup failure.
        from alvarez_cli.config import get_alvarez_home as _get_alvarez_home

        logs_dir = _get_alvarez_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.alvarez/logs/update.log`` only, never the terminal.

    During ``alvarez update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``alvarez update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from alvarez_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``alvarez update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from alvarez_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from alvarez_cli.config import recommended_update_command
        from alvarez_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``alvarez update`` without requiring a reinstall.  The
    ...
    ``alvarez`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/alvarez.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v alvarez'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/alvarez, code at /usr/local/lib/alvarez-agent).
    fhs_link = Path("/usr/local/bin/alvarez")
    ...
    path_comment = (
        "# Alvarez Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ALVAREZ_HOME before running the update.
    ...
    ALVAREZ_HOME directories.  The ``--backup`` flag on ``alvarez update``
    ...
    # ``alvarez_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # ALVAREZ_HOME can add minutes to every update. Users who want the
    ...
    try:
        from alvarez_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_alvarez_home so the user sees ~/.alvarez/...
    try:
        from alvarez_constants import get_alvarez_home, display_alvarez_home

        home = get_alvarez_home()
        try:
            display_path = f"{display_alvarez_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  alvarez import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    alvarez.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from alvarez_cli.gateway import (
    ...
    if not running_pids:
        ...
        try:
            from alvarez_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Alvarez...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m alvarez_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: alvarez gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``alvarez gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from alvarez_cli import gateway_windows
        from alvarez_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``alvarez update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Alvarez Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Alvarez Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `alvarez update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Alvarez via pip (for PyPI installs)."""
    from alvarez_cli import __version__
    from alvarez_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "alvarez-agent"]
        ...
        cmd = [pipx, "upgrade", "alvarez-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "alvarez-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "alvarez-agent"]
    ...
    print("✓ Update complete! Restart alvarez to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Alvarez Agent...")
    print()

    # On Windows, abort early if another alvarez.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_alvarez_instances(scripts_dir)
            if concurrent:
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            if method == "pip":
                ...
                return
            print("✗ Not a git repository. Reinstall from a git checkout of the Alvarez repo.")
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $ALVAREZ_HOME, this is cheap
        ...
        try:
            from alvarez_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in alvarez_cli/config.py bricked
        # every user who ran ``alvarez update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``alvarez
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``alvarez update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_alvarez_home added to alvarez_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``alvarez`` launch finishes the
        ...
        # build.  ``alvarez desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``alvarez desktop`` shouldn't be forced into a full
        # Electron build by ``alvarez update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from alvarez_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "alvarez_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `alvarez desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from alvarez_constants import display_alvarez_home as _dhh
        ...
        # current — so copy it straight over ~/.alvarez/cache/model_catalog.json
        ...
        # modules in this process.  Reload alvarez_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_alvarez_home() added since the last release.
        try:
            import importlib
            import alvarez_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ALVAREZ_HOME so
        # it is not affected by sync_skills()'s module-level ALVAREZ_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ALVAREZ_HOME env var points at the default or a named profile.
        try:
            from alvarez_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from alvarez_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'alvarez config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: alvarez config migrate")
            else:
                print()
                print("Skipped. Run 'alvarez config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from alvarez_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `alvarez curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``alvarez update`` gives users a
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from alvarez_cli.tools_config import install_cua_driver
        ...
        # When running as ``alvarez update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_alvarez_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... alvarez-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... alvarez-gateway*)
            ...
            # Discover all alvarez-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. alvarez-gateway.service or alvarez-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `alvarez update` restart it automatically, allow\n"
                            ...
                            # path in `alvarez gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: alvarez gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: alvarez gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: alvarez -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Alvarez gateway unit files are still installed.
        # When both alvarez.service (from a pre-rename install) and the
        # current alvarez-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `alvarez update` surfaces the issue until the user migrates.
        try:
            from alvarez_cli.gateway import (
                has_legacy_alvarez_units,
                _find_legacy_alvarez_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_alvarez_units():
                print()
                print("⚠ Legacy Alvarez gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_alvarez_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (alvarez.service) fight the current")
                print("  alvarez-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    alvarez gateway migrate-legacy")
                print()
        ...
        print("  alvarez model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``alvarez -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `alvarez profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_alvarez_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → alvarez -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.alvarez)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  alvarez profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     alvarez -p {name} chat")
                else:
        ...
        from alvarez_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from alvarez_constants import get_alvarez_home as _hh
        ...
        from alvarez_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `alvarez profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → alvarez -p {name}  ({wrapper})")
        print()
        ...
        from alvarez_cli.profiles import profile_exists, validate_alias_name
        ...
        from alvarez_cli.profiles import rename_profile
        ...
        from alvarez_cli.profiles import export_profile
        ...
        from alvarez_cli.profiles import import_profile
        ...
        import tempfile
        from alvarez_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="alvarez_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  alvarez -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      alvarez -p {plan.manifest.name} chat")
        ...
        from alvarez_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `alvarez profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"alvarez -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from alvarez_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("alvarez_requires"):
            print(f"Requires:     Alvarez {data['alvarez_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from alvarez_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.alvarez_requires:
        print(f"  Requires: Alvarez {mf.alvarez_requires}")
...
def cmd_prompt_size(args):
    ...
    from alvarez_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Alvarez log files."""
    from alvarez_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from alvarez_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``alvarez -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``alvarez_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``alvarez -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``alvarez`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ALVAREZ_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from alvarez_cli.plugins import discover_plugins
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from alvarez_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from alvarez_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from alvarez_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ALVAREZ_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ALVAREZ_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ALVAREZ_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `alvarez --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_alvarez_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_alvarez_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_alvarez_home()}/memories/\n")
    else:
        from alvarez_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Alvarez Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from alvarez_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from alvarez_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from alvarez_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from alvarez_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from alvarez_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from alvarez_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from alvarez_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from alvarez_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from alvarez_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for alvarez CLI."""
    # Cosmetic: make the process show up as 'alvarez' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from alvarez_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``alvarez.exe.old.*`` quarantine files left by previous
    # ``alvarez update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_alvarez_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``alvarez update``
    ...
    # ``alvarez skills install update``) merely defers recovery one launch;
    # under-matching (missing ``alvarez -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in alvarez_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from alvarez_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from alvarez_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `alvarez model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.alvarez/.env.  Currently supports "
    ...
    from alvarez_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from alvarez_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # setup command  (parser built in alvarez_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in alvarez_cli/subcommands/postinstall.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in alvarez_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from alvarez_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in alvarez_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in alvarez_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in alvarez_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in alvarez_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in alvarez_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in alvarez_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from alvarez_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from alvarez_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in alvarez_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in alvarez_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in alvarez_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in alvarez_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in alvarez_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in alvarez_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.alvarez/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo alvarez uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from alvarez_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in alvarez_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in alvarez_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in alvarez_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in alvarez_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from alvarez_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in alvarez_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``alvarez --help``, ``alvarez version``, ``alvarez logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from alvarez_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from alvarez_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Alvarez shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from alvarez_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from alvarez_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in alvarez_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in alvarez_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `alvarez computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `alvarez tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `alvarez computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Alvarez app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from alvarez_cli.tools_config import _cua_driver_cmd
            # Honor ALVAREZ_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from alvarez_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: alvarez computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: alvarez computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: alvarez computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: alvarez computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: alvarez computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: alvarez computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in alvarez_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_alvarez_home() / "sessions"
            ...
            # Launch alvarez --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in alvarez_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in alvarez_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in alvarez_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in alvarez_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in alvarez_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in alvarez_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in alvarez_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # for one release. The Alvarez-Setup.exe success screen tells users to
    # run `alvarez desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in alvarez_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in alvarez_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in alvarez_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``alvarez -c Pokemon Agent Dev`` → ``alvarez -c 'Pokemon Agent Dev'``
    ...
    from alvarez_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'alvarez -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (alvarez hooks list, cron
