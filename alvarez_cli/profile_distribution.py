"""Profile distributions — shareable, packaged Alvarez profiles via git.

A distribution is a Alvarez profile published as a git repository (or
...
* ``alvarez profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``alvarez skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``alvarez profile``, not a parallel tree):

    alvarez profile install <source> [--name N] [--alias] [--force] [--yes]
    alvarez profile update  <name>  [--force-config] [--yes]
    alvarez profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "alvarez_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".alvarez_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "alvarez-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            alvarez_requires=str(data.get("alvarez_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.alvarez_requires:
            out["alvarez_requires"] = self.alvarez_requires
        if self.author:
...
def check_alvarez_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Alvarez {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Alvarez distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``alvarez profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Alvarez profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Alvarez distribution."
        )

    # Version check up-front so we fail fast
    check_alvarez_requires(manifest.alvarez_requires, alvarez_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.alvarez).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="alvarez_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `alvarez profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`alvarez profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="alvarez_dist_update_") as tmp:
        plan = plan_install(
