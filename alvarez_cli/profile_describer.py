...
Used by ``alvarez profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``alvarez_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Alvarez Agent kanban board.
...
Rules:
  ...
  - Never write "Alvarez Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from alvarez_constants import get_alvarez_home  # type: ignore
            profile_dir = Path(get_alvarez_home())
        else:
