"""``alvarez logs`` — view and filter Alvarez log files.
...
Usage examples::
    ...
    alvarez logs                    # last 50 lines of agent.log
    alvarez logs -f                 # follow agent.log in real time
    alvarez logs errors             # last 50 lines of errors.log
    alvarez logs gateway -n 100    # last 100 lines of gateway.log
    alvarez logs gui -f            # follow gui.log (dashboard/pty/ws)
    alvarez logs desktop -f        # follow desktop.log (Electron app boot/backend)
    alvarez logs --level WARNING    # only WARNING+ lines
    alvarez logs --session abc123   # filter by session ID substring
    alvarez logs --component tools  # only tool-related lines
    alvarez logs --since 1h         # lines from the last hour
    alvarez logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_alvarez_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Alvarez runs — try 'alvarez chat' first)")
        sys.exit(1)
    ...
    if component:
        from alvarez_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_alvarez_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_alvarez_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_alvarez_home()}/logs/")
        return

    print(f"Log files in {display_alvarez_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'alvarez chat' to generate logs)")
