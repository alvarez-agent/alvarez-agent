"""
Configuration management for Alvarez Agent.

Config files are stored in ~/.alvarez/ for easy access:
- ~/.alvarez/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.alvarez/.env         - API keys and secrets

This module provides:
- alvarez config          - Show current configuration
- alvarez config edit     - Open config in editor
- alvarez config set      - Set a specific value
- alvarez config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``alvarez config set`` (which rewrites
    ...
    ``config.yaml`` in place: alvarez never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.alvarez/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``alvarez logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  alvarez config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Alvarez makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Alvarez itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``alvarez update``, the TUI build.
...
#   on every plugin install / ``alvarez update``.
...
# * ``ALVAREZ_HOME`` / ``ALVAREZ_PROFILE`` / ``ALVAREZ_CONFIG`` /
#   ``ALVAREZ_ENV`` — Alvarez runtime location flags. Writing these into
...
#
# IMPORTANT: ``ALVAREZ_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ALVAREZ_LANGFUSE_PUBLIC_KEY,
# ALVAREZ_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Alvarez runtime location — never via dashboard env writer.
    # NOT a ALVAREZ_* blanket: integration credentials (ALVAREZ_GEMINI_*,
    # ALVAREZ_LANGFUSE_*, ALVAREZ_SPOTIFY_*, ...) ARE allowed.
    "ALVAREZ_HOME", "ALVAREZ_PROFILE", "ALVAREZ_CONFIG", "ALVAREZ_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Alvarez runtime location "
            "(ALVAREZ_HOME, ALVAREZ_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.alvarez/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ALVAREZ_TOOL_PROGRESS", "ALVAREZ_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `alvarez plugins enable
    # observability/langfuse` or `alvarez tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ALVAREZ_LANGFUSE_ENV",
    "ALVAREZ_LANGFUSE_RELEASE",
    "ALVAREZ_LANGFUSE_SAMPLE_RATE",
    "ALVAREZ_LANGFUSE_MAX_CHARS",
    "ALVAREZ_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from alvarez_cli.colors import Colors, color
from alvarez_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ALVAREZ_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_alvarez_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Alvarez is running in package-manager-managed mode.

    Two signals: the ALVAREZ_MANAGED env var (set by the systemd service),
    or a .managed marker file in ALVAREZ_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade alvarez-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``alvarez_cli/`` — i.e. the git checkout for source
    installs, ``/opt/alvarez`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ALVAREZ_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Alvarez was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ALVAREZ_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ALVAREZ_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.alvarez``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ALVAREZ_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.alvarez:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ALVAREZ_HOME``, a
    ...
    ``docker`` and ``alvarez update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the alvarez-agent Docker image bakes a ``docker``
        stamp into ``/opt/alvarez`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ALVAREZ_HOME.
    try:
    ...
    #    host install whose shared $ALVAREZ_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``alvarez update``.
    try:
        method = (
            (get_alvarez_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``alvarez_constants.is_container`` (import-safe)."""
    try:
        from alvarez_constants import is_container
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ALVAREZ_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/alvarez`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Alvarez lives in a ``uv tool`` layout.

    ``uv tool install alvarez-agent`` places the install at
    ``.../uv/tools/alvarez-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``alvarez-agent`` happens to be uv-tool-installed on the machine while
    the *active* Alvarez is a regular pip/venv install, causing
    ``alvarez update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/alvarez-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade alvarez-agent"
    if method == "docker":
        return "docker pull <your-alvarez-agent-image>:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade alvarez-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade alvarez-agent"
        return "pip install --upgrade alvarez-agent"
    return "alvarez update"
...
# Long-form text for ``alvarez update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# alvarez_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Alvarez, it doesn't update
...
✗ ``alvarez update`` doesn't apply inside the Docker container.

Alvarez Agent runs as a Docker image, not a git checkout — the container has
no working tree to pull into.  Update by pulling (or rebuilding) a fresh
image and restarting your container instead:

  docker pull <your-alvarez-agent-image>:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate alvarez-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm <your-alvarez-agent-image>:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.
  • Your config and session history live under ``$ALVAREZ_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``alvarez update`` inside Docker.
...
def format_managed_message(action: str = "modify this Alvarez installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ALVAREZ_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Alvarez installation is managed by NixOS "
            f"(ALVAREZ_MANAGED={env_hint}).\n"
            "Edit services.alvarez-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Alvarez installation is managed by Homebrew "
            f"(ALVAREZ_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade alvarez-agent"
        )

    return (
        f"Cannot {action}: this Alvarez installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Alvarez."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ALVAREZ_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, alvarez_bin
    or None if container mode is not active, we're already inside the
    container, or ALVAREZ_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ALVAREZ_DEV") == "1":
        return None
    ...
    container_mode_file = get_alvarez_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "alvarez-agent")
    exec_user = info.get("exec_user", "alvarez")
    alvarez_bin = info.get("alvarez_bin", "/data/current-package/bin/alvarez")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "alvarez_bin": alvarez_bin,
    }
...
# Re-export from alvarez_constants — canonical definition lives there.
from alvarez_constants import get_alvarez_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_alvarez_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_alvarez_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_alvarez_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ALVAREZ_UID / ALVAREZ_GID env vars set by Docker deployments.

    Docker containers running Alvarez commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ALVAREZ_HOME once, but
    subdirectories created at runtime by ``ensure_alvarez_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ALVAREZ_UID", "").strip()
    gid_str = os.environ.get("ALVAREZ_GID", "").strip()
    try:
...
def _chown_to_alvarez_uid(path) -> None:
    """Chown ``path`` to ``ALVAREZ_UID:ALVAREZ_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_alvarez_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_alvarez_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the alvarez group can
    share state with the gateway service.

    The mode can be overridden via the ALVAREZ_HOME_MODE environment variable
    (e.g. ALVAREZ_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ALVAREZ_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ALVAREZ_UID``/``ALVAREZ_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ALVAREZ_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_alvarez_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Alvarez runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ALVAREZ_CONTAINER") or os.environ.get("ALVAREZ_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ALVAREZ_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ALVAREZ_HOME, upgrading legacy empty templates.
...
def ensure_alvarez_home():
    """Ensure ~/.alvarez directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_alvarez_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_alvarez_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ALVAREZ_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Alvarez-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Alvarez
        ...
        # identity slot (SOUL.md). Empty by default. The ALVAREZ_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Alvarez adds a coding operating brief
    ...
    "terminal": {
        ...
        #             ALVAREZ_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ALVAREZ_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Alvarez auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Alvarez sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Alvarez
        ...
        #  "/home/user/.alvarez/cache/documents:/output"]
        ...
        # bundled Alvarez image, which drops to the `alvarez` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Alvarez sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Alvarez should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``alvarez chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.alvarez/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: alvarez sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .alvarez.md, or .cursorrules before Alvarez applies
    ...
    # single read_file page exceeds these limits, Alvarez truncates the
    ...
    # See tools/kanban_tools.py and alvarez_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``alvarez dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``alvarez kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``alvarez kanban decompose`` and the kanban
        ...
        # ``alvarez profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `alvarez model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `alvarez` (and `alvarez chat`) launches by default:
        ...
        # REPL and `--tui` (or ALVAREZ_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `alvarez --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `alvarez -c` muscle memory.  Default off so existing
        # users aren't surprised.  ALVAREZ_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `alvarez --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/alvarez`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `alvarez pets`. Disabled until
        ...
        "pet": {
            ...
            # get_alvarez_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # (``ALVAREZ_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ALVAREZ_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ALVAREZ_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/alvarez``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Alvarez appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.alvarez/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.alvarez/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.alvarez/cache/delegation/
    ...
    # response. If not, Alvarez feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Alvarez auto-pauses the goal and
    ...
    # always goes to ~/.alvarez/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ALVAREZ_SKILL_DIR} and ${ALVAREZ_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `alvarez curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `alvarez curator run --consolidate` overrides
        ...
        # normally restored on every `alvarez update`, so pruning them only
        ...
        # skipped), snapshot ~/.alvarez/skills/ into
        # ~/.alvarez/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `alvarez curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for alvarez-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Alvarez Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ALVAREZ_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Alvarez' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.alvarez/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ALVAREZ_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `alvarez doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``alvarez_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Alvarez to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ALVAREZ_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # sets these at provision time.
        "chronos": {
            # NAS / scheduler base URL the agent calls to arm/cancel one-shots
            # and that mints the inbound fire JWT (used as the expected issuer).
            # Empty → Chronos is unavailable and the resolver falls back to the
            # built-in ticker.
            "portal_url": "",
        ...
        # Also overridable via ALVAREZ_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `alvarez -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `alvarez` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `alvarez kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     alvarez-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Alvarez tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.alvarez/logs/.
    ...
    # Remotely-hosted model catalog manifest.  When enabled and a URL is
    # configured, the CLI fetches curated model lists from it, falling back
    # to the in-repo snapshot on network failure.  Lets an operator update
    # model picker lists without shipping a alvarez-agent release.
    # No hosted catalog is configured by default — set ``url`` to self-host
    # one using the schema in alvarez_cli/model_catalog.py.
    "model_catalog": {
        "enabled": True,
        "url": "",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `alvarez model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal ALVAREZ_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the ALVAREZ_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.alvarez/.env,
        ...
        # behavior: files must live under the Alvarez cache, under
        ...
        # user. Bridged to ALVAREZ_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Alvarez
        # cache (~/.alvarez/cache/{documents,images,audio,video,screenshots})
        ...
        # to ALVAREZ_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ALVAREZ_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ALVAREZ_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.alvarez/state.db.
    ...
    "sessions": {
        ...
        # default of ``alvarez sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.alvarez/sessions/session_{sid}.json`` on every turn
    ...
    # ``alvarez update`` behaviour.
    "updates": {
        # Run a full ``alvarez backup``-style zip of ALVAREZ_HOME before every
        # ``alvarez update``.  Backups land in ``<ALVAREZ_HOME>/backups/`` and
        # can be restored with ``alvarez import <path>``.  Off by default:
        # zipping a large ALVAREZ_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``alvarez update
        ...
        "backup_keep": 5,
        # What `alvarez update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ALVAREZ_HOME>/lsp/bin/`` on first use.
    ...
    # `alvarez tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.alvarez/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.alvarez/.env (or your shell) and never in config.yaml.
            ...
            # ~/.alvarez/bin/ on first use.  When False you must install
            ...
            # `alvarez secrets bitwarden setup`.
            "server_url": "",
        },
    },
    ...
    "computer_use": {
        ...
        # by default upstream. Alvarez disables it for our users unless they
        # explicitly opt in here. When false (default), Alvarez sets
    ...
    # Alvarez Desktop (Electron app) launch options. These only affect
    # `alvarez desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ALVAREZ_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'alvarez model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Alvarez auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Alvarez, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @alvarez:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ALVAREZ_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #alvarez)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: alvarez-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'alvarez-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Alvarez API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Alvarez API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Alvarez API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ALVAREZ_TOOL_PROGRESS and ALVAREZ_TOOL_PROGRESS_MODE are deprecated —
    ...
    "ALVAREZ_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.alvarez.config`` entries, then checks
    ...
    try:
        ...
        # should never break `alvarez update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Alvarez versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``alvarez_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'alvarez setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Alvarez won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'alvarez doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ALVAREZ_HOME", "~/.alvarez")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "alvarez update / alvarez -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ALVAREZ_TOOL_PROGRESS")
            old_mode = get_env_value("ALVAREZ_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ALVAREZ_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ALVAREZ_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ALVAREZ_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ALVAREZ_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ALVAREZ_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_alvarez_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `alvarez plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `alvarez curator status` has no
    ...
    #   3. Creates `~/.alvarez/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_alvarez_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_alvarez_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `alvarez config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `alvarez config set`"
                    )
    ...
    # behavior set it to true explicitly via `alvarez config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from alvarez_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from alvarez_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: alvarez config set <key> <value>")
    ...
    # `alvarez update`, but only the version bump is persisted.
    ...
    # metadata.alvarez.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``alvarez config set``
    ...
    backends) — while display paths (``alvarez status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _alvarez_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.alvarez/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.alvarez/config.yaml.
    ...
    (which change ``ALVAREZ_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/alvarez/config.yaml invalidates the
...
#   openai-codex (OAuth — alvarez auth) — OpenAI Codex
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.alvarez/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Alvarez writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.alvarez/.env.
    ...
    interactive menu render (`alvarez tools`, `alvarez setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `alvarez tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Alvarez env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.alvarez/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.alvarez/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.alvarez/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.alvarez/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Alvarez — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Alvarez vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.alvarez/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.alvarez/.env`` over ``os.environ``.

    Used for Alvarez-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Alvarez Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from alvarez_cli.auth import get_anthropic_key
    ...
    # Warn on stale ALVAREZ_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ALVAREZ_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ALVAREZ_MAX_ITERATIONS={_env_ghost} "
                f"(run 'alvarez doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  alvarez config edit     # Edit config file", Colors.DIM))
    print(color("  alvarez config set <key> <value>", Colors.DIM))
    print(color("  alvarez setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `alvarez config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `alvarez config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: alvarez config set <key> <value>")
            print()
            print("Examples:")
            print("  alvarez config set model anthropic/claude-sonnet-4")
            print("  alvarez config set terminal.backend docker")
            print("  alvarez config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'alvarez config migrate' to add them")
        ...
        print("  alvarez config           Show current configuration")
        print("  alvarez config edit      Open config in editor")
        print("  alvarez config set <key> <value>   Set a config value")
        print("  alvarez config check     Check for missing/outdated config")
        print("  alvarez config migrate   Update config with new options")
        print("  alvarez config path      Show config file path")
        print("  alvarez config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``alvarez config`` UI so users
