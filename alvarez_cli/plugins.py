"""
Alvarez Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with alvarez-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.alvarez/plugins/<name>/``
3. **Project plugins** – ``./.alvarez/plugins/<name>/`` (opt-in via
   ``ALVAREZ_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``alvarez_agent.plugins``
   entry-point group.
...
from alvarez_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``ALVAREZ_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ALVAREZ_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ALVAREZ_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.alvarez/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ALVAREZ_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ALVAREZ_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Alvarez loggers.
    """
    ...
    logger.debug(
        "ALVAREZ_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Alvarez' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by alvarez_cli.kanban_db when a task
    ...
    # separate `alvarez -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `alvarez kanban dispatch`),
...
ENTRY_POINTS_GROUP = "alvarez_agent.plugins"

_NS_PARENT = "alvarez_plugins"
...
class PluginManifest:
    ...
    #              in ~/.alvarez/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``alvarez plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Alvarez profile name (e.g. ``"default"``).

        Derived from ``ALVAREZ_HOME`` via
        :func:`alvarez_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.alvarez/profiles/<name>``, or ``"custom"`` when
        ``ALVAREZ_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Alvarez core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``alvarez honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``alvarez <subcommand>``
    ...
    def register_slack_action_handler(
        ...
        Alvarez' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``alvarez model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: alvarez_cli.main imports plugins indirectly
        from alvarez_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.alvarez/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        # Safe mode (--safe-mode / ALVAREZ_SAFE_MODE=1): troubleshooting run
        ...
        if env_var_enabled("ALVAREZ_SAFE_MODE"):
            logger.info("ALVAREZ_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to alvarez_cli/. Two layouts are
        ...
        # 2. User plugins (~/.alvarez/plugins/)
        user_dir = get_alvarez_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.alvarez/plugins/)
        if _env_enabled("ALVAREZ_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".alvarez" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ALVAREZ_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with alvarez and must
            ...
            # all ~20 of them added several seconds to every `alvarez`
            # invocation — including plain `alvarez chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Alvarez ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `alvarez plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``alvarez plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`alvarez plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `alvarez plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``alvarez_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``alvarez_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="alvarez-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``alvarez tools`` TUI so plugin-provided toolsets appear
