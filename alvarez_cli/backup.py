"""
Backup and import commands for alvarez CLI.

`alvarez backup` creates a zip archive of the entire ~/.alvarez/ directory
(excluding the alvarez-agent repo and transient files).

`alvarez import` restores from a backup zip, overlaying onto the current
ALVAREZ_HOME root.
"""
...
from alvarez_constants import get_default_alvarez_root, get_alvarez_home, display_alvarez_home
...
# ``alvarez-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/alvarez-agent/``
...
# ALVAREZ_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "alvarez-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ALVAREZ_HOME) —
...
# File names that ``alvarez import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking" (NS-508 / the second half of NS-501).
...
# Reserved archive subtree for provider state that lives OUTSIDE ALVAREZ_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ALVAREZ_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to alvarez root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``alvarez-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/alvarez-agent/`` — must be preserved.
        if part == "alvarez-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ALVAREZ_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Alvarez home directory."""
    alvarez_root = get_default_alvarez_root()

    if not alvarez_root.is_dir():
        print(f"Error: Alvarez home directory not found at {alvarez_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"alvarez-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"alvarez-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_alvarez_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(alvarez_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(alvarez_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``alvarez-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "alvarez-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(alvarez_root)
    ...
    # outside ALVAREZ_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_alvarez_home()}."
        )
    ...
    print(f"\nRestore with: alvarez import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Alvarez backup.
    ...
    # Look for telltale files that a alvarez home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Alvarez backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.alvarez/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a alvarez dir name
        if prefix in {".alvarez", "alvarez"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_alvarez_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (alvarez_root / "config.yaml").exists()
        has_env = (alvarez_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Alvarez configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ALVAREZ_HOME.
            ...
            # reconciler on the target. Matched by basename so both the root
            # profile and named profiles (profiles/<name>/gateway_state.json)
            # are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = alvarez_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(alvarez_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_alvarez_home()}."
            )
        ...
        profiles_dir = alvarez_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # alvarez_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: alvarez profile list  (after installing alvarez)")

        # Guidance
        print()
        if not (alvarez_root / "alvarez-agent").is_dir():
            print("Note: The alvarez-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: alvarez update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  alvarez -p {pname} gateway install")

        print("Done. Your Alvarez configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and alvarez backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ALVAREZ_HOME).
...
# — `alvarez update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ALVAREZ_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ALVAREZ_HOME and the entry is silently
...
def _quick_snapshot_root(alvarez_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(alvarez_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ALVAREZ_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``alvarez update``.
    ...
    Args:
        ...
        alvarez_home: Override for the Alvarez home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(alvarez_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for alvarez backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_alvarez_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, alvarez_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``alvarez_root`` to ``out_path``.
...
def _pre_update_backup_dir(alvarez_home: Optional[Path] = None) -> Path:
    home = alvarez_home or get_alvarez_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ALVAREZ_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ALVAREZ_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``alvarez update``) should continue even if the backup fails.
    """
    alvarez_root = alvarez_home or get_default_alvarez_root()
    if not alvarez_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(alvarez_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, alvarez_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `alvarez claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ALVAREZ_HOME under ``backups/`` before a
    ``alvarez claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``alvarez import <archive>``.  Writes to
    ``<ALVAREZ_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `alvarez import` and the
