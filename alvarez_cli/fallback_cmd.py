"""
alvarez fallback — manage the fallback provider chain.

Fallback providers are tried in order when the primary model fails with
rate-limit, overload, or connection errors. See:

Subcommands:
  alvarez fallback [list]   Show the current fallback chain (default when no subcommand)
  alvarez fallback add      Pick provider + model via the same picker as `alvarez model`,
                           then append the selection to the chain
  alvarez fallback remove   Pick an entry to delete from the chain
  alvarez fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.alvarez/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from alvarez_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `alvarez model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  alvarez fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `alvarez model`, then append the selection to the chain."""
    from alvarez_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `alvarez model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `alvarez fallback list` to view, or `alvarez fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``alvarez fallback [subcommand]``."""
