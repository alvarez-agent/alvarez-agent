"""
Unified self-relaunch for Alvarez CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``alvarez sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``alvarez`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from alvarez_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched alvarez."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_alvarez_bin() -> Optional[str]:
    """Find the alvarez entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("alvarez")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m alvarez_cli.main``.
    ...
    ``alvarez.exe`` on PATH or the ``sys.executable -m alvarez_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with alvarez.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_alvarez_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "alvarez_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh alvarez invocation.
    ...
    relaunch contract wants: "run alvarez again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``alvarez.exe`` (a Python console-script shim that wraps
    ``python -m alvarez_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "alvarez exited, then
    new alvarez started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``alvarez`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nAlvarez relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run alvarez.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
