"""MCP picker — interactive `alvarez mcp picker` (also the default `alvarez mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``alvarez mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `alvarez plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from alvarez_cli.cli_output import prompt_yes_no
from alvarez_cli.curses_ui import curses_single_select
from alvarez_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Alvarez session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from alvarez_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `alvarez mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: alvarez mcp install <name>    Picker: alvarez mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Alvarez is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Alvarez — run `alvarez update` "
...
def show_catalog() -> None:
    """`alvarez mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`alvarez mcp picker` (and default `alvarez mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`alvarez mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `alvarez mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
