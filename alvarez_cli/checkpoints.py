"""`alvarez checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.alvarez/checkpoints/``.  Actions:

    alvarez checkpoints               # same as `status`
    alvarez checkpoints status        # total size, project count, breakdown
    alvarez checkpoints list          # per-project checkpoint counts + workdir
    alvarez checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    alvarez checkpoints clear [-f]    # nuke the entire base (asks first)
    alvarez checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    alvarez checkpoints
    alvarez checkpoints prune --retention-days 3 --max-size-mb 200
    alvarez checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: alvarez checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``alvarez checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `alvarez checkpoints` → status
