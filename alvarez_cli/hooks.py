"""alvarez hooks — inspect and manage shell-script hooks.

Usage::

    alvarez hooks list
    alvarez hooks test <event> [--for-tool X] [--payload-file F]
    alvarez hooks revoke <command>
    alvarez hooks doctor

Consent records live under ``~/.alvarez/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.alvarez/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``alvarez hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: alvarez hooks {list|test|revoke|doctor}")
        print("Run 'alvarez hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.alvarez/config.yaml.")
        print("See `alvarez hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `alvarez hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `alvarez hooks test` and `alvarez hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Alvarez wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `alvarez hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `alvarez hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `alvarez hooks doctor`.")
