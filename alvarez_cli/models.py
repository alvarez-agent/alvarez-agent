...
Add, remove, or reorder entries here — both `alvarez setup` and
`alvarez` provider-selection will pick up the change automatically.
"""
...
from alvarez_cli import __version__ as _ALVAREZ_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ALVAREZ_USER_AGENT = f"alvarez-cli/{_ALVAREZ_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `alvarez model`
    flow without maintaining a separate static list.
    """
    from alvarez_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ALVAREZ_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Alvarez-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ALVAREZ_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Alvarez refreshes the cache from
...
# this list:  alvarez model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `alvarez model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `alvarez model` TUI
...
#
# Some vendors expose several Alvarez provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `alvarez model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``alvarez model``, the
...
# (``get_default_model_for_provider``) for providers whose curated lists lead
# with an expensive flagship. Currently empty.
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``alvarez auth add openai-codex`` without
    ``alvarez model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``alvarez model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    alvarez-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from alvarez_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — alvarez-agent
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (e.g. OpenRouter).
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``alvarez model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from alvarez_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex etc. support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from alvarez_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model openrouter` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Alvarez' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``alvarez_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Alvarez should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``alvarez auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.alvarez/.env``.
    ...
    """
    try:
        from alvarez_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from alvarez_cli.auth import read_credential_pool
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Alvarez release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Alvarez release). Falls back to the hardcoded catalog if no token
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``alvarez model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $ALVAREZ_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_alvarez_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ALVAREZ_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ALVAREZ_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Alvarez' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ALVAREZ_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from alvarez_constants import get_alvarez_home
    return get_alvarez_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `alvarez moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Alvarez will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Alvarez cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Alvarez actually ships.  (#46850)
