...
on Japanese locales, etc.).  Alvarez's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``alvarez_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Alvarez is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ALVAREZ_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ALVAREZ_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Alvarez-managed Git install directories so
    ...
    # already-running shells don't see the change — which means alvarez
    ...
    # install without Alvarez-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Alvarez's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Alvarez's
    ``alvarez config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Alvarez (User env var in Windows Settings, or
    export in a PowerShell profile) and Alvarez picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Alvarez-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\alvarez\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``alvarez`` from right after install) retain their old PATH.

    Any subprocess Alvarez spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Alvarez-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "alvarez", "git", "cmd"),
        os.path.join(local_appdata, "alvarez", "git", "bin"),
        os.path.join(local_appdata, "alvarez", "git", "usr", "bin"),
        # Alvarez venv Scripts directory — host of the alvarez.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes alvarez, but harmless to include.
        os.path.join(local_appdata, "alvarez", "alvarez-agent", "venv", "Scripts"),
