"""
Cron subcommand for alvarez CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(alvarez\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*alvarez)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*alvarez)"
    r"|(p?kill\s+.*alvarez.*gateway)"
)
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: alvarez gateway install", Colors.DIM))
    print(color("                    sudo alvarez gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  alvarez cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'alvarez cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: alvarez gateway restart")
        ...
        print("    alvarez gateway install    # Install as a user service")
        print("    sudo alvarez gateway install --system  # Linux servers: boot-time system service")
        print("    alvarez gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `alvarez gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: alvarez cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
