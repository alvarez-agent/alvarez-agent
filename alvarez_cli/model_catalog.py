...
An operator-configured URL can host a JSON manifest of curated models for
providers we want to update without shipping a release. This module fetches,
validates, and caches that manifest, falling back to the in-repo hardcoded
lists when the network is unavailable or no URL is configured.

Pipeline
--------
1. ``get_catalog()`` — returns a parsed manifest dict.
   - Checks in-process cache (invalidated by TTL).
   - Reads disk cache at ``~/.alvarez/cache/model_catalog.json``.
...
2. ``get_curated_openrouter_models()`` —
   thin accessor returning the shape existing callers expect. Falls
   back to the in-repo hardcoded list on any lookup failure.
...
# No hosted catalog is configured by default — operators can self-host a
# manifest and point ``model_catalog.url`` at it. Empty → no remote fetch,
# the in-repo hardcoded lists are used.
DEFAULT_CATALOG_URL = ""
# Fallback fetch chain tried when the primary URL fails.
...
def _cache_path() -> Path:
    ...
    return get_alvarez_home() / "cache" / "model_catalog.json"
...
def _fetch_manifest_with_fallback(
    ...
    operator who configured the catalog URL to point at a mirror of a
    fallback doesn't double-fetch.
    """
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    # No URL configured → no remote catalog; skip straight to the fallbacks.
    fetched = (
        _fetch_manifest_with_fallback(cfg["url"], DEFAULT_FETCH_TIMEOUT)
        if cfg["url"]
        else None
    )
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``alvarez update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.alvarez/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``alvarez model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
