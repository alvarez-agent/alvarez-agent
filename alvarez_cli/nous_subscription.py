...
# coverage category (alvarez_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `alvarez tools` picker scope its entitlement gate to the selected backend, so a
...
class NousSubscriptionFeatures:
    ...
    account_info: Optional[object] = None  # always None since the portal was removed
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("ALVAREZ_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Nous Portal removed (hermes separation, 2026-07-03) — no account, so all
    # managed/entitlement flags below resolve False and only local BYO-key
    # feature detection remains.
    account_info = None
    ...
    # Tool Gateway via `alvarez model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("ALVAREZ_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("ALVAREZ_LOCAL_STT_COMMAND"))
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    """
    # Portal removed — no entitlement is ever present.
    return [], [], []
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from alvarez_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`alvarez tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    """Managed tool gateway removed — access can never be granted."""
    print(f"  {capability} is no longer available; configure a direct API key instead.")
    return False
