...
Pure logic: walks a Alvarez config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `alvarez doctor` and a future `alvarez migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Alvarez config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Alvarez config and return retirement issues.
