"""Shell completion script generation for alvarez CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_alvarez_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Alvarez Agent bash completion
# Add to ~/.bashrc:
#   eval "$(alvarez completion bash)"

_alvarez_profiles() {{
    local profiles_dir="$HOME/.alvarez/profiles"
...
_alvarez_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_alvarez_profiles)" -- "$cur"))
        return
    fi
...
complete -F _alvarez_completion alvarez
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef alvarez
# Alvarez Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(alvarez completion zsh)"

_alvarez_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.alvarez/profiles" ]]; then
        profiles+=($HOME/.alvarez/profiles/*(N/:t))
    fi
...
_alvarez() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_alvarez_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'alvarez command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Alvarez Agent fish completion",
        "# Add to your config:",
        "#   alvarez completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __alvarez_profiles",
        "    echo default",
        "    if test -d $HOME/.alvarez/profiles",
        "        for d in $HOME/.alvarez/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c alvarez -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c alvarez -f -s p -l profile"
        " -d 'Profile name' -xa '(__alvarez_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__alvarez_profiles)' -d 'Profile name'"
                )
