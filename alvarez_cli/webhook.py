"""alvarez webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    alvarez webhook subscribe <name> [options]
    alvarez webhook list
    alvarez webhook remove <name>
    alvarez webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.alvarez/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _alvarez_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: alvarez gateway run
"""
...
def webhook_command(args):
    """Entry point for 'alvarez webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: alvarez webhook {subscribe|list|remove|test}")
        print("Run 'alvarez webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (alvarez gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: alvarez webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from alvarez webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (alvarez gateway run)")
