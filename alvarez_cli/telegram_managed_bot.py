...
manual BotFather token copy-paste. Alvarez talks only to the Nous onboarding
...
# No hosted onboarding service exists for this fork — set
# TELEGRAM_ONBOARDING_URL to a self-hosted endpoint to enable managed setup.
DEFAULT_API_URL = ""
...
DEFAULT_MANAGER_BOT = "AlvarezSetupBot"

DEFAULT_BOT_NAME = "Alvarez Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``alvarez_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``alvarez_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"alvarez_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    if not pairing:
        print("  ✗ Could not reach the Alvarez Telegram onboarding service.")
