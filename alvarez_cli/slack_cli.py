"""``alvarez slack ...`` CLI subcommands.

Today only ``alvarez slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ alvarez slack manifest > slack-manifest.json
    # or:
    $ alvarez slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Alvarez. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Alvarez deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from alvarez_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Alvarez in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Alvarez agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``alvarez_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ALVAREZ_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Alvarez")
    ...
    """
    name = getattr(args, "name", None) or "Alvarez"
    description = getattr(args, "description", None) or "Your Alvarez agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(get_alvarez_home()) / "slack-manifest.json"
            except Exception:
                target = Path(os.environ.get("ALVAREZ_HOME") or str(Path.home() / ".alvarez")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Alvarez app\n"
