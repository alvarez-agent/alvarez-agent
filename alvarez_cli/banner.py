...
Pure display functions with no AlvarezCLI state dependency.
"""
...
from alvarez_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _fetch_pypi_latest(package: str = "alvarez-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Alvarez update is available.

    Two paths: if ``ALVAREZ_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = alvarez_home / ".update_check"
    embedded_rev = os.environ.get("ALVAREZ_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ALVAREZ_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `alvarez update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/alvarez/update/check`
    ...
    if embedded_rev:
        ...
        # $ALVAREZ_HOME/alvarez-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = alvarez_home / "alvarez-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Alvarez git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ALVAREZ_HOME/alvarez-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``alvarez_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from alvarez_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"Alvarez Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        # ponytail: hero art is opt-in per skin now; no default graphic.
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        provider_str = f" [dim {dim}]·[/] [dim {dim}]{provider}[/]" if (provider or "").strip() else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str}{provider_str}")

    if os.getenv("ALVAREZ_YOLO_MODE"):
    ...
    try:
        from alvarez_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from alvarez_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built alvarez; we know an update
    ...
    # Pip-install warning — `pip install alvarez-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ALVAREZ_AGENT_LOGO
        console.print(_logo)
        console.print()
