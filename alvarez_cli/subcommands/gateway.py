"""``alvarez gateway`` and ``alvarez proxy`` subcommand parsers.

Extracted verbatim from ``alvarez_cli/main.py:main()`` (god-file Phase 2).
...
from alvarez_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ALVAREZ_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy alvarez.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Alvarez gateway unit files "
            "(e.g. alvarez.service) left over from older installs. Profile "
            "units (alvarez-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.alvarez/.env. "
            "Requires being logged in (alvarez setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.alvarez/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. xAI Grok OAuth). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="xai",
        help="Upstream provider (default: xai). See `alvarez proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
