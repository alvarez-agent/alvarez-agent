"""``alvarez debug`` debug tools for Alvarez Agent.

Currently supports:
    alvarez debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.alvarez/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[alvarez debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.alvarez/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``alvarez debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``alvarez debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Alvarez version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `alvarez debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "alvarez-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``alvarez debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.alvarez/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``alvarez debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "alvarez-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----AlvarezDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from alvarez_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_alvarez_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``alvarez dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``alvarez dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by the paste.rs and --local paths)
# ---------------------------------------------------------------------------
...
def collect_share_bundle(
    ...
    This is the single source of collection + redaction shared by the
    paste.rs path (:func:`build_debug_share`) and the ``--local`` path —
    redaction is the safety boundary, so both see identical, force-redacted
    snapshots.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``alvarez debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "alvarez debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `alvarez debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  alvarez debug delete <url>")

    print(f"\nShare these links with the Alvarez team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: alvarez debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'alvarez debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``alvarez debug`` call.
    ...
    # best-effort — any failure is swallowed so ``alvarez debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: alvarez debug <command>")
        print()
        print("Commands:")
