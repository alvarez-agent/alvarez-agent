...
- ``alvarez_cli.providers``        -- canonical provider identity + overlays
- ``alvarez_cli.model_normalize``  -- per-provider name formatting
...
_ALVAREZ_MODEL_WARNING = (
    "Nous Research Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with Alvarez Agent. They lack the tool-calling capabilities "
...
_NOUS_ALVAREZ_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_alvarez_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_ALVAREZ_NON_AGENTIC_RE.search(model_name))


def _check_alvarez_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_alvarez_non_agentic(model_name):
        return _ALVAREZ_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``alvarez config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from alvarez_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'alvarez model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'alvarez doctor' — config issues detected:"
        ...
        if target_provider == "moa" and not new_model:
            try:
                from alvarez_cli.config import load_config
                from alvarez_cli.moa_config import normalize_moa_config
        ...
        from alvarez_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from alvarez_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from alvarez_cli.config import load_config
            from alvarez_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from alvarez_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in alvarez_cli.runtime_provider.resolve_runtime_provider;
    ...
    alvarez_warn = _check_alvarez_model_warning(new_model)
    if alvarez_warn:
        warnings.append(alvarez_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from alvarez_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from alvarez_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from alvarez_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by alvarez provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from alvarez_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from alvarez_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Alvarez-mapped providers ---
    from alvarez_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from alvarez_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for alvarez_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(alvarez_id)
        if (
            _alias_target
            and _alias_target != alvarez_id
        ...
        pconfig = PROVIDER_REGISTRY.get(alvarez_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ALVAREZ_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from alvarez_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(alvarez_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `alvarez model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(alvarez_id)
        if not model_ids:
            model_ids = curated.get(alvarez_id, [])
            if alvarez_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(alvarez_id, model_ids)
        total = len(model_ids)
        if alvarez_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Alvarez-only providers (openai-codex, copilot, opencode-go) ---
    from alvarez_cli.providers import ALVAREZ_OVERLAYS
    from alvarez_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Alvarez provider ID.
    # ALVAREZ_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Alvarez IDs ("copilot").
    _mdev_to_alvarez = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ALVAREZ_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Alvarez slug — e.g. "github-copilot" → "copilot"
        alvarez_slug = _mdev_to_alvarez.get(pid, pid)
        if alvarez_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(alvarez_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, alvarez_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or alvarez_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(alvarez_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", alvarez_slug, exc)
        ...
        # configured.
        if not has_creds and alvarez_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_alvarez_oauth_credentials,
                )
                alvarez_creds = read_alvarez_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (alvarez_creds and alvarez_creds.get("accessToken")) or \
        ...
        if alvarez_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(alvarez_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(alvarez_slug)
                model_ids = _ids if _ids else (curated.get(alvarez_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(alvarez_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(alvarez_slug, []) or curated.get(pid, [])
                if alvarez_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(alvarez_slug, model_ids)
        total = len(model_ids)
        if alvarez_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": alvarez_slug,
            "name": get_label(alvarez_slug),
            "is_current": alvarez_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "alvarez",
        ...
        seen_slugs.add(alvarez_slug.lower())
        _record_builtin_endpoint(alvarez_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ALVAREZ_OVERLAYS (keeps /model in sync
    # with `alvarez model`).
    try:
        from alvarez_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Alvarez's canonical write key (matches
            ...
            # Alvarez writes ``models:`` as a dict keyed by model id
            # (see alvarez_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from alvarez_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Alvarez's own writer uses; a hyphen variant is accepted
            ...
            # active model. Alvarez's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, alvarez_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from alvarez_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`alvarez_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from alvarez_cli.models import fetch_openrouter_models
