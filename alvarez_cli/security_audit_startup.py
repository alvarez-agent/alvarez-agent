...
3. Running inside a container with no persistent volume mount over the
   ALVAREZ_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("alvarez.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Alvarez as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("ALVAREZ_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(alvarez_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = alvarez_home or Path(
        os.environ.get("ALVAREZ_HOME", os.path.expanduser("~/.alvarez"))
    )
    try:
...
def run_security_audit(
    *, alvarez_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(alvarez_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(alvarez_home=alvarez_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
