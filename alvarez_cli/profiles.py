"""
Profile management for multiple isolated Alvarez instances.

Each profile is a fully independent ALVAREZ_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.alvarez/profiles/<name>/`` by default.

The "default" profile is ``~/.alvarez`` itself — backward compatible,
zero migration needed.

Usage::

    alvarez profile create coder          # fresh profile + bundled skills
    alvarez profile create coder --clone  # also copy config, .env, SOUL.md, skills
    alvarez profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    alvarez -p coder chat                 # or via flag
    alvarez profile use coder             # set as sticky default
    alvarez profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See alvarez_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.alvarez``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   alvarez-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `alvarez backup` archives
...
# Marker file written by `alvarez profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `alvarez
...
# `alvarez skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Alvarez infrastructure directories that only the default profile
         (``~/.alvarez``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_alvarez_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.alvarez) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "alvarez-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "alvarez", "default", "test", "tmp", "root", "sudo",
})

# Alvarez subcommands that cannot be used as profile names/aliases
_ALVAREZ_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the alvarez root, NOT to the current ALVAREZ_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ALVAREZ_HOME points outside
    ``~/.alvarez``, profiles live under ``ALVAREZ_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_alvarez_home() / "profiles"


def _get_default_alvarez_home() -> Path:
    """Return the default (pre-profile) ALVAREZ_HOME path.

    In standard deployments this is ``~/.alvarez``.
    In Docker/custom deployments where ALVAREZ_HOME is outside ``~/.alvarez``
    (e.g. ``/opt/data``), returns ALVAREZ_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_alvarez_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Alvarez installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ALVAREZ_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_alvarez_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, alvarez subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ALVAREZ_SUBCOMMANDS:
        return f"'{canon}' conflicts with a alvarez subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            alvarez_exe = shutil.which("alvarez") or "alvarez"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(alvarez_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Alvarez. If we
    ...
    try:
        ...
        token = set_alvarez_home_override(str(profile_dir))
        try:
        ...
        # not be migrated. The next `alvarez doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``alvarez -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``alvarez profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``alvarez -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``alvarez profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ALVAREZ_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Alvarez config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_alvarez_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, alvarez_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ALVAREZ_HOME.
    ...
    The returned ``alvarez_home`` is the path to pass to
    ``set_alvarez_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_alvarez_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``alvarez update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.alvarez)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.alvarez/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `alvarez -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Alvarez profile.\n"
    ...
    if not soul_path.exists():
        try:
            from alvarez_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `alvarez update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`alvarez profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Alvarez (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `alvarez profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `alvarez -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ALVAREZ_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``alvarez profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ALVAREZ_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_alvarez_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.alvarez).\n"
            "To remove everything, use: alvarez uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ALVAREZ_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from alvarez_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ALVAREZ_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ALVAREZ_HOME")
    try:
        os.environ["ALVAREZ_HOME"] = str(profile_dir)
        from alvarez_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ALVAREZ_HOME"] = old_home
        elif "ALVAREZ_HOME" in os.environ:
            del os.environ["ALVAREZ_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.alvarez/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: alvarez profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ALVAREZ_HOME.

    Returns ``"default"`` if ALVAREZ_HOME is not set or points to ``~/.alvarez``.
    Returns the profile name if ALVAREZ_HOME points into ``~/.alvarez/profiles/<name>``.
    Returns ``"custom"`` if ALVAREZ_HOME is set to an unrecognized path.
    """
    from alvarez_constants import get_alvarez_home
    alvarez_home = get_alvarez_home()
    resolved = alvarez_home.resolve()

    default_resolved = _get_default_alvarez_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.alvarez itself — its parent is ~/ and its
        # directory name is ".alvarez", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: alvarez profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.alvarez itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.alvarez). "
            "Specify a different name: alvarez profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="alvarez_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"alvarez_{old_name}"
    legacy_old_host = f"alvarez.{old_name}"
    new_host = f"alvarez_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_alvarez_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("alvarez_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ALVAREZ_HOME path string.

    Called early in the CLI entry point, before any alvarez modules
    are imported, to set the ALVAREZ_HOME environment variable.
    """
