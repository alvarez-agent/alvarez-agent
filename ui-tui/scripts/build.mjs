...
await build({
  ...
  // Skip the prebuilt @alvarez/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@alvarez/ink': resolve(root, 'packages/alvarez-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The alvarez_cli launcher
