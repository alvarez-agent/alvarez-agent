import type { RunExternalProcess } from '@alvarez/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`alvarez ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching alvarez: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`alvarez ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
