import { Box, Text, useStdout } from '@alvarez/ink'
...
const TAG_FULL = 'Alvarez · wire in, dive deep'
const TAG_MID = 'wire in, dive deep'
const TAG_TINY = 'Alvarez'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  // alvarez_cli/banner.py) and the "connected" label on the collapse toggle.
      ...
      {wide && (
        <Box flexDirection="column" marginRight={2} width={leftW}>
          ...
          <Text color={t.color.accent}>
            ...
            {info.provider ? <Text color={t.color.muted}> · {info.provider}</Text> : null}
          </Text>
      ...
      <Box flexDirection="column" width={w}>
        ...
        {typeof info.update_behind === 'number' && info.update_behind > 0 && (
          <Text bold color={t.color.warn}>
            ...
            <Text bold color={t.color.warn}>
              {info.update_command || 'alvarez update'}
            </Text>
            <Text bold={false} color={t.color.warn} dimColor>
              {' '}
              to update
