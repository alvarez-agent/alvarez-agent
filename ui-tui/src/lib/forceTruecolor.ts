...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ALVAREZ_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Alvarez on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ALVAREZ_TUI_TRUECOLOR=1.
