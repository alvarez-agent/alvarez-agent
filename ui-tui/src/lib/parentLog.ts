...
const logDir = join(process.env.ALVAREZ_HOME?.trim() || join(homedir(), '.alvarez'), 'logs')
...
// a real ~/.alvarez (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('alvarez-tui: parent lifecycle log unavailable\n')
    }
  }
}
