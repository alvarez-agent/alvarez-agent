...
React + Ink terminal UI for Alvarez. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ALVAREZ_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `alvarez setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.alvarez/.alvarez_history` or under `ALVAREZ_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `alvarez setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/alvarez-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external alvarez setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.alvarez/tui-parent.log
    ...
    types/
      alvarez-ink.d.ts            type declarations for @alvarez/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AlvarezCLI subprocess for slash commands
```