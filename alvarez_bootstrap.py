"""Windows UTF-8 bootstrap for Alvarez entry points.
...
should be imported at the very top of every Alvarez entry point
(``alvarez``, ``alvarez-agent``, ``alvarez-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Alvarez modules.

    Alvarez ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Alvarez source root
    ...
    """
    root = src_root or os.environ.get("ALVAREZ_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ALVAREZ_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ALVAREZ_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import alvarez_bootstrap``
# (or ``from alvarez_bootstrap import apply_windows_utf8_bootstrap``) at
