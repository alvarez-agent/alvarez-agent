#!/usr/bin/env bash
# GitHub environment detection helper for Alvarez Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _alvarez_env="${ALVAREZ_HOME:-$HOME/.alvarez}/.env"; [ -f "$_alvarez_env" ] && grep -q "^GITHUB_TOKEN=" "$_alvarez_env" 2>/dev/null; then
