...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _alvarez_env="${ALVAREZ_HOME:-$HOME/.alvarez}/.env"; [ -f "$_alvarez_env" ] && grep -q "^GITHUB_TOKEN=" "$_alvarez_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_alvarez_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Alvarez Agent\",
    \"comments\": [
...
```bash
source "${ALVAREZ_HOME:-$HOME/.alvarez}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Alvarez Agent. Code looks clean — good test coverage, no security concerns."
