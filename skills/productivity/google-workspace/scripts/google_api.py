#!/usr/bin/env python3
"""Google Workspace API CLI for Alvarez Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Alvarez-facing JSON contract and falls back to the Python client
...
from _alvarez_home import get_alvarez_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ALVAREZ_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Alvarez Agent")
