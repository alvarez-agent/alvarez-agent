...
metadata:
  alvarez:
    ...
    related_skills: [claude-code, alvarez-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Alvarez terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Alvarez itself, `model.provider: openai-codex` uses Alvarez-managed Codex
OAuth from `~/.alvarez/auth.json` after `alvarez auth add openai-codex`. For the
...
## Alvarez Gateway Caveat

When invoking the Codex CLI from a Alvarez gateway/service context (for example,
