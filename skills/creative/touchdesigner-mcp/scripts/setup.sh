...
ALVAREZ_HOME_DIR="${ALVAREZ_HOME:-$HOME/.alvarez}"
ALVAREZ_CFG="${ALVAREZ_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Alvarez config has twozero_td MCP entry ──
if [[ ! -f "$ALVAREZ_CFG" ]]; then
    echo -e " ${FAIL} Alvarez config not found at ${ALVAREZ_CFG}"
    manual_steps+=("Create ${ALVAREZ_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ALVAREZ_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Alvarez config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Alvarez config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ALVAREZ_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ALVAREZ_CFG} manually"); }
    manual_steps+=("Restart Alvarez session to pick up config change")
fi
