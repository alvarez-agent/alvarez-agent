...
## 5. Alvarez Configuration

### Config location

`$ALVAREZ_HOME/config.yaml` (defaults to `~/.alvarez/config.yaml` when `ALVAREZ_HOME` is unset)
...
Restart the Alvarez session for changes to take effect. The MCP connection is
