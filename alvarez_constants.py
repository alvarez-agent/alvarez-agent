"""Shared constants for Alvarez Agent.
...
_UNSET = object()
_ALVAREZ_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ALVAREZ_HOME_OVERRIDE", default=_UNSET
)


def set_alvarez_home_override(path: str | Path | None) -> Token:
    """Set a context-local Alvarez home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ALVAREZ_HOME_OVERRIDE.set(value)


def reset_alvarez_home_override(token: Token) -> None:
    """Restore the previous context-local Alvarez home override."""
    _ALVAREZ_HOME_OVERRIDE.reset(token)


def get_alvarez_home_override() -> str | None:
    """Return the active context-local Alvarez home override, if any."""
    override = _ALVAREZ_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_alvarez_home() -> Path:
    """Return the platform-native default Alvarez home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "alvarez"
    return Path.home() / ".alvarez"


def get_alvarez_home() -> Path:
    """Return the Alvarez home directory (default: platform-native path).

    Reads ALVAREZ_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``ALVAREZ_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``ALVAREZ_HOME`` explicitly (see the systemd
    template in ``alvarez_cli/gateway.py`` and the kanban dispatcher in
    ``alvarez_cli/kanban_db.py``).  See upstream hermes-agent issue #18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_alvarez_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[ALVAREZ_HOME fallback] ALVAREZ_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass ALVAREZ_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_alvarez_home()


def get_default_alvarez_root() -> Path:
    """Return the root Alvarez directory for profile-level operations.

    In standard deployments this is the platform-native Alvarez home
    (``~/.alvarez`` on POSIX, ``%LOCALAPPDATA%\\alvarez`` on native Windows).

    In Docker or custom deployments where ``ALVAREZ_HOME`` points outside
    ``~/.alvarez`` (e.g. ``/opt/data``), returns ``ALVAREZ_HOME`` directly
    — that IS the root.

    In profile mode where ``ALVAREZ_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.alvarez/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_alvarez_home()
    env_home = os.environ.get("ALVAREZ_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # ALVAREZ_HOME is under ~/.alvarez (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ALVAREZ_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Alvarez is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ALVAREZ_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ALVAREZ_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_alvarez_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (curated
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``ALVAREZ_OPTIONAL_MCPS``.
    """
    override = os.getenv("ALVAREZ_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_alvarez_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ALVAREZ_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<ALVAREZ_HOME>/skills`` last-resort
    """
    override = os.getenv("ALVAREZ_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_alvarez_home() / "skills"


def get_alvarez_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Alvarez subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to ALVAREZ_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ALVAREZ_HOME (e.g. ``"image_cache"``).
...
def iter_alvarez_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Alvarez-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\alvarez\\node``. POSIX installs use
    ``$ALVAREZ_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_alvarez_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with alvarezManagedNodePathEntries() in
...
_ALVAREZ_NODE_TARGET_MAJOR = int(os.environ.get("ALVAREZ_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Alvarez-managed Node trees live under ``$ALVAREZ_HOME/node`` (or a profile's
    ``ALVAREZ_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_alvarez_node_executable``
    used to trust file presence alone, so ``alvarez update`` would pick that
...
def alvarez_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Alvarez-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_alvarez_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%ALVAREZ_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_alvarez_home()
    index_url = f"https://nodejs.org/dist/latest-v{_ALVAREZ_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_ALVAREZ_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_alvarez_managed_node() -> bool:
    """Redownload Alvarez-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not alvarez_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ALVAREZ_HOME": str(get_alvarez_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_alvarez_node_executable(command: str) -> str | None:
    """Return a Alvarez-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_alvarez_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_alvarez_managed_node():
        for directory in iter_alvarez_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Alvarez-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Alvarez-managed installs.

    This is for Alvarez-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_alvarez_node_executable(command)
    if managed:
        return managed
    if alvarez_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_alvarez_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Alvarez-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_alvarez_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``alvarez update`` — leaving a **dangling symlink** that ``which`` still
...
def display_alvarez_home() -> str:
    """Return a user-friendly display string for the current ALVAREZ_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.alvarez``
        profile:  ``~/.alvarez/profiles/coder``
        custom:   ``/opt/alvarez-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.alvarez``.  For code that needs a real ``Path``, use
    :func:`get_alvarez_home` instead.
    """
    home = get_alvarez_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ALVAREZ_HOME`` or other path
    env vars resolve to an unexpected location.

    See upstream hermes-agent issue #25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ALVAREZ_HOME}/home`` when the profile-home directory exists."""
    alvarez_home = get_alvarez_home_override() or (env or {}).get("ALVAREZ_HOME") or os.getenv("ALVAREZ_HOME")
    if not alvarez_home:
        return None
    profile_home = os.path.join(alvarez_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("ALVAREZ_REAL_HOME") or os.getenv("ALVAREZ_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Alvarez profile HOME.

    ``ALVAREZ_HOME`` scopes Alvarez state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ALVAREZ_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ALVAREZ_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ALVAREZ_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Alvarez' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ALVAREZ_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: upstream hermes-agent issue #47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ALVAREZ_HOME.

    Replaces the ``get_alvarez_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, alvarez_logging.py, alvarez_time.py, etc.).
    """
    return get_alvarez_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ALVAREZ_HOME."""
    return get_alvarez_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ALVAREZ_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_alvarez_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._alvarez_ipv4_patched = True  # type: ignore[attr-defined]
