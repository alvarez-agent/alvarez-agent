...
# 5. Symlinks the 'alvarez' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Alvarez Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/alvarez-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink alvarez into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up alvarez command..."

ALVAREZ_BIN="$SCRIPT_DIR/venv/bin/alvarez"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ALVAREZ_BIN" "$COMMAND_LINK_DIR/alvarez"
echo -e "${GREEN}✓${NC} Symlinked alvarez → $COMMAND_LINK_DISPLAY_DIR/alvarez"
...
# ============================================================================
# Seed bundled skills into ~/.alvarez/skills/
# ============================================================================

ALVAREZ_SKILLS_DIR="${ALVAREZ_HOME:-$HOME/.alvarez}/skills"
mkdir -p "$ALVAREZ_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.alvarez/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ALVAREZ_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  alvarez status        # Check configuration"
if is_termux; then
    echo "  alvarez gateway       # Run gateway in foreground"
else
    echo "  alvarez gateway install # Install gateway service (messaging + cron)"
fi
echo "  alvarez cron list     # View scheduled jobs"
echo "  alvarez doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m alvarez_cli.main setup
fi
