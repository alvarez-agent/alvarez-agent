"""disk_cleanup — ephemeral file cleanup for Alvarez Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ALVAREZ_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ALVAREZ_HOME and /tmp/alvarez-*
Never touches: ~/.alvarez/logs/ or any system directory.
"""
...
try:
    ...
    def get_alvarez_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ALVAREZ_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".alvarez").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ALVAREZ_HOME/logs/``."""
    return get_alvarez_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ALVAREZ_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ALVAREZ_HOME or ``/tmp/alvarez-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    alvarez_home = get_alvarez_home()
    try:
        path.resolve().relative_to(alvarez_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/alvarez-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("alvarez-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "alvarez-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ALVAREZ_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so ALVAREZ_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        alvarez_home = get_alvarez_home()
        for parent in ("cron", "cronjobs"):
            base = alvarez_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ALVAREZ_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ALVAREZ_HOME, but never recurse into known
    # durable state trees.  Some installs place the Alvarez checkout, venv,
    # and desktop build under ALVAREZ_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    alvarez_home = get_alvarez_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in alvarez_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    alvarez_home = get_alvarez_home()
    try:
        rel = path.resolve().relative_to(alvarez_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "alvarez-agent",
        }:
            return None
        ...
        # Path isn't under ALVAREZ_HOME (e.g. /tmp/alvarez-*) — fall through.
        pass
