...
Auto-tracks and cleans up ephemeral files created during Alvarez Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ALVAREZ_HOME` and `/tmp/alvarez-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ALVAREZ_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ALVAREZ_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `ALVAREZ_HOME` or `/tmp/alvarez-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ALVAREZ_HOME/disk-cleanup/` is itself excluded
- `$ALVAREZ_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
