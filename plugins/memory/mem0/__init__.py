...
Configuration
-------------
Secret (lives in $ALVAREZ_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $ALVAREZ_HOME/mem0.json, set via `alvarez memory
setup`):
  ...
  agent_id           — Agent identifier (default: alvarez)
...
def _load_config() -> dict:
    """Load config from env vars, with $ALVAREZ_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "alvarez"),
        "oss": {},
    }
    ...
    config_path = get_alvarez_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, alvarez_home):
        """Write config to $ALVAREZ_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, alvarez_home: str, config: dict) -> None:
        ...
        post_setup(alvarez_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $ALVAREZ_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "alvarez")
