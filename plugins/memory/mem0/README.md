...
```bash
alvarez memory setup    # select "mem0"
```

Or manually:
```bash
alvarez config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.alvarez/.env
```

## Config

Behavioral settings live in `$ALVAREZ_HOME/mem0.json` (set them via `alvarez memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.alvarez/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `alvarez-user` | User identifier on Mem0 |
| `agent_id` | `alvarez` | Agent identifier |
...
```bash
alvarez memory setup mem0 --mode oss \
...
```bash
alvarez memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ALVAREZ_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.alvarez/mem0_qdrant"}}
  }
}
```
...
```bash
alvarez memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
alvarez memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$ALVAREZ_HOME/mem0.json`).
