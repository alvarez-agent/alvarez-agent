...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with alvarez-agent)
2. User-installed providers: ``$ALVAREZ_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_alvarez_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_alvarez_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_alvarez_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ALVAREZ_HOME/plugins/`` or None if unavailable."""
    try:
        from alvarez_constants import get_alvarez_home
        d = get_alvarez_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ALVAREZ_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _alvarez_user_memory.<name>.cli, usually
