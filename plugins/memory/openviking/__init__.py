...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Alvarez peer ID in OpenViking (default: alvarez)
...
_DEFAULT_AGENT = "alvarez"
_AGENT_PROMPT_LABEL = "Alvarez peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ALVAREZ_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Alvarez slash-skill scaffolding before sending content to OpenViking.
...
def _load_alvarez_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ALVAREZ_HOME", "")).expanduser() if os.environ.get("ALVAREZ_HOME") else Path.home() / ".alvarez"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Alvarez session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Alvarez only", "write values only to Alvarez .env"),
    ...
    _save_alvarez_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Alvarez .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See alvarez-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_alvarez_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Alvarez peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, alvarez_home: str, config: dict) -> None:
        ...
        from alvarez_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        alvarez_home_path = Path(alvarez_home)
        env_path = alvarez_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_alvarez_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Alvarez run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Alvarez' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Alvarez canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See alvarez-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/alvarez/.overview.md).
