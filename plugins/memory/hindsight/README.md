...
```bash
alvarez memory setup    # select "hindsight"
```
...
```bash
alvarez config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.alvarez/.env
```
...
Alvarez spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.alvarez/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p alvarez ui start
```
...
Config file: `~/.alvarez/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `alvarez` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `alvarez-{profile}` isolates memory per active Alvarez profile. Empty placeholders collapse cleanly (e.g. `alvarez-{user}` with no user becomes `alvarez`). |
...
| `retain_context` | `conversation between Alvarez Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.alvarez/.env` as `HINDSIGHT_LLM_API_KEY`.
