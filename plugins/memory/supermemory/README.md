...
- Supermemory API key from [app.supermemory.ai/integrations?connect=alvarez](http://app.supermemory.ai/integrations?connect=alvarez)

## Setup

```bash
alvarez memory setup    # select "supermemory"
```

Or manually:

```bash
alvarez config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.alvarez/.env
```

## Config

Config file: `$ALVAREZ_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `alvarez` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `alvarez-{identity}` → `alvarez-coder`). |
...
All Supermemory API calls send `x-sm-source: alvarez`, and document writes stamp
`metadata.sm_source: alvarez`. This is a **functional routing key, not telemetry**:
it groups Alvarez-written memories into a dedicated "Alvarez" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Alvarez profile:

```json
{
  "container_tag": "alvarez-{identity}"
}
```

For a profile named `coder`, this resolves to `alvarez-coder`. The default profile resolves to `alvarez-default`. Without `{identity}`, all profiles share the same container.
