...
```bash
alvarez memory setup    # select "byterover"
```

Or manually:
```bash
alvarez config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.alvarez/.env
```
...
Working directory: `$ALVAREZ_HOME/byterover/` (profile-scoped).
