...
```bash
alvarez memory setup    # select "holographic"
```

Or manually:
```bash
alvarez config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.alvarez-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ALVAREZ_HOME/memory_store.db` | SQLite database path |
