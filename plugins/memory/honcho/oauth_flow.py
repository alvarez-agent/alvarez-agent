...
``alvarez://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Alvarez OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "alvarez-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``alvarez-desktop`` / ``alvarez-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Alvarez.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ALVAREZ_HOME override can't reach.
