...
``{title, url, description, position}`` rows every other Alvarez web
provider produces.
...
prefers Alvarez-managed xAI Grok OAuth (via ``alvarez auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.alvarez/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``alvarez tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `alvarez auth` to sign in with "
