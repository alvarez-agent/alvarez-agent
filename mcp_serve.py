"""
Alvarez MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Alvarez-specific extra)

Usage:
    alvarez mcp serve
    alvarez mcp serve --verbose
...
logger = logging.getLogger("alvarez.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ALVAREZ_HOME."""
    try:
        from alvarez_constants import get_alvarez_home
        return get_alvarez_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ALVAREZ_HOME", Path.home() / ".alvarez")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from alvarez_constants import get_alvarez_home
        directory_file = get_alvarez_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ALVAREZ_HOME", Path.home() / ".alvarez")
...
class EventBridge:
    ...
    This is the Alvarez equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from alvarez_constants import get_alvarez_home
            db_file = get_alvarez_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ALVAREZ_HOME", Path.home() / ".alvarez")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Alvarez MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "alvarez",
        instructions=(
            "Alvarez Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Alvarez MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
