# OpenAI-Compatible API Server for Alvarez Agent
...
SSE streaming. By exposing this endpoint, alvarez-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  alvarez-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.alvarez/.env`
2. Run `alvarez gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with alvarez-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns alvarez-agent as a model) |
...
Authorization: Bearer alvarez-api-key-here
...
But alvarez-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "alvarez-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured alvarez-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of alvarez-agent's core prompt
...
Once implemented, alvarez-agent works as a drop-in backend for:
