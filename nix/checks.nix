...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      alvarez-agent = self'.packages.default;
      alvarezVenv = alvarez-agent.alvarezVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "alvarez-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${alvarezVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "alvarez-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "alvarez-build-package" { } ''
          echo "PASS: package built at ${alvarez-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "alvarez-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "alvarez-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${alvarez-agent}/bin/alvarez || (echo "FAIL: alvarez binary missing"; exit 1)
          test -x ${alvarez-agent}/bin/alvarez-agent || (echo "FAIL: alvarez-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${alvarez-agent}/bin/alvarez version 2>&1 | grep -qi "alvarez" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "alvarez-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in alvarez alvarez-agent alvarez-acp; do
            test -x ${alvarez-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "alvarez-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking alvarez --help ==="
          ${alvarez-agent}/bin/alvarez --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${alvarez-agent}/bin/alvarez --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "alvarez-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${alvarez-agent}/share/alvarez-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${alvarez-agent}/share/alvarez-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ALVAREZ_BUNDLED_SKILLS" ${alvarez-agent}/bin/alvarez || \
            (echo "FAIL: ALVAREZ_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ALVAREZ_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "alvarez-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${alvarez-agent}/share/alvarez-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${alvarez-agent}/share/alvarez-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ALVAREZ_BUNDLED_PLUGINS" ${alvarez-agent}/bin/alvarez || \
            (echo "FAIL: ALVAREZ_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ALVAREZ_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "alvarez-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${alvarez-agent}/share/alvarez-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${alvarez-agent}/share/alvarez-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${alvarez-agent}/share/alvarez-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ALVAREZ_BUNDLED_LOCALES" ${alvarez-agent}/bin/alvarez || \
            (echo "FAIL: ALVAREZ_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ALVAREZ_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ALVAREZ_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ALVAREZ_BUNDLED_LOCALES=${alvarez-agent}/share/alvarez-agent/locales \
            ${alvarezVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ALVAREZ_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${alvarezVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${alvarezVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ALVAREZ_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "alvarez-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${alvarez-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${alvarez-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ALVAREZ_TUI_DIR" ${alvarez-agent}/bin/alvarez || \
            (echo "FAIL: ALVAREZ_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ALVAREZ_TUI_DIR set in wrapper"
        ...
        # Verify ALVAREZ_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        alvarez-node = pkgs.runCommand "alvarez-node-version" { } ''
          set -e
          echo "=== Checking ALVAREZ_NODE in wrapper ==="
          grep -q "ALVAREZ_NODE" ${alvarez-agent}/bin/alvarez || \
            (echo "FAIL: ALVAREZ_NODE not set in wrapper"; exit 1)
          echo "PASS: ALVAREZ_NODE present in wrapper"

          ALVAREZ_NODE=$(sed -n "s/^export ALVAREZ_NODE='\(.*\)'/\1/p" ${alvarez-agent}/bin/alvarez)
          test -x "$ALVAREZ_NODE" || (echo "FAIL: ALVAREZ_NODE=$ALVAREZ_NODE not executable"; exit 1)
          echo "PASS: ALVAREZ_NODE executable at $ALVAREZ_NODE"

          NODE_MAJOR=$("$ALVAREZ_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ALVAREZ_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ALVAREZ_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "alvarez-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ALVAREZ_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ALVAREZ_MANAGED guards ==="
          check_blocked "config set" ${alvarez-agent}/bin/alvarez config set model foo
          check_blocked "config edit" ${alvarez-agent}/bin/alvarez config edit
        ...
        extra-python-packages = let
          ...
          alvarezWithExtra = alvarez-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "alvarez-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${alvarezWithExtra}/bin/alvarez || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${alvarezWithExtra}/bin/alvarez || \
          ...
          if grep -q "PYTHONPATH" ${alvarez-agent}/bin/alvarez; then
        ...
        extra-dependency-groups = let
          alvarezWithGroups = alvarez-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "alvarez-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${alvarezWithGroups}"
          echo "venv: ${alvarezWithGroups.alvarezVenv}"
        ...
        messaging-variant = pkgs.runCommand "alvarez-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.alvarezVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "alvarez-config-roundtrip" {
          ...
          merge_and_load() {
            local alvarez_home="$1"
            export ALVAREZ_HOME="$alvarez_home"
            ${configMergeScript} ${nixSettings} "$alvarez_home/config.yaml"
            ${alvarezVenv}/bin/python3 -c '
import json, sys
