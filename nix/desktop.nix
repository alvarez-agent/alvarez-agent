# nix/desktop.nix — Alvarez Desktop (Electron) app build + wrapper
#
# `alvarezAgent` is the fully-built `.#default` package — it ships the
# `alvarez` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ALVAREZ_DESKTOP_ALVAREZ` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Alvarez CLI").
...
let
  npm = alvarezNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "alvarez-desktop";
  };
  ...
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "alvarez-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/alvarez-desktop $out/bin
    cp -r ${renderer}/* $out/share/alvarez-desktop/
    ...
    substituteInPlace $out/share/alvarez-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/alvarez-desktop'"
    ...
    substituteInPlace $out/share/alvarez-desktop/electron/git-review-ops.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/alvarez-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ALVAREZ_DESKTOP_ALVAREZ to the absolute path of the nix-built `alvarez`
    # binary so the desktop's resolver step 4 ("existing Alvarez CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/alvarez-desktop \
      --add-flags "$out/share/alvarez-desktop" \
      --set ALVAREZ_DESKTOP_ALVAREZ "${lib.getExe alvarezAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Alvarez Agent";
    homepage = "https://github.com/NousResearch/hermes-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "alvarez-desktop";
  };
}
