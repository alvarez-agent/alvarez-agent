...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      alvarezNpmLib = self'.packages.default.passthru.alvarezNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. alvarez-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "Alvarez Agent dev shell"
          ${combinedNonNpm}
          ${alvarezNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          echo "Ready. Run 'alvarez' to start."
        '';
      };
    };
}
