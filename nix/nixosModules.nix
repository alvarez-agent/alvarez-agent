# nix/nixosModules.nix — NixOS module for alvarez-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: alvarez runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ALVAREZ_HOME/.env
# and read by alvarez at startup — no container recreation needed for env changes.
#
# Tool resolution: the alvarez wrapper uses --suffix PATH for nix store tools,
...
#
# Usage:
#   services.alvarez-agent = {
#     enable = true;
#     settings.model = "anthropic/claude-sonnet-4";
#     environmentFiles = [ config.sops.secrets."alvarez/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.alvarez-agent;
    effectivePackage =
    ...
    alvarez-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-alvarez-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "alvarez-config-attrs";
      description = "Alvarez YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "alvarez-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # ALVAREZ_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "alvarez-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ALVAREZ_DOC_EOF'\n${value}\nALVAREZ_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "alvarez-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/alvarez";
    ...
    # alvarez user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "alvarez-container-entrypoint" ''
      set -eu

      ALVAREZ_UID="''${ALVAREZ_UID:?ALVAREZ_UID must be set}"
      ALVAREZ_GID="''${ALVAREZ_GID:?ALVAREZ_GID must be set}"

      # ── Group: ensure a group with GID=$ALVAREZ_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ALVAREZ_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="alvarez"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ALVAREZ_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ALVAREZ_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ALVAREZ_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ALVAREZ_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="alvarez"
        TARGET_HOME="/home/alvarez"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ALVAREZ_UID" -g "$ALVAREZ_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ALVAREZ_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ALVAREZ_UID:$ALVAREZ_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ALVAREZ_HOME is owned by the target user.
      ...
      if [ -n "''${ALVAREZ_HOME:-}" ] && [ -d "$ALVAREZ_HOME" ]; then
        find "$ALVAREZ_HOME" \! -user "$ALVAREZ_UID" -exec chown "$ALVAREZ_UID:$ALVAREZ_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/alvarez-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/alvarez-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/alvarez ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/alvarez
        chmod 0440 /etc/sudoers.d/alvarez
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ALVAREZ_UID" --regid="$ALVAREZ_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ALVAREZ_HOME/.env.
    ...
    # Default: /var/lib/alvarez/workspace → /data/workspace.
  ...
  in {
    options.services.alvarez-agent = with lib; {
      enable = mkEnableOption "Alvarez Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = alvarez-agent;
        description = "The alvarez-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/alvarez";
        description = "State directory. Contains .alvarez/ subdir (ALVAREZ_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Alvarez config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ALVAREZ_HOME/.env at activation time.
          Alvarez reads this file on every startup via load_alvarez_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ALVAREZ_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ALVAREZ_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `alvarez gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the alvarez user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the alvarez plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Alvarez discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          alvarez_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as alvarez (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in alvarez-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the alvarez CLI to environment.systemPackages and export
          ALVAREZ_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.alvarez symlink to the service
            stateDir. These users are automatically added to the alvarez group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.alvarez-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the alvarez CLI to system PATH and export ALVAREZ_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.alvarez/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ALVAREZ_HOME = "${cfg.stateDir}/.alvarez";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.alvarez-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the alvarez user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.alvarez-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed alvarez
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure alvarez is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.alvarez        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alvarez/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alvarez/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alvarez/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alvarez/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alvarez/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."alvarez-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.alvarez
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.alvarez ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.alvarez ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.alvarez -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.alvarez/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.alvarez/$_subdir"
            chmod 2770 "${cfg.stateDir}/.alvarez/$_subdir"
            find "${cfg.stateDir}/.alvarez/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # alvarez-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.alvarez/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.alvarez/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alvarez/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.alvarez/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.alvarez/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alvarez/.managed
          chmod 0644 ${cfg.stateDir}/.alvarez/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.alvarez/.container-mode <<'ALVAREZ_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    alvarez_bin=${containerDataDir}/current-package/bin/alvarez
    ALVAREZ_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alvarez/.container-mode
            chmod 0644 ${cfg.stateDir}/.alvarez/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.alvarez/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alvarez";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.alvarez" ]; then
                  rm -f "${symlinkPath}"
                  echo "alvarez-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.alvarez -> stateDir/.alvarez for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alvarez";
                target = "${cfg.stateDir}/.alvarez";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "alvarez-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alvarez/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.alvarez/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alvarez/auth.json
              fi
            ''}
          ''}
          ...
          # Alvarez reads $ALVAREZ_HOME/.env at startup via load_alvarez_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.alvarez/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ALVAREZ_NIX_ENV_EOF'
    ${envFileContent}
    ALVAREZ_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.alvarez/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.alvarez/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alvarez/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.alvarez-agent = {
          description = "Alvarez Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            ALVAREZ_HOME = "${cfg.stateDir}/.alvarez";
            ALVAREZ_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $ALVAREZ_HOME/.env by the activation script. load_alvarez_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/alvarez"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the alvarez group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.alvarez-agent = {
          description = "Alvarez Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ALVAREZ_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ALVAREZ_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ALVAREZ_UID="$ALVAREZ_UID" \
                --env ALVAREZ_GID="$ALVAREZ_GID" \
                --env ALVAREZ_HOME=${containerDataDir}/.alvarez \
                --env ALVAREZ_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/alvarez gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
