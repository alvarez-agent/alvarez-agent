"""ACP auth helpers — detect and advertise Alvarez authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "alvarez-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Alvarez runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Alvarez can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Alvarez.
    ...
    not have Alvarez provider credentials configured yet, so Alvarez always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Alvarez using the currently configured "
