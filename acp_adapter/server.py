"""ACP agent server — exposes Alvarez Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_alvarez_interactive_context,
    set_alvarez_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from alvarez_cli import __version__ as ALVAREZ_VERSION
except Exception:
    ALVAREZ_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Alvarez running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Alvarez cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Alvarez/OpenAI-compatible user content payload."""
...
class AlvarezACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Alvarez AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Alvarez version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Alvarez maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from alvarez_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from alvarez_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Alvarez estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_alvarez_session_id: str,
        previous_alvarez_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.alvarez.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_alvarez_session_id,
                previous_alvarez_session_id=previous_alvarez_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_alvarez_session_id: Optional[str] = None,
        previous_alvarez_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Alvarez changes it.

        When the internal Alvarez head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_alvarez_session_id`` so the
        attached ``_meta.alvarez.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # alvarez_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_alvarez_session_id or session_id,
            previous_alvarez_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["alvarez-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="alvarez-agent", version=ALVAREZ_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Alvarez' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Alvarez setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Alvarez
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Alvarez on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Alvarez' local kawaii waiting/status
        ...
        # tools.approval (set_alvarez_interactive_context) rather than
        # os.environ["ALVAREZ_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue ALVAREZ_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ALVAREZ_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_alvarez_interactive_context(True)
            ...
            previous_session_id = os.environ.get("ALVAREZ_SESSION_ID")
            os.environ["ALVAREZ_SESSION_ID"] = session_id
            try:
                ...
                if interactive_token is not None:
                    reset_alvarez_interactive_context(interactive_token)
                # Restore ALVAREZ_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("ALVAREZ_SESSION_ID", None)
                else:
                    os.environ["ALVAREZ_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Alvarez DB session id before the turn so we
            ...
            pre_turn_alvarez_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (ALVAREZ_SESSION_KEY in
        ...
        # _meta.alvarez.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_alvarez_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_alvarez_id
            and pre_turn_alvarez_id
            and post_turn_alvarez_id != pre_turn_alvarez_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_alvarez_session_id=post_turn_alvarez_id,
                    previous_alvarez_session_id=pre_turn_alvarez_id,
                )
            except Exception:
                logger.debug(
        ...
        # Alvarez' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Alvarez Agent v{ALVAREZ_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Alvarez has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
