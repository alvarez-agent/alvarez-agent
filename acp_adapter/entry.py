"""CLI entry point for the alvarez-agent ACP adapter.

Loads environment variables from ``~/.alvarez/.env``, configures logging
...
try:
    ...
    # shadowing Alvarez's own modules — ``alvarez acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    alvarez_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from ALVAREZ_HOME (default ``~/.alvarez``)."""
    from alvarez_cli.env_loader import load_alvarez_dotenv

    alvarez_home = get_alvarez_home()
    loaded = load_alvarez_dotenv(alvarez_home=alvarez_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", alvarez_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="alvarez-acp",
        description="Run Alvarez Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Alvarez version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Alvarez provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.alvarez/node/ "
...
def _print_version() -> None:
    ...
    print(alvarez_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import AlvarezACPAgent  # noqa: F401

    print("Alvarez ACP check OK")


def _run_setup() -> None:
    from alvarez_cli.main import main as alvarez_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "alvarez", "model"]
        alvarez_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``alvarez postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting alvarez-agent ACP adapter")
    ...
    import acp
    from .server import AlvarezACPAgent
