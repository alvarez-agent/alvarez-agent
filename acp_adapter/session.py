"""ACP session manager — maps ACP sessions to Alvarez AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.alvarez/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Alvarez itself is running in WSL.

    Windows ACP clients can launch ``alvarez acp`` inside WSL while still sending
    ...
    """
    from alvarez_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Alvarez from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["alvarez-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Alvarez agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Alvarez AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Alvarez runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.alvarez/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ALVAREZ_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_alvarez_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=alvarez_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Alvarez daemon's process cwd.
