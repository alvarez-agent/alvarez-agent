...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_alvarez_env(self):
        """Alvarez' own .env inside ALVAREZ_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: alvarez internal .env is in ~/.alvarez/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.alvarez/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.alvarez/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Alvarez cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        alvarez_home = tmp_path / ".alvarez"
        cache = alvarez_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._alvarez_home_path", return_value=alvarez_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Alvarez cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        alvarez_home = tmp_path / ".alvarez"
        hub = alvarez_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_alvarez_home(self, tmp_path):
        """The env basename guard does not depend on ALVAREZ_HOME resolution."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        alvarez_home = tmp_path / ".alvarez"
        cache = alvarez_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
