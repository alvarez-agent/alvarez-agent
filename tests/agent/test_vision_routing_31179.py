...
def isolated_home(monkeypatch):
    """Temp ALVAREZ_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="alvarez_test_31179_")
    alvarez_home = os.path.join(test_home, ".alvarez")
    os.makedirs(alvarez_home)
...
def _fresh_modules():
    """Drop cached alvarez modules so each test reloads against current env."""
