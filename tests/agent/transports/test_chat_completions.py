...
class TestChatCompletionsBasic:
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Alvarez-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when Alvarez has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. request tags) must not reach a native
    Gemini endpoint — Google's REST API rejects unknown fields with HTTP 400.
    """

    def _tagging_profile(self):
        from providers.base import ProviderProfile

        class _Tagging(ProviderProfile):
            def build_extra_body(self, *args, **kwargs):
                return {"tags": ["session:test"]}

        return _Tagging(name="tagging-test")
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            [{"role": "user", "content": "hi"}],
            None,
            provider_profile=self._tagging_profile(),
            base_url="https://generativelanguage.googleapis.com/v1beta",
            session_id="s1",
            max_tokens=None,
        )
    ...
    def test_tags_preserved_on_plain_openai_compat_endpoint(self, transport):
        kw = transport.build_kwargs(
            "hermes-3-405b",
            [{"role": "user", "content": "hi"}],
            None,
