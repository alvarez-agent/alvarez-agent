...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Alvarez passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ALVAREZ_HOME", "/users/alice/.alvarez/profiles/backend-worker")
        monkeypatch.setenv("ALVAREZ_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ALVAREZ_KANBAN_DB",
            "/users/alice/.alvarez/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.alvarez/kanban/boards/smoke"]'
            in cmd
        )
