...
""",
        ...
        from alvarez_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        from alvarez_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from alvarez_cli.plugins import PluginContext, PluginManifest, PluginManager
