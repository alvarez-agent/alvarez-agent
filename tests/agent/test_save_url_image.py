...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ALVAREZ_HOME under tmp_path."""
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path / ".alvarez"))
    (tmp_path / ".alvarez").mkdir()

    # Force the constants/image cache helpers to re-read ALVAREZ_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("alvarez_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_alvarez_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ALVAREZ_HOME — gateway cleanup
