"""
Tests for mcp_serve — Alvarez MCP server.
...
def _isolate_alvarez_home(tmp_path, monkeypatch):
    """Redirect ALVAREZ_HOME to a temp directory."""
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    try:
        import alvarez_constants
        monkeypatch.setattr(alvarez_constants, "get_alvarez_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking alvarez_state schema."""
