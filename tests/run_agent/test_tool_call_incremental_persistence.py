...
def _make_agent():
    alvarez_home = Path(tempfile.mkdtemp(prefix="alvarez-test-home-"))
    (alvarez_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._alvarez_home", alvarez_home),
