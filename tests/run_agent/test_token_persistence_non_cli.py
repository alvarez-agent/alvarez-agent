...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    alvarez_state = ModuleType("alvarez_state")
    alvarez_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "alvarez_state", alvarez_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Alvarez"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Alvarez"
