"""Tests for alvarez_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Alvarez entry point
(alvarez, alvarez-agent, alvarez-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Alvarez entry point imports alvarez_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported alvarez_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("alvarez_bootstrap", None)
    import alvarez_bootstrap  # noqa: WPS433
    return alvarez_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Alvarez CLI
...
class TestEntryPointsImportBootstrap:
    """Every Alvarez entry point must import alvarez_bootstrap as its
    ...
    # Entry points that invoke Alvarez as a process.  Each one must
    # import alvarez_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "alvarez_cli/main.py",   # alvarez CLI (console_script)
        "run_agent.py",          # alvarez-agent (console_script)
        "acp_adapter/entry.py",  # alvarez-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import alvarez_bootstrap' and that
        ...
        half-finished ``alvarez update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``alvarez_bootstrap`` as a top-level module) leaves alvarez
        ...
        """
        # Resolve relative to the alvarez-agent repo root.  Tests live
        # at tests/test_alvarez_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # alvarez start even when alvarez_bootstrap hasn't been
        ...
        assert first_import_name == "alvarez_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'alvarez_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import alvarez_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Alvarez's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ALVAREZ_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ALVAREZ_PYTHON_SRC_ROOT"] = env
            elif "ALVAREZ_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ALVAREZ_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/alvarez")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ALVAREZ_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ALVAREZ_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/alvarez", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Alvarez root.  The guard must relocate Alvarez to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/alvarez"])
        assert result[0] == "/opt/alvarez"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Alvarez root.
        assert result.index("/opt/alvarez") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/alvarez", "/opt/alvarez", ""])
        assert result.count("/opt/alvarez") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ALVAREZ_PYTHON_SRC_ROOT"] = "/env/alvarez"
            hb.harden_import_path()
            assert sys.path[0] == "/env/alvarez"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ALVAREZ_PYTHON_SRC_ROOT", None)
