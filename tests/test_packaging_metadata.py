...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``alvarez_cli``) for the package
    itself and ``alvarez_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``alvarez_cli`` without ``alvarez_cli.*``,
    so the wheel shipped ``alvarez_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'alvarez_cli.dashboard_auth'``.
...
def test_packaging_declared_as_core_dependency():
    ...
    alvarez_cli/main.py) yet was undeclared, so it only reached users
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (alvarez_cli/plugins.py) registers each bundled plugin by
...
def test_starlette_pinned_above_cve_2026_48710_floor_in_pyproject():
    ...
    # The server-surface extras must each carry the direct pin.
