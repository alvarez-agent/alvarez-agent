...
CronScheduler plan, .alvarez/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - alvarez_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_cron_status_reports_alive_but_failing(tmp_path, monkeypatch, capsys):
    ...
    import cron.jobs as jobs
    from alvarez_cli import cron as cron_cli

    monkeypatch.setattr("alvarez_cli.gateway.find_gateway_pids", lambda: [4321])
