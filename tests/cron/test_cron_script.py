...
- Path resolution (absolute, relative to ALVAREZ_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ALVAREZ_HOME."""
    alvarez_home = tmp_path / ".alvarez"
    alvarez_home.mkdir()
    (alvarez_home / "cron").mkdir()
    (alvarez_home / "cron" / "output").mkdir()
    (alvarez_home / "scripts").mkdir()
    monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ALVAREZ_DIR", alvarez_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", alvarez_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", alvarez_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", alvarez_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Alvarez provider env (SECURITY.md §2.3)."""
        ...
        blocked_var = sorted(_ALVAREZ_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.alvarez/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ALVAREZ_SESSION_PLATFORM") is None
        assert os.environ.get("ALVAREZ_SESSION_CHAT_ID") is None
        assert os.environ.get("ALVAREZ_SESSION_CHAT_NAME") is None
