...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAlvarezSessionId"] == "root1"
    assert prov["rootAlvarezSessionId"] == "root1"
    assert prov["parentAlvarezSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_alvarez_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAlvarezSessionId"] == "old"
    assert prov["rootAlvarezSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAlvarezSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootAlvarezSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAlvarezSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_alvarez_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousAlvarezSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"alvarez"}
    assert "sessionProvenance" in meta["alvarez"]
    assert meta["alvarez"]["sessionProvenance"]["currentAlvarezSessionId"] == "root1"
