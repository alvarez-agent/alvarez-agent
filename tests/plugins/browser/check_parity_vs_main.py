...
Run from the PR worktree:
    ...
    cd ~/.alvarez/alvarez-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.alvarez/alvarez-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.alvarez/alvarez-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a alvarez-agent checkout"
)
...
# Isolated ALVAREZ_HOME for the config write.
