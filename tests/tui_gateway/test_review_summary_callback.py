...
def server():
    with patch.dict(
        "sys.modules",
        {
            "alvarez_constants": MagicMock(
                get_alvarez_home=MagicMock(return_value="/tmp/alvarez_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'alvarez-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'alvarez-release' patched"
    }
