...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.alvarez/alvarez-agent/utils.py)
...
2. ``alvarez update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``alvarez_cli/main.py`` (the ``alvarez update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``alvarez_constants`` after the
