...
class TestAllResolvableCommandsBypassGuard:
    ...
    def test_should_bypass_returns_true_for_every_registered_command(self):
        ...
        for cmd in (
            "model", "reasoning", "mood", "voice", "insights", "title",
...
class TestBypassWithBotnameSuffix:
    ...
    async def test_stop_with_botname(self):
        """/stop@MyAlvarezBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/stop@MyAlvarezBot"))

        assert sk not in adapter._pending_messages, (
            "/stop@MyAlvarezBot was queued instead of bypassing"
        )
        assert any("handled:stop" in r for r in adapter.sent_responses)

    @pytest.mark.asyncio
    async def test_new_with_botname(self):
        """/new@MyAlvarezBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/new@MyAlvarezBot"))
