...
closure the PR changed, against a real temp ``ALVAREZ_HOME``.
"""
...
def _fake_switch_result():
    ...
    from alvarez_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "alvarez_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_alvarez_home() / config.yaml`` — point it here.
    monkeypatch.setattr("alvarez_constants.get_alvarez_home", lambda: alvarez_home)
    monkeypatch.setattr("alvarez_cli.config.get_alvarez_home", lambda: alvarez_home)
    return cfg_path
