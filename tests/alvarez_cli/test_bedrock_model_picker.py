...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ALVAREZ_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ALVAREZ_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ALVAREZ_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from alvarez_cli.providers import ALVAREZ_OVERLAYS
        assert "bedrock" in ALVAREZ_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from alvarez_cli.providers import ALVAREZ_OVERLAYS
        assert ALVAREZ_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from alvarez_cli.providers import ALVAREZ_OVERLAYS
        assert ALVAREZ_OVERLAYS["bedrock"].auth_type == "aws_sdk"
