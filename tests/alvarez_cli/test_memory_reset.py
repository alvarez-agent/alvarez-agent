"""Tests for the `alvarez memory reset` CLI command.
...
- Profile-scoped reset (uses ALVAREZ_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ALVAREZ_HOME with memory files."""
    alvarez_home = tmp_path / ".alvarez"
    memories = alvarez_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAlvarez repo is at ~/.alvarez/alvarez-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `alvarez memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        (alvarez_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ALVAREZ_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_alvarez_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
