...
When ``ALVAREZ_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.alvarez/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.alvarez            (has its own auth.json fixture)
    * Profile     -> tmp_path/.alvarez/profiles/coder   (active, ALVAREZ_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Alvarez is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from alvarez_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    alvarez_home = tmp_path / "classic"
    alvarez_home.mkdir()
    monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))

    _write(alvarez_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ALVAREZ_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.alvarez as the "real home" — which is NOT equal
    # to the ALVAREZ_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    alvarez_home = tmp_path / "classic"
    ...
    _write(alvarez_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from alvarez_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ALVAREZ_HOME is set to a custom path that is NOT under
    # ~/.alvarez/profiles/ — get_default_alvarez_root() returns ALVAREZ_HOME
