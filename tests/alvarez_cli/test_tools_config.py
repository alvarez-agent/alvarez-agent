"""Tests for alvarez_cli.tools_config platform tool persistence."""
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'alvarez' instead of 'alvarez-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import alvarez_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["alvarez"]}}

    with caplog.at_level(logging.WARNING, logger="alvarez_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "alvarez" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["alvarez-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["alvarez-cli", "bogus"]}}
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `alvarez tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "alvarez_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "alvarez_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `alvarez tools`,
    ...
    # User explicitly opted into spotify but not x_search via `alvarez tools`.
    config = {"platform_toolsets": {"cli": ["alvarez-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[alvarez-cli, spotify]`` (composite + configurable) must keep the full
    ``alvarez-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``alvarez-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when alvarez-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["alvarez-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``alvarez-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["alvarez-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _ALVAREZ_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `alvarez tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("alvarez_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (alvarez-cli, alvarez-telegram, etc.) must NOT
    ...
    ``alvarez tools``, but alvarez-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "alvarez-cli",  # <-- the culprit
    ...
    # alvarez-cli must NOT survive — it's a platform default, not an MCP server
    assert "alvarez-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_alvarez_telegram():
    """Same bug for Telegram — alvarez-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "alvarez-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "alvarez-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "alvarez-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("alvarez_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from alvarez_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "alvarez_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("alvarez_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "alvarez_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("alvarez_cli.tools_config.save_config", lambda config: None)
...
class TestPlatformToolsetConsistency:
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from alvarez_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from alvarez_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `alvarez tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's ALVAREZ_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"ALVAREZ_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"ALVAREZ_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from alvarez_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from alvarez_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from alvarez_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("alvarez_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("alvarez_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("alvarez_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`alvarez tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "alvarez-_test_platform"},
    }

    with mock_patch("alvarez_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'alvarez-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `alvarez tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from alvarez_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `alvarez tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from alvarez_cli.tools_config import _save_platform_tools
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("alvarez_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("alvarez_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("alvarez_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("alvarez_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from alvarez_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from alvarez_cli import tools_config
...
#    reported as added/removed by `alvarez tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `alvarez tools` printed ``- kanban``
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    as ``alvarez model`` and writes the selection to the auxiliary config keys
    the resolver reads.
    """
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    import alvarez_cli.tools_config as tc
    ...
    seq = iter([1, 0, 1])
    with patch("alvarez_cli.model_switch.list_authenticated_providers",
