"""Tests for the `alvarez proxy` subcommand and its upstream adapters."""
...
from alvarez_cli.proxy.adapters import ADAPTERS, get_adapter
from alvarez_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from alvarez_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic ALVAREZ_HOME."""
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alvarez_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alvarez_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from alvarez_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    from alvarez_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from alvarez_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from alvarez_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "alvarez auth add xai-oauth" in err
