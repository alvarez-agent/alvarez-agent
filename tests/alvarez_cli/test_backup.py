"""Tests for alvarez backup and import commands."""
...
def _make_alvarez_tree(root: Path) -> None:
    """Create a realistic ~/.alvarez directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "alvarez_state.db").write_bytes(b"fake-state")
    ...
    # alvarez-agent repo (should be EXCLUDED)
    (root / "alvarez-agent").mkdir(exist_ok=True)
    (root / "alvarez-agent" / "run_agent.py").write_text("# big file\n")
    (root / "alvarez-agent" / ".git").mkdir()
    (root / "alvarez-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_alvarez_agent(self):
        from alvarez_cli.backup import _should_exclude
        assert _should_exclude(Path("alvarez-agent/run_agent.py"))
        assert _should_exclude(Path("alvarez-agent/.git/HEAD"))
    ...
    def test_includes_nested_alvarez_agent_in_skills(self):
        """skills/autonomous-ai-agents/alvarez-agent/ must NOT be excluded —
        only the root-level alvarez-agent/ repo is skipped."""
        from alvarez_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/alvarez-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/alvarez-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under ALVAREZ_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        _make_alvarez_tree(alvarez_home)

        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        # get_default_alvarez_root needs this
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import alvarez_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = alvarez_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, alvarez_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_alvarez_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include alvarez-agent/ directory."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        _make_alvarez_tree(alvarez_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "alvarez-agent" in n]
            assert agent_files == [], f"alvarez-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under ALVAREZ_HOME must be
        ...
        venv_pkg = alvarez_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = alvarez_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_alvarez_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../alvarez-agent/ but NOT root alvarez-agent/."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        _make_alvarez_tree(alvarez_home)

        # Add a nested alvarez-agent directory inside skills (like the real layout)
        nested = alvarez_home / "skills" / "autonomous-ai-agents" / "alvarez-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Alvarez Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root alvarez-agent must be excluded
            root_agent = [n for n in names if n.startswith("alvarez-agent/")]
            assert root_agent == [], f"root alvarez-agent leaked: {root_agent}"
            # Nested skill alvarez-agent must be included
            assert "skills/autonomous-ai-agents/alvarez-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/alvarez-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/alvarez-backup-*.zip."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("alvarez-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ALVAREZ_HOME."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        _make_alvarez_tree(alvarez_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(alvarez_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Alvarez backup."""
        from alvarez_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only alvarez_state.db (old wrong name) is rejected."""
        from alvarez_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["alvarez_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into alvarez home."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (alvarez_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (alvarez_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (alvarez_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (alvarez_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_alvarez_prefix(self, tmp_path, monkeypatch):
        """Import strips .alvarez/ prefix if all entries share it."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".alvarez/config.yaml": "model: test\n",
            ".alvarez/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (alvarez_home / "config.yaml").read_text() == "model: test\n"
        assert (alvarez_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_alvarez_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a alvarez backup."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (alvarez_home / "config.yaml").exists()
        # traversal file should NOT exist outside alvarez home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (alvarez_home / "gateway_state.json").write_text(live_state)
        ...
        assert (alvarez_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (alvarez_home / "config.yaml").exists()
        assert not (alvarez_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        (alvarez_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (alvarez_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (alvarez_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            alvarez_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (alvarez_home / "gateway.pid").write_text("4242")
        (alvarez_home / "processes.json").write_text('{"live": true}')
        ...
        assert (alvarez_home / "gateway.pid").read_text() == "4242"
        assert (alvarez_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (alvarez_home / "cron.pid").exists()
        assert not (alvarez_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (alvarez_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (alvarez_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (alvarez_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (alvarez_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".alvarez"
        src_home.mkdir(parents=True)
        _make_alvarez_tree(src_home)

        monkeypatch.setenv("ALVAREZ_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from alvarez_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".alvarez"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ALVAREZ_HOME", str(dst_home))
        ...
        # alvarez-agent should NOT be present
        assert not (dst_home / "alvarez-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from alvarez_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without alvarez markers fails validation."""
        import io
        from alvarez_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_alvarez(self):
        """Detects .alvarez/ prefix wrapping all entries."""
        import io
        from alvarez_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".alvarez/config.yaml", "test")
            zf.writestr(".alvarez/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".alvarez/"
...
class TestBackupEdgeCases:
    def test_nonexistent_alvarez_home(self, tmp_path, monkeypatch):
        """Backup exits when alvarez home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".alvarez"
        monkeypatch.setenv("ALVAREZ_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("alvarez-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_alvarez_home(self, tmp_path, monkeypatch):
        """Backup handles empty alvarez home (no files to back up)."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        # Only excluded dirs, no actual files
        (alvarez_home / "__pycache__").mkdir()
        (alvarez_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = alvarez_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = alvarez_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_alvarez(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside alvarez root."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside alvarez home
        out_zip = alvarez_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = alvarez_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (alvarez_home / "config.yaml").exists()
        assert (alvarez_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (alvarez_home / "profiles" / "coder" / "config.yaml").exists()
        assert (alvarez_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "alvarez -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from alvarez_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def alvarez_home(self, tmp_path):
        """Create a fake ALVAREZ_HOME with critical state files."""
        home = tmp_path / ".alvarez"
        home.mkdir()
    ...
    def test_creates_snapshot(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        assert snap_id is not None
        snap_dir = alvarez_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", alvarez_home=alvarez_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        db_copy = alvarez_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        assert (alvarez_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        copied = alvarez_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        with open(alvarez_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(alvarez_home=empty) is None

    def test_list_snapshots(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", alvarez_home=alvarez_home)
        id2 = create_quick_snapshot(label="second", alvarez_home=alvarez_home)

        snaps = list_quick_snapshots(alvarez_home=alvarez_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", alvarez_home=alvarez_home)
        snaps = list_quick_snapshots(limit=3, alvarez_home=alvarez_home)
        assert len(snaps) == 3

    def test_restore_config(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)

        (alvarez_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (alvarez_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, alvarez_home=alvarez_home)
        assert result is True
        assert "openrouter" in (alvarez_home / "config.yaml").read_text()

    def test_restore_state_db(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)

        conn = sqlite3.connect(str(alvarez_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, alvarez_home=alvarez_home)
    ...
    def test_restore_nonexistent(self, alvarez_home):
        from alvarez_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", alvarez_home=alvarez_home) is False

    def test_auto_prune(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", alvarez_home=alvarez_home)
        snaps = list_quick_snapshots(limit=100, alvarez_home=alvarez_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", alvarez_home=alvarez_home)
        deleted = prune_quick_snapshots(keep=3, alvarez_home=alvarez_home)
        assert deleted == 7
        assert len(list_quick_snapshots(alvarez_home=alvarez_home)) == 3

    def test_snapshot_includes_pairing_directories(self, alvarez_home):
        ...
        (alvarez_home / "platforms" / "pairing").mkdir(parents=True)
        (alvarez_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (alvarez_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (alvarez_home / "pairing").mkdir()
        (alvarez_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (alvarez_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, alvarez_home):
        ...
        assert restore_quick_snapshot(snap_id, alvarez_home=alvarez_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, alvarez_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (alvarez update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, alvarez_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, alvarez_home):
        ...
        escape_dst = alvarez_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped ALVAREZ_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from alvarez_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        copy = alvarez_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        copy = alvarez_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, alvarez_home):
        from alvarez_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(alvarez_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, alvarez_home):
        ...
        board_dir = alvarez_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        copy = (
            alvarez_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, alvarez_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(alvarez_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, alvarez_home):
        ...
        board = alvarez_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(alvarez_home=alvarez_home)
        snap = alvarez_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, alvarez_home, monkeypatch):
        ...
        import alvarez_cli.backup as bk
        from alvarez_cli.backup import create_quick_snapshot
        ...
        copy = alvarez_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``alvarez update``
    runs before touching anything."""

    @pytest.fixture
    def alvarez_home(self, tmp_path):
        root = tmp_path / ".alvarez"
        root.mkdir()
        _make_alvarez_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, alvarez_home):
        from alvarez_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(alvarez_home=alvarez_home)
        assert out is not None
        assert out.exists()
        assert out.parent == alvarez_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, alvarez_home):
        """Pre-update backup should include the same user data that
        ``alvarez backup`` would, and should exclude the same directories."""
        from alvarez_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(alvarez_home=alvarez_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # alvarez-agent repo excluded
        assert not any(n.startswith("alvarez-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, alvarez_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from alvarez_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(alvarez_home=alvarez_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(alvarez_home=alvarez_home)
    ...
    def test_rotation_keeps_only_n(self, alvarez_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(alvarez_home=alvarez_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (alvarez_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, alvarez_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from alvarez_cli.backup import create_pre_update_backup

        (alvarez_home / "backups").mkdir(exist_ok=True)
        manual = alvarez_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(alvarez_home=alvarez_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from alvarez_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(alvarez_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, alvarez_home):
        ...
        """
        from alvarez_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(alvarez_home=alvarez_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, alvarez_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, alvarez_home):
        ...
        first = create_pre_update_backup(alvarez_home=alvarez_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(alvarez_home=alvarez_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(alvarez_home=alvarez_home, keep=0)
    ...
    def test_skips_symlinked_files(self, alvarez_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ALVAREZ_HOME."""
        from alvarez_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def alvarez_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".alvarez"
        root.mkdir()
        _make_alvarez_tree(root)
        # Point ALVAREZ_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ALVAREZ_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for alvarez_cli.config + alvarez_constants so they pick up ALVAREZ_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("alvarez_cli.config") or mod == "alvarez_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, alvarez_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from alvarez_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "alvarez import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((alvarez_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, alvarez_home, capsys):
        """With the default (``pre_update_backup: false``), ``alvarez update``
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((alvarez_home / "backups").glob("pre-update-*.zip")) \
            if (alvarez_home / "backups").exists() else True

    def test_no_backup_flag_skips(self, alvarez_home, capsys):
        from alvarez_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (alvarez_home / "backups").exists() or not list(
            (alvarez_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, alvarez_home, capsys):
        ...
        import yaml
        (alvarez_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("alvarez_cli.config"):
    ...
    def test_config_disabled_is_silent(self, alvarez_home, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, alvarez_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (alvarez claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``alvarez claw migrate`` runs before mutating ~/.alvarez/."""
    ...
    def test_creates_backup_under_backups_dir(self, alvarez_home):
        from alvarez_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(alvarez_home=alvarez_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `alvarez
        # import` and the update-backup listing both pick them up.
        assert out.parent == alvarez_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, alvarez_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``alvarez backup`` / ``create_pre_update_backup`` — no drift."""
        from alvarez_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(alvarez_home=alvarez_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_alvarez_import(self, alvarez_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Alvarez
        backup — `alvarez import` should accept it."""
        from alvarez_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(alvarez_home=alvarez_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, alvarez_home):
        from alvarez_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(alvarez_home=alvarez_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(alvarez_home=alvarez_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, alvarez_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(alvarez_home=alvarez_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((alvarez_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_alvarez_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.alvarez yet — nothing to back up."""
        from alvarez_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(alvarez_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, alvarez_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from alvarez_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(alvarez_home=alvarez_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(alvarez_home=alvarez_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`alvarez update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, alvarez_home: Path, label="pre-update"):
        from alvarez_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, alvarez_home=alvarez_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from alvarez_cli.backup import restore_cron_jobs_if_emptied
        alvarez_home = tmp_path / ".alvarez"
        jobs_path = alvarez_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(alvarez_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, alvarez_home=alvarez_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = alvarez_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, alvarez_home=alvarez_home) is None
        assert restore_cron_jobs_if_emptied("", alvarez_home=alvarez_home) is None
...
# location, NOT under ALVAREZ_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, alvarez_home: Path) -> None:
        alvarez_home.mkdir(parents=True, exist_ok=True)
        (alvarez_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (alvarez_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (alvarez_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        self._make_min_tree(alvarez_home)
        # External provider state living OUTSIDE ALVAREZ_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ALVAREZ_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        alvarez_home = dst_home / ".alvarez"
        alvarez_home.mkdir()
        ...
        # External state did NOT leak into ALVAREZ_HOME.
        assert not (alvarez_home / "_external").exists()
