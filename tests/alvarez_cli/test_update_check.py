"""Tests for the update check mechanism in alvarez_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("alvarez_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "alvarez_cli" / "banner.py"
    ...
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    monkeypatch.delenv("ALVAREZ_REVISION", raising=False)
    with patch("alvarez_cli.banner.subprocess.run") as mock_run, \
         patch("alvarez_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    with patch("alvarez_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    literal so the test protects the SSH-avoidance mechanism itself,
    independent of exactly which URL is configured.
    """
    import alvarez_cli.banner as banner
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:alvarez-agent/alvarez-agent.git\n")
    ...
    with patch("alvarez_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/alvarez-agent/alvarez-agent.git\n")
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    with patch("alvarez_cli.banner.subprocess.run") as mock_run:
        with patch("alvarez_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    ...
    # Point ALVAREZ_HOME at a temp dir with no alvarez-agent/.git
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    with patch("alvarez_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    ALVAREZ_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `alvarez update` correctly refusing to run there. The guard
    ...
    with patch("alvarez_cli.config.detect_install_method", return_value="docker"), \
         patch("alvarez_cli.banner.subprocess.run") as mock_run, \
         patch("alvarez_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("alvarez_cli.config.detect_install_method", return_value="pip"), \
         patch("alvarez_cli.banner.subprocess.run") as mock_run, \
         patch("alvarez_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from alvarez_cli.main import _invalidate_update_cache

    # Build a fake ~/.alvarez with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ALVAREZ_HOME": str(default_home)}):
