"""Behavior tests for alvarez_cli.inventory.
...
from alvarez_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("alvarez_cli.models.provider_model_ids") as mock_pm:
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # alvarez model fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `alvarez model`")
        )
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("alvarez_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from alvarez_cli import model_switch

    with patch("alvarez_cli.models.clear_provider_models_cache") as clear:
