...
`ALVAREZ_REDACT_SECRETS` at module-import time. `alvarez_cli/main.py` at
...
in .env), the toggle was silently ignored in both `alvarez chat` and
`alvarez gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `ALVAREZ_REDACT_SECRETS`
env var in `alvarez_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports alvarez_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("ALVAREZ_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import alvarez_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('ALVAREZ_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["ALVAREZ_HOME"] = str(alvarez_home)
    env.pop("ALVAREZ_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env ALVAREZ_REDACT_SECRETS takes precedence over config.yaml."""
    alvarez_home = tmp_path / ".alvarez"
    alvarez_home.mkdir()
    (alvarez_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (alvarez_home / ".env").write_text("ALVAREZ_REDACT_SECRETS=true\n")
