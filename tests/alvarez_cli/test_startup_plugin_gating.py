...
``alvarez_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``alvarez --help``, ``alvarez version``,
``alvarez logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``alvarez --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from alvarez_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["alvarez", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["alvarez", "-w", "--tui"], None),
        (["alvarez", "version"], "version"),
        (["alvarez", "--tui", "chat"], "chat"),
        (["alvarez", "-w", "logs"], "logs"),
        (["alvarez", "chat", "hello world"], "chat"),
        (["alvarez", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["alvarez", "-m", "gpt5", "chat"], "chat"),
        (["alvarez", "--model", "gpt5", "chat", "hi"], "chat"),
        (["alvarez", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["alvarez", "-z", "hello world"], None),
        (["alvarez", "-z", "hello", "chat"], "chat"),
        (["alvarez", "--model=gpt5", "chat"], "chat"),     # inline form
        (["alvarez", "--", "chat"], "chat"),               # -- terminator
        (["alvarez", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["alvarez", "some-plugin-cmd"], "some-plugin-cmd"),
        (["alvarez", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["alvarez"],                          # bare → chat
        ["alvarez", "--help"],                # top-level help
        ["alvarez", "-h"],
        ["alvarez", "version"],               # known built-in
        ["alvarez", "logs"],
        ["alvarez", "gateway", "run"],
        ["alvarez", "--tui"],
        ["alvarez", "-w", "--tui"],
        ["alvarez", "chat", "hi"],
        ["alvarez", "help"],                  # accepted built-in-ish
        ["alvarez", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["alvarez", "meet", "join"],          # potential google_meet plugin
        ["alvarez", "honcho", "status"],      # potential memory plugin
        ["alvarez", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``alvarez help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"alvarez_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
