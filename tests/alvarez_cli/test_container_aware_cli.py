...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ALVAREZ_HOME with .container-mode file."""
    alvarez_home = tmp_path / ".alvarez"
    alvarez_home.mkdir()
    monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
    monkeypatch.delenv("ALVAREZ_DEV", raising=False)

    container_mode = alvarez_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=alvarez-agent\n"
        "exec_user=alvarez\n"
        "alvarez_bin=/data/current-package/bin/alvarez\n"
    )
    return alvarez_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("alvarez_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "alvarez-agent"
    assert info["exec_user"] == "alvarez"
    assert info["alvarez_bin"] == "/data/current-package/bin/alvarez"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("alvarez_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_alvarez_dev(container_env, monkeypatch):
    """Returns None when ALVAREZ_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ALVAREZ_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_alvarez_dev_zero(container_env, monkeypatch):
    """ALVAREZ_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ALVAREZ_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        alvarez_home = Path(tmpdir) / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("alvarez_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_alvarez_home": lambda: alvarez_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ALVAREZ_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=alvarez-custom\n"
        "exec_user=myuser\n"
        "alvarez_bin=/opt/alvarez/bin/alvarez\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "alvarez-custom"
    assert info["exec_user"] == "myuser"
    assert info["alvarez_bin"] == "/opt/alvarez/bin/alvarez"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from alvarez_cli.main import _exec_in_container
