...
``alvarez setup`` / ``alvarez model`` when an API key already exists in ``.env``.
...
def _run_prompt(existing_key, choice, new_key="", provider_id="", pconfig_name="deepseek"):
    ...
    with patch("builtins.input", return_value=choice), \
         patch("alvarez_cli.secret_prompt.masked_secret_prompt", return_value=new_key):
...
def test_replace_saves_new_key(profile_env):
    from alvarez_cli.config import get_env_value, save_env_value
