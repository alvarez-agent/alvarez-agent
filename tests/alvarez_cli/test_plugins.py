"""Tests for the Alvarez plugin system (alvarez_cli.plugins)."""
...
from alvarez_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<alvarez_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<alvarez_home>/plugins/``; we derive
    ``<alvarez_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ALVAREZ_HOME>/config.yaml.
        # Config is always read from ALVAREZ_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        alvarez_home_str = os.environ.get("ALVAREZ_HOME")
        if alvarez_home_str:
            alvarez_home = Path(alvarez_home_str)
        else:
            alvarez_home = base.parent
        alvarez_home.mkdir(parents=True, exist_ok=True)
        cfg_path = alvarez_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.alvarez/plugins/ are discovered."""
        plugins_dir = tmp_path / "alvarez_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path / "alvarez_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("alvarez_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ALVAREZ_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".alvarez" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        alvarez_home = tmp_path / "alvarez_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under alvarez_plugins.<name>."""
        plugins_dir = tmp_path / "alvarez_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path / "alvarez_test"))

        # Clean up any prior namespace module
        sys.modules.pop("alvarez_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "alvarez_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="alvarez_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "alvarez_cli.plugins.invoke_hook",
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("alvarez_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("alvarez_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from alvarez_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from alvarez_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("alvarez_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import alvarez_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `alvarez plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("alvarez_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / "alvarez_test"
        plugins_dir = alvarez_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "alvarez_test" / "plugins"
        # Set ALVAREZ_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("alvarez_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("alvarez_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("alvarez_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("alvarez_cli.plugins.PluginContext.dispatch_tool.__module__", "alvarez_cli.plugins"):
...
class TestPluginDebugLogging:
    """ALVAREZ_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ALVAREZ_PLUGINS_DEBUG", raising=False)
        from alvarez_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ALVAREZ_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ALVAREZ_PLUGINS_DEBUG", "1")
        from alvarez_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from ALVAREZ_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """ALVAREZ_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """ALVAREZ_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".alvarez" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALVAREZ_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".alvarez" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALVAREZ_HOME", str(prof))
        ctx = self._ctx()
