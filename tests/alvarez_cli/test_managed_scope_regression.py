...
def alvarez_home(tmp_path, monkeypatch):
    ...
    # /etc/alvarez on the dev/CI box can't influence the test.
    monkeypatch.setenv("ALVAREZ_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(alvarez_home, monkeypatch):
    from alvarez_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(alvarez_home, monkeypatch):
    from alvarez_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(alvarez_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_alvarez_dotenv(alvarez_home=str(home))
