"""Tests for alvarez_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate ALVAREZ_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from alvarez_cli import model_catalog
    importlib.reload(model_catalog)
    # No hosted catalog ships by default; give tests a configured URL so the
    # fetch pipeline is exercised.
    monkeypatch.setattr(
        model_catalog, "DEFAULT_CATALOG_URL", "https://catalog.test/model-catalog.json"
    )
    yield home
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from alvarez_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://catalog.test/model-catalog.json"
    FALLBACK = "https://fallback.test/model-catalog.json"
...
class TestNoUrlConfigured:
    def test_empty_url_skips_remote_fetch(self, isolated_home, monkeypatch):
        """No configured catalog URL → no network fetch, empty catalog."""
        from alvarez_cli import model_catalog

        monkeypatch.setattr(model_catalog, "DEFAULT_CATALOG_URL", "")
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in alvarez_cli.models."""
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            side_effect=lambda slug, **kw: expected if slug == "deepseek" else [],
        ):
            ...
            zero = list_authenticated_providers(
                current_provider="deepseek", max_models=0
            )
...
#
# History: qwen/qwen3.6-plus was added to a curated provider list in commit
