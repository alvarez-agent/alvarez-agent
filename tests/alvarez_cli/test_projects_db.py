"""Tests for the per-profile Projects store (alvarez_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Alvarez Agent", folders=["/tmp/alvarez"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "alvarez-agent"
    assert proj.name == "Alvarez Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/alvarez"
    assert [f.path for f in proj.folders] == ["/tmp/alvarez"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "alvarez-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Alvarez Agent")
    pdb.create_project(conn, name="Alvarez Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["alvarez-agent", "alvarez-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ALVAREZ_HOME.
...
def test_db_path_under_alvarez_home():
    # Resolves under ALVAREZ_HOME (set by the autouse isolation fixture).
