"""Tests for IRC gateway configuration via `alvarez setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Alvarez install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Alvarez install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#alvarez")
            monkeypatch.setenv("IRC_NICKNAME", "alvarez-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `alvarez setup gateway` experience with IRC present."""
