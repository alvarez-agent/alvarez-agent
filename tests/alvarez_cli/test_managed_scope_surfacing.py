"""Surfacing tests — managed scope shown in `config show` and `alvarez doctor`."""
import pytest
...
def test_config_show_flags_managed(homes, capsys):
    from alvarez_cli.config import show_config
...
def test_doctor_reports_managed_scope(homes, capsys):
    ...
    from alvarez_cli import doctor
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("ALVAREZ_MANAGED_DIR", str(tmp_path / "nope"))
    from alvarez_cli import managed_scope, doctor
