"""Tests for `alvarez chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("ALVAREZ_SAFE_MODE", "ALVAREZ_IGNORE_USER_CONFIG", "ALVAREZ_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in alvarez_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("ALVAREZ_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under ALVAREZ_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("ALVAREZ_SAFE_MODE", "1")
        from alvarez_cli.plugins import PluginManager
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("ALVAREZ_SAFE_MODE", raising=False)
        from alvarez_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under ALVAREZ_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `alvarez chat`."""
