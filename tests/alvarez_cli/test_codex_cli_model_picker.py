...
Covers:
 - Normal path (tokens already in Alvarez auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Alvarez
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `alvarez auth openai-codex`. The old
...
@pytest.fixture()
def alvarez_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Alvarez auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(alvarez_auth_only_env):
    """openai-codex appears when tokens are already in Alvarez auth store."""
...
def test_codex_picker_uses_live_codex_catalog(alvarez_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Alvarez
    auth store."""
    alvarez_home = tmp_path / ".alvarez"
    alvarez_home.mkdir()
    ...
    (alvarez_home / "auth.json").write_text(
