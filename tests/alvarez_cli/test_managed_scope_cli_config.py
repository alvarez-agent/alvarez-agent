...
alvarez_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `alvarez config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_alvarez_home = get_alvarez_home()`` at import time (module
    singleton), so monkeypatching ALVAREZ_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ALVAREZ_HOME.)
    """
    import cli

    cli._alvarez_home = home
