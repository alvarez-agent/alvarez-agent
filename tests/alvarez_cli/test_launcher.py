"""Tests for the top-level `./alvarez` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./alvarez` should use `alvarez_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "alvarez"
    called = []

    fake_main_module = types.ModuleType("alvarez_cli.main")

    def fake_main():
        called.append("alvarez_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "alvarez_cli.main", fake_main_module)
    ...
    assert called == ["alvarez_cli.main"]
