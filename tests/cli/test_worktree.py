...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("alvarez/alvarez-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned alvarez/* and pr-* branches."""

    def test_prunes_orphaned_alvarez_branch(self, git_repo):
        """alvarez/alvarez-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "alvarez/alvarez-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "alvarez/alvarez-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "alvarez/alvarez-deadbeef" in result.stdout
        ...
        assert "alvarez/alvarez-deadbeef" in orphaned
        ...
        assert "alvarez/alvarez-deadbeef" not in result.stdout
