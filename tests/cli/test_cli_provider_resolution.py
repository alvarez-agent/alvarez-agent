...
def test_alvarez_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in AlvarezCLI.__init__")

    monkeypatch.setattr("alvarez_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("alvarez_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.AlvarezCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("alvarez_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("alvarez_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.AlvarezCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("alvarez_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "alvarez_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.AlvarezCLI(compact=True, max_turns=1)
...
def _seed_stale_custom_model(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "alvarez"
    config_home.mkdir()
    monkeypatch.setenv("ALVAREZ_HOME", str(config_home))
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "alvarez_cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "alvarez_cli.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("alvarez_cli.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
        "alvarez_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: "anthropic/claude-sonnet-4.6",
    )
    monkeypatch.setattr("alvarez_cli.auth.deactivate_provider", lambda: None)

    alvarez_main._model_flow_openrouter({}, current_model="glm-5.2")
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alvarez_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "alvarez_cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "alvarez_cli.auth._prompt_model_selection",
    ...
    alvarez_main._model_flow_anthropic({}, current_model="glm-5.2")
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.AlvarezCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.AlvarezCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("alvarez_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("alvarez_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("alvarez_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(alvarez_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    alvarez_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "alvarez_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("alvarez_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("alvarez_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("alvarez_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("alvarez_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "alvarez_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("alvarez_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    alvarez_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("alvarez_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("alvarez_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "alvarez_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    monkeypatch.setattr("alvarez_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("alvarez_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "alvarez_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("alvarez_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    alvarez_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_auto_provider_name_localhost():
    from alvarez_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from alvarez_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "alvarez_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("alvarez_cli.config.save_config", _save)
