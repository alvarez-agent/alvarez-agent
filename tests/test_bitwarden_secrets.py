...
binary download are exercised manually by `alvarez secrets bitwarden
...
def alvarez_home(tmp_path, monkeypatch):
    """Point Alvarez at an isolated home directory."""
    home = tmp_path / ".alvarez"
    home.mkdir()
    monkeypatch.setenv("ALVAREZ_HOME", str(home))
    # Some modules cache get_alvarez_home; clear if needed.
    import alvarez_constants
    if hasattr(alvarez_constants, "_ALVAREZ_HOME_CACHE"):
        alvarez_constants._ALVAREZ_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(alvarez_home, monkeypatch):
...
def test_install_bws_happy_path(alvarez_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(alvarez_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(alvarez_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from alvarez_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under alvarez_home/cache/."""
    home = tmp_path / ".alvarez"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
