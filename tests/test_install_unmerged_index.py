...
The ``alvarez update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m alvarez_cli.main gateway run`` — image name
    ``pythonw``, not ``alvarez.exe`` — so the ``taskkill /IM alvarez.exe`` guard
    ...
    # The alvarez.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM alvarez.exe' in text
