...
Build the real image and verify at runtime:
  ...
  1. /opt/alvarez is not writable by the alvarez user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and ALVAREZ_DISABLE_LAZY_INSTALLS are set
  3. /opt/alvarez/.install_method contains "docker" (code-scoped stamp)
  4. $ALVAREZ_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $ALVAREZ_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_alvarez(
    built_image: str, container_name: str,
) -> None:
    """The alvarez user must not be able to modify /opt/alvarez.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/alvarez as the alvarez user
        "touch /opt/alvarez/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"alvarez user can write to /opt/alvarez (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/alvarez/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"alvarez user can write to /opt/alvarez/.venv: {r.stdout}"
    )


def test_alvarez_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    ALVAREZ_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$ALVAREZ_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"ALVAREZ_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/alvarez/.install_method (code-scoped), NOT in $ALVAREZ_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/alvarez/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/alvarez/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $ALVAREZ_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        f"$ALVAREZ_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $ALVAREZ_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $ALVAREZ_HOME was not healed on boot: "
        f"{r.stdout}"
    )
