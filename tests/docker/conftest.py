...
Override the image with ``ALVAREZ_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ALVAREZ_TEST_IMAGE", "alvarez-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ALVAREZ_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ALVAREZ_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged alvarez user
# ---------------------------------------------------------------------------
#
# Background: every Alvarez runtime path inside the container drops to UID
# 10000 (the ``alvarez`` user) via ``s6-setuidgid alvarez``. ``docker exec``
...
# fails (PermissionError swallowed) for alvarez, so a test that ran as root
...
# helpers below run every probe under ``-u alvarez`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: alvarez).
