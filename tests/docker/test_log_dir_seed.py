"""Runtime smoke test for Docker $ALVAREZ_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the alvarez user after container boot.
...
registered later runs its log service as the dropped alvarez user and
...
def test_logs_gateways_seeded_and_alvarez_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by alvarez after boot."""
    ...
    assert "logs=alvarez" in r.stdout, (
        f"logs/ not owned by alvarez: {r.stdout}"
    )
    assert "gateways=alvarez" in r.stdout, (
        f"logs/gateways/ not owned by alvarez: {r.stdout}"
    )
