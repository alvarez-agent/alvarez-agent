...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$ALVAREZ_HOME" = "/opt/data" && '
        'test "$ALVAREZ_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_alvarez_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        r"""
        set -eu
        /opt/alvarez/.venv/bin/python - <<'PY'
        from pathlib import Path

        install_file = Path("/opt/alvarez/agent/message_sanitization.py")
        try:
