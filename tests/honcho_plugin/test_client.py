...
from alvarez_cli.profiles import _get_default_alvarez_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "alvarez"
        assert config.workspace_id == "alvarez"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.alvarez/honcho.json
        monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"alvarez": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"alvarez": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"alvarez": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="alvarez-agent"
        ):
            result = config.resolve_session_name("/home/user/alvarez-agent/subdir")
        assert result == "alvarez-agent"
...
class TestResolveConfigPath:
    def test_prefers_alvarez_home_when_exists(self, tmp_path):
        alvarez_home = tmp_path / "alvarez"
        alvarez_home.mkdir()
        local_cfg = alvarez_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ALVAREZ_HOME points at ~/.alvarez/profiles/<name>, so
        # _get_default_alvarez_home() must resolve back to ~/.alvarez — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".alvarez"
        ...
        assert _get_default_alvarez_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_alvarez_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".alvarez".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".alvarez"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "alvarez_coder"
        assert profile_host_key("default") == "alvarez"

    def test_default_returns_alvarez(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ALVAREZ_HONCHO_HOST", None)
            os.environ.pop("ALVAREZ_HOME", None)
            assert resolve_active_host() == "alvarez"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ALVAREZ_HONCHO_HOST": "alvarez.coder"}):
            assert resolve_active_host() == "alvarez.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ALVAREZ_HONCHO_HOST", None)
            with patch("alvarez_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "alvarez_coder"

    def test_default_profile_returns_alvarez(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ALVAREZ_HONCHO_HOST", None)
            with patch("alvarez_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "alvarez"

    def test_custom_profile_returns_alvarez(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ALVAREZ_HONCHO_HOST", None)
            with patch("alvarez_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "alvarez"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ALVAREZ_HONCHO_HOST", None)
            # Temporarily remove alvarez_cli.profiles to simulate import failure
            saved = sys.modules.get("alvarez_cli.profiles")
            sys.modules["alvarez_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "alvarez"
            finally:
                if saved is not None:
                    sys.modules["alvarez_cli.profiles"] = saved
                else:
                    sys.modules.pop("alvarez_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="alvarez_coder")
        assert config.host == "alvarez_coder"
        assert config.workspace_id == "alvarez"  # shared workspace
        assert config.ai_peer == "alvarez_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="alvarez")
        assert config.host == "alvarez"
        assert config.workspace_id == "alvarez"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "alvarez": {"aiPeer": "alvarez", "peerName": "alice"},
                "alvarez_coder": {
                    "aiPeer": "alvarez_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="alvarez_coder", config_path=config_file,
        )
        assert config.host == "alvarez_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "alvarez_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="alvarez_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "alvarez_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "alvarez.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="alvarez_dreamer",
            config_path=config_file,
        )
        assert config.host == "alvarez_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "alvarez_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"alvarez": {"enabled": True, "aiPeer": "alvarez"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"alvarez": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_alvarez_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="alvarez",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("alvarez_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("alvarez_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_alvarez_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="alvarez",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("alvarez_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"alvarez": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
