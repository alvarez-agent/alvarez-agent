"""Shared fixtures for the alvarez-agent test suite.
...
2. **Isolated ALVAREZ_HOME.** ALVAREZ_HOME points to a per-test tempdir so
   code reading ``~/.alvarez/*`` via ``get_alvarez_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".alvarez"`` instead of the canonical
   ``get_alvarez_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ALVAREZ_SESSION_* inheritance** — the agent's current gateway
...
# ALVAREZ_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ALVAREZ_BEHAVIORAL_VARS = frozenset({
    "ALVAREZ_YOLO_MODE",
    "ALVAREZ_INTERACTIVE",
    "ALVAREZ_QUIET",
    "ALVAREZ_TOOL_PROGRESS",
    "ALVAREZ_TOOL_PROGRESS_MODE",
    "ALVAREZ_MAX_ITERATIONS",
    "ALVAREZ_SESSION_PLATFORM",
    "ALVAREZ_SESSION_CHAT_ID",
    "ALVAREZ_SESSION_CHAT_NAME",
    "ALVAREZ_SESSION_THREAD_ID",
    "ALVAREZ_SESSION_SOURCE",
    "ALVAREZ_SESSION_KEY",
    "ALVAREZ_GATEWAY_SESSION",
    "ALVAREZ_CRON_SESSION",
    "_ALVAREZ_GATEWAY",
    "ALVAREZ_PLATFORM",
    "ALVAREZ_MODEL",
    "ALVAREZ_INFERENCE_MODEL",
    "ALVAREZ_INFERENCE_PROVIDER",
    "ALVAREZ_TUI_PROVIDER",
    "ALVAREZ_MANAGED",
    "ALVAREZ_MANAGED_DIR",
    "ALVAREZ_DEV",
    "ALVAREZ_CONTAINER",
    ...
    "ALVAREZ_TIMEZONE",
    "ALVAREZ_REDACT_SECRETS",
    "ALVAREZ_BACKGROUND_NOTIFICATIONS",
    "ALVAREZ_EXEC_ASK",
    "ALVAREZ_HOME_MODE",
    "ALVAREZ_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.alvarez/kanban.db instead of the per-test ALVAREZ_HOME.
    "ALVAREZ_KANBAN_DB",
    "ALVAREZ_KANBAN_BOARD",
    "ALVAREZ_KANBAN_HOME",
    "ALVAREZ_KANBAN_WORKSPACES_ROOT",
    "ALVAREZ_KANBAN_LOGS_ROOT",
    "ALVAREZ_KANBAN_TASK",
    "ALVAREZ_KANBAN_WORKSPACE",
    "ALVAREZ_KANBAN_RUN_ID",
    "ALVAREZ_KANBAN_CLAIM_LOCK",
    "ALVAREZ_KANBAN_DISPATCH_IN_GATEWAY",
    "ALVAREZ_TENANT",
    ...
    "ALVAREZ_DASHBOARD_OAUTH_CLIENT_ID",
    "ALVAREZ_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ALVAREZ_HOME to per-test tempdirs so code that
    reads ``~/.alvarez/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ALVAREZ_* vars that could change test semantics.
    for name in _ALVAREZ_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ALVAREZ_HOME to a per-test tempdir. Code that reads
    #    ``~/.alvarez/*`` via ``get_alvarez_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.alvarez/*`` via
    #    ``Path.home() / ".alvarez"`` instead of ``get_alvarez_home()``
    #    is a bug to fix at the callsite.
    fake_alvarez_home = tmp_path / "alvarez_test"
    fake_alvarez_home.mkdir()
    (fake_alvarez_home / "sessions").mkdir()
    (fake_alvarez_home / "cron").mkdir()
    (fake_alvarez_home / "memories").mkdir()
    (fake_alvarez_home / "skills").mkdir()
    monkeypatch.setenv("ALVAREZ_HOME", str(fake_alvarez_home))
    ...
    try:
        import alvarez_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal alvarez config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``alvarez-gateway`` process
...
# correlated with a ``tests/alvarez_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> alvarez-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart alvarez-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting alvarez/python patterns are also blocked.
    """
    ...
    def _matches_alvarez_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ALVAREZ_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_alvarez_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch alvarez-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m alvarez_cli.main".
                if (
                    "alvarez" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live alvarez-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting alvarez/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `alvarez update` (or the
        # equivalent `python -m alvarez_cli.main update`).  These commands
        ...
        if "update" in low and (
            # alvarez update / alvarez update --gateway / setsid bash -c ... alvarez update
            ("alvarez" in low and "update" in low.split())
            or
            # python -m alvarez_cli.main update --gateway
            ("alvarez_cli" in low and "update" in low.split())
            or
            # venv/bin/alvarez update  (absolute path variant used in tests)
            (".venv/bin/alvarez" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`alvarez update` against the real checkout, fetching "
