...
class TestToolsetConsistency:
    ...
    def test_alvarez_platforms_share_core_tools(self):
        """All alvarez-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on alvarez-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["alvarez-cli", "alvarez-telegram"]
...
class TestDefaultPlatformWebSearchCoverage:
    ...
    def test_alvarez_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("alvarez-api-server")
