...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"alvarez-agent[{extra}]" in spec
        ]
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `alvarez update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid alvarez update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `alvarez update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `alvarez update` downgrades the "
