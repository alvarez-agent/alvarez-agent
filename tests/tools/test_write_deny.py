...
class TestWriteDenyExactPaths:
    ...
    def test_alvarez_env(self):
        # ``.env`` under the active ALVAREZ_HOME (profile-aware, not just
        # ``~/.alvarez``) must be write-denied. The hermetic test conftest
        # points ALVAREZ_HOME at a tempdir — resolve via get_alvarez_home()
        # to match the denylist.
        from alvarez_constants import get_alvarez_home
        path = str(get_alvarez_home() / ".env")
        assert _is_write_denied(path) is True

    def test_alvarez_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.alvarez/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "alvarez_root"
        ...
        # Sanity check: ALVAREZ_HOME does point to the profile dir, not the root.
        from alvarez_constants import get_alvarez_home, get_default_alvarez_root
        assert get_alvarez_home() == profile_home
        assert get_default_alvarez_root() == root
...
class TestWriteAllowed:
    ...
    def test_alvarez_control_files_requested_writable(self):
        from alvarez_constants import get_alvarez_home
