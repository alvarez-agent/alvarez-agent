...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        alvarez_home.mkdir()
        (alvarez_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ALVAREZ_HOME": str(alvarez_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(alvarez_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.alvarez/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        alvarez_home = tmp_path / ".alvarez"
        skills_dir = alvarez_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.alvarez/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        alvarez_home = tmp_path / ".alvarez"
        (alvarez_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ALVAREZ_HOME": str(alvarez_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.alvarez")

        assert mounts[0]["container_path"] == "/home/user/.alvarez/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.alvarez/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.alvarez/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ALVAREZ_HOME."""
        monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path / ".alvarez"))
        (tmp_path / ".alvarez").mkdir()

        # Create a sensitive file one level above alvarez_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside alvarez_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ALVAREZ_HOME must still be registered."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        (alvarez_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_alvarez_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ALVAREZ_HOME must be allowed."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        subdir = alvarez_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ALVAREZ_HOME pointing outside must be rejected."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))

        # Create a sensitive file outside alvarez_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside alvarez_home pointing outside
        symlink = alvarez_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ALVAREZ_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, alvarez_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ALVAREZ_HOME."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
        ...
        sensitive.write_text("{}")
        self._write_config(alvarez_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(alvarez_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ALVAREZ_HOME via config must still mount."""
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))

        (alvarez_home / "oauth.json").write_text("{}")
        self._write_config(alvarez_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        (alvarez_home / "cache" / "documents").mkdir(parents=True)
        (alvarez_home / "cache" / "audio").mkdir(parents=True)
        (alvarez_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.alvarez/cache/documents" in paths
        assert "/root/.alvarez/cache/audio" in paths
        assert "/root/.alvarez/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.alvarez/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_alvarez_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        alvarez_home = tmp_path / ".alvarez"
        alvarez_home.mkdir()
        # Use legacy dir name with content — get_alvarez_dir prefers
        # populated old over new.
        legacy_doc = alvarez_home / "document_cache"
        legacy_img = alvarez_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(alvarez_home / "document_cache") in host_paths
        assert str(alvarez_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.alvarez/cache/documents" in container_paths
        assert "/root/.alvarez/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        alvarez_home = tmp_path / ".alvarez"
        img_dir = alvarez_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.alvarez/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.alvarez")
            == "/home/agent/.alvarez/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        alvarez_home = tmp_path / ".alvarez"
        (alvarez_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ALVAREZ_HOME", str(alvarez_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(alvarez_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        doc_dir = alvarez_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        alvarez_home = tmp_path / ".alvarez"
        ss_dir = alvarez_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.alvarez/cache/screenshots/session_abc/screen1.png"
