...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("alvarez_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("alvarez_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ALVAREZ_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_alvarez_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.alvarez/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_alvarez_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ALVAREZ_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_alvarez_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$ALVAREZ_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestAlvarezConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.alvarez/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.alvarez/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.alvarez/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.alvarez/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.alvarez/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.alvarez/config.yaml")
        assert dangerous is True
        assert "alvarez config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.alvarez/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_alvarez_home_config(self):
    ...
    def test_sed_in_place_absolute_alvarez_home_env(self):
        env_path = get_alvarez_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_alvarez_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ALVAREZ_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.alvarez/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_alvarez_home_config(self):
        config_path = get_alvarez_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.alvarez/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_alvarez_home_env(self):
        env_path = get_alvarez_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.alvarez/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.alvarez/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.alvarez/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.alvarez/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.alvarez/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Alvarez config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_alvarez_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $ALVAREZ_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.alvarez/config.yaml/.env must require approval — the
    ...
    def test_cp_to_alvarez_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.alvarez/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Alvarez-home prefixes must fold to ~/ and
    ~/.alvarez/ in dangerous-command detection.
    ...
    SSH, and Alvarez config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_alvarez_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ALVAREZ_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_alvarez_home_config_folds(self, monkeypatch):
        # Alvarez home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("ALVAREZ_HOME", r"C:\Users\tester\.alvarez")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.alvarez\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.alvarez/alvarez-agent && source venv/bin/activate && python -m alvarez_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m alvarez_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m alvarez_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "alvarez_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart alvarez-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_alvarez_gateway_stop_detected(self):
        cmd = "alvarez gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_alvarez_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `alvarez` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "alvarez -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_alvarez_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "alvarez --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_alvarez_gateway_multiple_flags_detected(self):
        cmd = "alvarez -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_alvarez_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "alvarez -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_alvarez_gateway_start_not_flagged(self):
        cmd = "alvarez gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_alvarez_detected(self):
        """pkill targeting alvarez/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_alvarez_detected(self):
        cmd = "killall alvarez"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep alvarez) bypasses the pkill/killall name-matching
    ...
    def test_pkill_alvarez_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof alvarez)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof alvarez_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof alvarez`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Alvarez service
    label achieves the same effect as `alvarez gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_alvarez_detected(self):
        cmd = "launchctl stop ai.alvarez.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "alvarez" in desc.lower()

    def test_launchctl_kickstart_alvarez_detected(self):
        cmd = "launchctl kickstart -k system/ai.alvarez.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_alvarez_detected(self):
        cmd = "launchctl bootout system/ai.alvarez.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_alvarez_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.alvarez.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Alvarez label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ALVAREZ_GATEWAY_SESSION", "ALVAREZ_CRON_SESSION",
                      "ALVAREZ_YOLO_MODE",
                      "ALVAREZ_SESSION_KEY", "ALVAREZ_INTERACTIVE")
        }
        os.environ.pop("ALVAREZ_YOLO_MODE", None)
        os.environ.pop("ALVAREZ_INTERACTIVE", None)
        # ALVAREZ_CRON_SESSION takes priority over ALVAREZ_GATEWAY_SESSION in
        ...
        os.environ.pop("ALVAREZ_CRON_SESSION", None)
        os.environ["ALVAREZ_GATEWAY_SESSION"] = "1"
        os.environ["ALVAREZ_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("alvarez_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ALVAREZ_INTERACTIVE": "1"}, clear=False):
