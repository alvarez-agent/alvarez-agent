...
def test_alvarez_provider_subclass_exists():
    """AlvarezMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ALVAREZ_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ALVAREZ_PROVIDER_CLS is not None
    assert issubclass(_ALVAREZ_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_alvarez_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AlvarezMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ALVAREZ_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ALVAREZ_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ALVAREZ_PROVIDER_CLS is not None
    assert isinstance(provider, _ALVAREZ_PROVIDER_CLS)
    assert provider._alvarez_server_name == "srv"
