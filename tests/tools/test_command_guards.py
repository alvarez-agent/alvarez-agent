...
def _clean_state():
    ...
    saved = {}
    for k in ("ALVAREZ_INTERACTIVE", "ALVAREZ_GATEWAY_SESSION", "ALVAREZ_EXEC_ASK", "ALVAREZ_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ALVAREZ_INTERACTIVE"] = "1"
        session_key = os.getenv("ALVAREZ_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ALVAREZ_INTERACTIVE or ALVAREZ_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
