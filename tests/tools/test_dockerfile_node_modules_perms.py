...
Older images made /opt/alvarez/ui-tui and /opt/alvarez/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ALVAREZ_TUI_DIR=/opt/alvarez/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R alvarez:alvarez /opt/alvarez/ui-tui" not in text
    assert "chown -R alvarez:alvarez /opt/alvarez/node_modules" not in text
