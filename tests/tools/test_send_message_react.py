...
def test_react_dispatches_to_add_reaction():
    ...
    with patch("gateway.run._gateway_runner_ref", lambda: _runner_with(adapter)):
        result = _call(
            {"action": "react", "target": "telegram:12345", "emoji": "❤️"}
        )
    assert result["success"] is True
    assert adapter.calls == [("add", "12345", "❤️", None)]
...
def test_unreact_dispatches_to_remove_reaction():
    ...
    assert result["success"] is True
    assert adapter.calls == [("remove", "12345", "msg-9")]


def test_react_requires_emoji():
    result = _call({"action": "react", "target": "telegram:12345"})
...
def test_unreact_does_not_require_emoji():
    adapter = _FakePhotonAdapter()
    with patch("gateway.run._gateway_runner_ref", lambda: _runner_with(adapter)):
        result = _call({"action": "unreact", "target": "telegram:12345"})
    assert result["success"] is True
    assert adapter.calls == [("remove", "12345", None)]
...
def test_react_unsupported_platform_adapter():
    ...
    with patch("gateway.run._gateway_runner_ref", lambda: _runner_with(adapter)):
        result = _call(
            {"action": "react", "target": "telegram:12345", "emoji": "👍"}
        )
    assert result.get("success") is not True
