...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ALVAREZ_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ALVAREZ_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("ALVAREZ_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ALVAREZ_CRON_SESSION", "1")
    monkeypatch.delenv("ALVAREZ_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_alvarez_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ALVAREZ_HOME": "/h", "ALVAREZ_PROFILE": "p",
        "ALVAREZ_CONFIG": "/c.yaml", "ALVAREZ_ENV": "/e",
        # other ALVAREZ_* → dropped (broad prefix removed)
        "ALVAREZ_BASE_URL": "https://x", "ALVAREZ_INTERACTIVE": "1",
        "ALVAREZ_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ALVAREZ_HOME", "ALVAREZ_PROFILE", "ALVAREZ_CONFIG", "ALVAREZ_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ALVAREZ_BASE_URL", "ALVAREZ_INTERACTIVE", "ALVAREZ_KANBAN_DB",
...
def test_env_scrub_logs_dropped_alvarez_vars(caplog):
    """Dropping a non-allowlisted, non-secret ALVAREZ_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ALVAREZ_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ALVAREZ_HOME": "/h",          # allowlisted → kept, not logged
        "ALVAREZ_BASE_URL": "https://x",   # dropped → logged
        "ALVAREZ_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ALVAREZ_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ALVAREZ_HOME" in out and "PATH" in out
    assert "ALVAREZ_BASE_URL" not in out and "ALVAREZ_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ALVAREZ_BASE_URL" in msgs and "ALVAREZ_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ALVAREZ_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ALVAREZ_* vars."""
    import logging
