...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._alvarez_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(alvarez_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(alvarez_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_alvarez_bin(self):
        """After _INSTALL_FAILED, manual install in ALVAREZ_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        alvarez_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(alvarez_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(alvarez_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._alvarez_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == alvarez_bin
            assert _tirith_mod._resolved_path == alvarez_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ALVAREZ_HOME isolation
# ---------------------------------------------------------------------------

class TestAlvarezHomeIsolation:
    def test_alvarez_bin_dir_respects_alvarez_home(self):
        """_alvarez_bin_dir must use ALVAREZ_HOME, not hardcoded ~/.alvarez."""
        from tools.tirith_security import _alvarez_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ALVAREZ_HOME": tmpdir}):
            result = _alvarez_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_alvarez_home(self):
        """_failure_marker_path must use ALVAREZ_HOME, not hardcoded ~/.alvarez."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ALVAREZ_HOME": "/custom/alvarez"}):
            result = _failure_marker_path()
        assert result == "/custom/alvarez/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ALVAREZ_HOME; verify it's active."""
        alvarez_home = os.getenv("ALVAREZ_HOME")
        assert alvarez_home is not None, "ALVAREZ_HOME should be set by conftest"
        assert "alvarez_test" in alvarez_home, "Should point to test temp dir"

    def test_get_alvarez_home_fallback(self):
        """Without ALVAREZ_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_alvarez_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ALVAREZ_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".alvarez")
            result = _get_alvarez_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._alvarez_bin_dir",
