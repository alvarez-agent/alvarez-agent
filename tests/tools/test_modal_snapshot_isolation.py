...
def _restore_tool_modules():
    original_alvarez_home = os.environ.get("ALVAREZ_HOME")
    original_modules = {
        name: module
        ...
        or name == "alvarez_cli"
        or name.startswith("alvarez_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_alvarez_home is None:
            os.environ.pop("ALVAREZ_HOME", None)
        else:
            os.environ["ALVAREZ_HOME"] = original_alvarez_home
        _reset_modules(("tools", "alvarez_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    alvarez_cli = types.ModuleType("alvarez_cli")
    alvarez_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["alvarez_cli"] = alvarez_cli
    alvarez_home = tmp_path / "alvarez-home"
    os.environ["ALVAREZ_HOME"] = str(alvarez_home)
    sys.modules["alvarez_cli.config"] = types.SimpleNamespace(
        get_alvarez_home=lambda: alvarez_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="alvarez-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": alvarez_home / "modal_snapshots.json",
