# Alvarez Agent - Development Guide

Instructions for AI coding assistants and developers working on the alvarez-agent codebase.

**Never give up on the right solution.**

## What Alvarez Is

Alvarez is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Alvarez ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`alvarez tools`, `alvarez setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `ALVAREZ_HOME`. Mocks hide integration bugs.
...
- **New `ALVAREZ_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `alvarez tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.alvarez/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `alvarez <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `alvarez webhook`, `alvarez cron`, `alvarez tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.alvarez/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.alvarez/alvarez-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _ALVAREZ_CORE_TOOLS list
├── cli.py                # AlvarezCLI class — interactive CLI orchestrator (~11k LOC)
├── alvarez_state.py       # SessionDB — SQLite session store (FTS5 search)
├── alvarez_constants.py   # get_alvarez_home(), display_alvarez_home() — profile-aware paths
├── alvarez_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── alvarez_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── alvarez-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `alvarez --tui`
...
**User config:** `~/.alvarez/config.yaml` (settings), `~/.alvarez/.env` (API keys only).
**Logs:** `~/.alvarez/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_alvarez_home()`.
Browse with `alvarez logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Alvarez: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`alvarez_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `AlvarezCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.alvarez/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`alvarez_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/alvarez` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `alvarez_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `AlvarezCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `alvarez --tui` or `ALVAREZ_TUI=1`.
...
npm run dev       # watch mode (rebuilds alvarez-ink + tsx --watch)
npm start         # production
npm run build     # full build (alvarez-ink + tsc)
...
### TUI in the Dashboard (`alvarez dashboard` → `/chat`)

The dashboard embeds the real `alvarez --tui` — **not** a rewrite.  See `alvarez_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `alvarez_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `alvarez --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `alvarez --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@alvarez/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `alvarez serve` backend server (the same gateway `dashboard` serves, minus the browser UI). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.cjs` + `backendSupportsServe()` in `main.cjs`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `alvarez` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `alvarez --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `alvarez-desktop-app-work` skill, not `alvarez-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Alvarez built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Alvarez core.
Use the plugin route instead: create `~/.alvarez/plugins/<name>/plugin.yaml`
and `~/.alvarez/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Alvarez tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_ALVAREZ_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_ALVAREZ_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_alvarez_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `ALVAREZ_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_alvarez_home()` for the base directory — never `Path.home() / ".alvarez"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `alvarez_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `alvarez_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `alvarez tools`, `alvarez setup`, most CLI subcommands | `alvarez_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`alvarez_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
alvarez_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.alvarez/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Alvarez gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `alvarez_cli/skin_engine.py`:
...
Users create `~/.alvarez/skins/<name>.yaml`:
...
Alvarez has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.alvarez/plugins/` and pip-installed entry points.

### General plugins (`alvarez_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.alvarez/plugins/`, `./.alvarez/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `alvarez` at startup so
  `alvarez <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(alvarez_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `alvarez <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `alvarez --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `alvarez_cli/main.py`, etc.).
...
into `~/.alvarez/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `alvarez memory setup` / `post_setup()` without
...
`~/.alvarez/plugins/` (or via pip entry points). They register through
...
2. User: `$ALVAREZ_HOME/plugins/model-providers/<name>/`
...
the upstream `alvarez-example-plugins` repo
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `alvarez skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.alvarez.tags`, `metadata.alvarez.category`,
`metadata.alvarez.related_skills`, `metadata.alvarez.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.alvarez.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Alvarez tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Alvarez Agent" is the secondary collaborator. If the
   contributor's commit shows "Alvarez Agent" as author (because they
   used Alvarez to draft the skill), replace it with their actual name
...
lives in the `alvarez-agent-dev` skill at
...
`"messaging"`); `_ALVAREZ_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `alvarez tools` (the curses UI) or the
...
Alvarez returns a delegation id immediately and the result re-enters the
...
go to `~/.alvarez/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `alvarez_cli/curator.py` wires `alvarez curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.alvarez/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `alvarez cron <verb>`
...
- File lock at `~/.alvarez/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `alvarez kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `alvarez_cli/kanban.py` wires `alvarez kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`alvarez-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `ALVAREZ_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Alvarez-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `ALVAREZ_BACKGROUND_NOTIFICATIONS` env var):
...
Alvarez supports **profiles** — multiple fully isolated instances, each with its own
`ALVAREZ_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `alvarez_cli/main.py` sets
`ALVAREZ_HOME` before any module imports. All `get_alvarez_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_alvarez_home()` for all ALVAREZ_HOME paths.** Import from `alvarez_constants`.
   NEVER hardcode `~/.alvarez` or `Path.home() / ".alvarez"` in code that reads/writes state.
   ```python
   # GOOD
   from alvarez_constants import get_alvarez_home
   config_path = get_alvarez_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".alvarez" / "config.yaml"
   ```

2. **Use `display_alvarez_home()` for user-facing messages.** Import from `alvarez_constants`.
   This returns `~/.alvarez` for default or `~/.alvarez/profiles/<name>` for profiles.
   ```python
   # GOOD
   from alvarez_constants import display_alvarez_home
   print(f"Config saved to {display_alvarez_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.alvarez/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_alvarez_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_alvarez_home()`,
   not `Path.home() / ".alvarez"`.

4. **Tests that mock `Path.home()` must also set `ALVAREZ_HOME`** — since code now uses
   `get_alvarez_home()` (reads env var), not `Path.home() / ".alvarez"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"ALVAREZ_HOME": str(tmp_path / ".alvarez")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not ALVAREZ_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".alvarez" / "profiles"`, NOT `get_alvarez_home() / "profiles"`.
   This is intentional — it lets `alvarez -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.alvarez` paths
Use `get_alvarez_home()` from `alvarez_constants` for code paths. Use `display_alvarez_home()`
for user-facing print/log messages. Hardcoding `~/.alvarez` breaks profiles — each profile
has its own `ALVAREZ_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `alvarez_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `alvarez_cli/curses_ui.py` — see
`alvarez_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `ALVAREZ_HOME`.

### Tests must not write to `~/.alvarez/`
The `_isolate_alvarez_home` autouse fixture in `tests/conftest.py` redirects `ALVAREZ_HOME` to a temp dir. Never hardcode `~/.alvarez/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_alvarez_home()` resolve within the temp dir.
Use the pattern from `tests/alvarez_cli/test_profiles.py`:
```python
@pytest.fixture
...
| HOME / `~/.alvarez/` | Your real config+auth.json                  | Temp dir per test                         |
